...
`docs/system/` · `docs/generated/` · `docs/features/{slug}/` (**2 files**, v27.0.0 — down from 4:
`functional-spec.md` BA/QA + `technical-spec.md` dev) · `docs/flows/` · `docs/decisions/`.
...
| Path | Surgical edit? |
|---|---|
| `docs/generated/*` | YES — raw inventories (route-list, entities, permissions-matrix, user-stories, feature-list, behavior-logic) |
| `docs/system/*` | YES (guardrailed prose) — curated narratives; forward-authored (Cap. A), reconciled by Core pass |
| `docs/features/*/technical-spec.md` | YES — BR/SM/ALG/INT/DEC table rows |
| `docs/features/*/functional-spec.md` | YES — **per section**, not per file (v27.0.0, D-d). See `## Functional-Spec Section Guardrails` below — this is the security-relevant guardrail; do not treat this whole file as one guardrailed-prose unit |
...
## Functional-Spec Section Guardrails (v27.0.0, D-d)

`functional-spec.md` replaces the three former per-file guardrails (`business-context.md`,
`screens.md`, `edge-cases.md`) with ONE file split into 10 numbered H2 sections. File-level
protection does not carry over section-for-section — **the rule is stated as which SECTIONS this
agent may patch, never which files**. This is the security-relevant edit in the v27 split: loosening
file-level protection into section-level protection must never widen what gets rewritten.

**Default-deny.** A section not listed below as human-editable is machine-owned. When in doubt,
treat the section as machine-owned and skip it.

| Section | Owner | Edit rule |
|---|---|---|
| § 1 Overview | **human-editable** | Narrative prose (Problem/Solution/Users/Goals/Non-Goals) — patch within section, same discipline as the old `business-context.md` guardrail |
| § 2 Open Decisions | machine-owned | D### table rows only — add a row per new `[NEEDS_DOMAIN_CONFIRMATION]`, never rewrite an existing row's prose |
| § 3 Requirements | machine-owned | FR-### one-liners, banded (0xx/1xx/2xx-3xx/4xx/6xx) — add/remove/edit rows, preserve numbering |
| § 4 Business Rules | machine-owned | BR-###/DEC-###/SM-### one-liners — row-level edits only |
| § 5 Screens | machine-owned | Screen table + SCR### codes + `### User Journey` — row/step-level edits, same discipline as the old `screens.md` guardrail |
| § 6 User Stories | machine-owned | US### narrative + acceptance-criteria checkboxes — regenerated by `rebuild-spec`, not surgically authored here |
| § 7 Scenarios | **human-editable** | Given/When/Then wording — patch within section |
| § 8 Edge Cases | machine-owned | Table rows — same discipline as the old `edge-cases.md` guardrail |
| § 9 Edge Behaviours to Verify | machine-owned | FR → plain-language-behaviour trace lines |
| § 10 Configuration | machine-owned | Fenced `name = value` block — regenerated by `rebuild-spec`, not surgically authored here |

**MAY edit (machine-owned sections):** add/remove/edit rows or one-liners within the section; preserve
every code family (FR/BR/SM/DEC/SCR/US) verbatim; never touch a human-editable section while doing so.

**MAY edit (human-editable sections, § 1 and § 7 ONLY):** patch prose within the section under the
same rules as `## Prose-Edit Guardrails` below (user-lock check, patch-in-place, never full-rewrite).

**MUST NOT:** rewrite a machine-owned section's structure, table schema, or codes; infer that silence
about a section means it is editable — silence means machine-owned; touch §§ 2/3/4/5/6/8/9/10 as if
they were free-form prose just because the file also contains § 1/§ 7 prose.

**Five-section walk (why the merge does not leak edit rights):**
1. § 2 Open Decisions — was `business-context.md`'s domain-confirmation content pre-merge (that file
   had no table structure); now a structured table. Still machine-owned: no upgrade in agent write access.
2. § 3 Requirements — was `technical-spec.md`'s **Requirements fulfilled:** field, restated as one-liners
   here. Still machine-owned, same as its technical-spec.md counterpart.
3. § 4 Business Rules — was `technical-spec.md`'s **Rule:**/`user_visible_outcome:** fields, restated
   as one-liners here. Still machine-owned, same as its technical-spec.md counterpart.
4. § 5 Screens — was `screens.md`'s `## Screen List` table verbatim. Still machine-owned, unchanged
   permission from the pre-merge file.
5. § 9 Edge Behaviours to Verify — was `technical-spec.md`'s **Verification** section, restated in
   plain language. Still machine-owned, same as its technical-spec.md counterpart.

None of the five gained human-editable status by moving into the merged file — the merge changed
*where* the content lives, never *who* may rewrite it.

## Prose-Edit Guardrails

Applies to the **guardrailed-prose** paths: `docs/features/*/functional-spec.md` (**§ 1 and § 7
ONLY** — see `## Functional-Spec Section Guardrails` above for the other 8 sections),
