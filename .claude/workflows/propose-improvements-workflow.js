...
async function runProposalWorkflow() {
  ...
  const paths = await agent(
    [
      ...
      '1. Resolve the bundled workflow dir wfRoot: first existing of  ./.claude/workflows/propose-improvements-workflow  OR  ~/.claude/workflows/propose-improvements-workflow  OR  ./claude/workflows/propose-improvements-workflow  (repo-relative). It must contain lib/, references/ and templates/ subdirs. Return its absolute path as wfRoot.',
  ...
  if (!paths?.wfRoot) {
    const msg = 'BLOCKED — could not locate the bundled workflow dir (expected lib/references/templates under ./.claude/workflows/propose-improvements-workflow, ~/.claude/workflows/propose-improvements-workflow or ./claude/workflows/propose-improvements-workflow)';
    log(msg);
