...
/**
 * Compare two Dirent entries by name in code-unit order — the JS equivalent of
 * Python's `sorted(..., key=lambda p: p.name)`. Directory iteration order is
 * filesystem-dependent, so every read that feeds `signals` sorts through this
 * to keep the JSON byte-identical to detect_sdd.py (see __tests__/parity.test.mjs).
 * Deliberately not localeCompare: that is ICU/locale-dependent and would drift.
 */
...
function* _walk(root, maxDepth = 6, fileFilter = () => true) {
  ...
  while (stack.length > 0) {
    ...
    let entries;
    try {
      entries = fs.readdirSync(cur, { withFileTypes: true }).sort(_byName);
    } catch {
      continue;
    }
...
function _toolingMarkerSignals(repoRoot) {
  ...
  try {
    ...
    if (wfStat.isDirectory()) {
      let entries;
      try { entries = fs.readdirSync(workflowsDir, { withFileTypes: true }).sort(_byName); } catch { entries = []; }
      for (const wanted of ['.yml', '.yaml', '.json', '.md']) {
        for (const entry of entries) {
          if (!entry.isFile()) continue;
          if (path.extname(entry.name).toLowerCase() !== wanted) continue;
...
function _planDirSignals(repoRoot) {
  ...
  const out = [];
  let children;
  try { children = fs.readdirSync(plansDir, { withFileTypes: true }).sort(_byName); } catch { return []; }
