...
describe('subagent-init.cjs', () => {
  ...
  describe('Issue #540: ck plan CLI injection for plan-aware agents', () => {
    ...
    it('injects ck plan CLI section for plan-aware agent types', async () => {
      const planAwareTypes = ['planner', 'delivery-tracker', 'code-simplifier', 'brainstormer', 'reviewer', 'implementer'];
  ...
  describe('Monorepo/Submodule Scenarios', () => {
    ...
    it('outputs TKM_DEBUG info when enabled', async () => {
      const result = await runHook({
        agent_type: 'test-agent',
        agent_id: 'debug-test',
        cwd: process.cwd()
      ...
      if (process.env.TKM_DEBUG || result.stderr.includes('effectiveCwd')) {
        assert.ok(
  ...
  describe('Plan 260513-1134: Project Docs Index injection', () => {
    ...
    it('injects index for planner with current rebuild-spec layout (docs/features/{slug}/)', async () => {
      const dir = makeTempProject();
      try {
        // Current canonical layout: docs/features/F###_*/technical-spec.md,
        // docs/generated/feature-list.md, docs/system/overview.md
        const featuresDir = path.join(dir, 'docs', 'features');
        fs.mkdirSync(path.join(dir, 'docs', 'generated'), { recursive: true });
        fs.mkdirSync(path.join(dir, 'docs', 'system'), { recursive: true });
        fs.writeFileSync(path.join(dir, 'docs', 'generated', 'feature-list.md'), '#');
        fs.writeFileSync(path.join(dir, 'docs', 'system', 'overview.md'), '#');
        fs.mkdirSync(path.join(featuresDir, 'F001_Auth'), { recursive: true });
        fs.writeFileSync(path.join(featuresDir, 'F001_Auth', 'technical-spec.md'), '#');
        fs.mkdirSync(path.join(featuresDir, 'F002_Profile'), { recursive: true });
        fs.writeFileSync(path.join(featuresDir, 'F002_Profile', 'technical-spec.md'), '#');
        ...
        assert.ok(ctx.includes('docs/generated/feature-list.md'), 'Should surface feature-list landmark by path');
        assert.ok(ctx.includes('docs/system/overview.md'), 'Should surface overview landmark by path');
        assert.ok(/docs\/features\/ — 2 feature specs/.test(ctx), 'Should summarize features dir');
      } finally { cleanup(dir); }
    });

    it('injects index for planner with legacy docs/specs/ layout (backward compat)', async () => {
      const dir = makeTempProject();
      try {
    ...
    it('relays docs.maxLoc on the Paths line so a subagent sees the ceiling', async () => {
      const dir = makeTempProject();
      try {
        // A spawned subagent never sees the main session's `## Paths` line, so the
        // doc size ceiling has to arrive through this hook or not at all.
        fs.mkdirSync(path.join(dir, '.claude'), { recursive: true });
        fs.writeFileSync(path.join(dir, '.claude', '.tkm.json'), JSON.stringify({ docs: { maxLoc: 300 } }));
        const result = await runHook({ agent_type: 'doc-writer', agent_id: 't-maxloc', cwd: dir }, { cwd: dir });
        const ctx = result.output?.hookSpecificOutput?.additionalContext || '';
        assert.match(ctx, /- Paths: .*docs\.maxLoc: 300/, 'configured ceiling should reach the subagent');
      } finally { cleanup(dir); }
    });

    it('falls back to 800 when no docs.maxLoc is configured', async () => {
      const dir = makeTempProject();
      try {
        const result = await runHook({ agent_type: 'doc-writer', agent_id: 't-maxloc-default', cwd: dir }, { cwd: dir });
        const ctx = result.output?.hookSpecificOutput?.additionalContext || '';
        assert.match(ctx, /- Paths: .*docs\.maxLoc: 800/, 'default ceiling should be reported, not omitted');
      } finally { cleanup(dir); }
    });
