...
### Step 1.5 — Pre-upload encoding check (HTML only, MANDATORY)

The web viewer serves HTML inside a sandboxed iframe. An HTML file with **no charset declaration** makes the browser guess the encoding — Vietnamese/Japanese text then renders as mojibake ("lỗi font"). The file bytes are fine; only the declaration is missing. Users blame the Takumi web for this, so fix it **before** upload, silently and automatically.

Run this check on every `.html`/`.htm` file about to be uploaded. For a **folder** upload, check only the HTML files the CLI will actually upload — never touch ignored files:

```bash
# Folder: enumerate HTML files that survive .gitignore (git folders)
git -C <folder> ls-files --cached --others --exclude-standard -- '*.html' '*.htm'
# Non-git folder: find <folder> -iname '*.html' -o -iname '*.htm'
# (rare: a .tkmignore with !negation may re-include a gitignored file — check it by eye)
```

Per file, two checks:

```bash
# 1. Is utf-8 explicitly declared? (covers <meta charset=utf-8> AND http-equiv Content-Type)
grep -qiE "<meta[^>]+charset=['\" ]*utf-?8" <file> && echo OK
# 1b. Any charset declared at all? (if 1 fails but this hits → declared but NOT utf-8)
grep -qiE '<meta[^>]+charset' <file> && echo DECLARED_OTHER

# 2. Confirm the file bytes really are valid UTF-8 before declaring/correcting
iconv -f UTF-8 -t UTF-8 <file> > /dev/null 2>&1 && echo VALID_UTF8
```
...
| Result                                     | Action                                                                                                    |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `utf-8` already declared                   | Nothing — proceed to upload                                                                               |
| Declared but NOT utf-8 + file is valid UTF-8 | Correct the existing declaration to `utf-8` with the Edit tool (stale boilerplate like `ISO-8859-1` causes the exact same mojibake). Tell the user in ONE line what was corrected. |
| No declaration + file is valid UTF-8       | Insert `<meta charset="utf-8">` with the Edit tool: right after the opening `<head...>` tag; no `<head>` → right after `<html...>`; neither → right after `<!DOCTYPE ...>` if present, else prepend as the first line. Tell the user in ONE line: `Đã thêm <meta charset="utf-8"> vào <file> để tránh lỗi font tiếng Việt trên web.` |
| File is NOT valid UTF-8                    | Do **not** insert or correct (declaring utf-8 on non-UTF-8 bytes makes it worse). Warn the user, name the file, suggest converting to UTF-8 first (`iconv -f <encoding> -t UTF-8`) — ask before uploading as-is. |

Only touch the charset declaration — never reformat or otherwise rewrite the file content.
...
- `tkm:generate-slide --html` — slide deck HTML (`extras` kit)
