...
5. **Blueprint Documentation** → Hand the planner subagent the job of writing the full plan
   - **Capability tier:** select the planner variant by `--level` per `../_shared/processing-levels.md`
     → "Model & Effort per Level": `--level low` → `planner-lite`, `--level medium`/`high` → `planner`,
     `--level max` → `planner-max`. Spawn that variant as the subagent — the variants share one blueprint
     contract and differ only in model / reasoning effort.
   ...
   - **`--level low`:** Even when Stage 1.5 was minimal (a partial draft), the planner MUST still
     ...
     `/tkm:rebuild-spec --features F###` run completes the 2-file set (v27.0.0 — `functional-spec.md` +
