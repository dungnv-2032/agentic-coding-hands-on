...
description: "Audit the JUDGMENT in rebuild-spec's synthesis tier — not coverage (A1), not truth (audit-doc-parity), but whether the feature partition is well-drawn, the user-story enumeration sound, the claimed 'why' a valid inference, and the artifact set complete + non-redundant. Report-only; emits a machine-readable PASS/FAIL. Reach for it on 'is the partition right', 'are the user stories over/under-merged', 'is this capability one outcome', 'audit synthesis judgment', as a post-rebuild-spec soundness gate before takumi consumes the specs."
argument-hint: "[--scope feature-list|user-stories|system|all] [--plan-dir <p>] [--docs-root <d>] [--strict-coverage] [--level low|medium|high|max]"
metadata:
  author: takumi-agent-kit
  version: "2.0.0"
module: testing-code-quality
triggers: ["audit synthesis judgment", "is the partition right", "over-merge", "under-merge", "user stories sound", "capability intent", "judgment soundness", "synthesis soundness", "feature partition audit"]
---
...
"why" a user story claims is a defensible inference or a guess, and whether the artifact set is
complete without emitting the same artifact twice. A coverage stat can't see a bad partition. A
truth audit can't — by construction — grade an inference (audit-doc-parity's Iron Law #6 forbids it). This skill is
...
| **3 — adversarial judgment** | inference validity ("why"/"so that"), boundary naming, granularity, capability intent (does a § 2 CAP row bundle >1 business outcome) | WARN | LLM judges + refutation pass |
...
- **WARN** — boundary/conformance/granularity/naming/inference/capability-intent issues
  (severity-tagged). Engine-2 and Engine-3 findings are **WARN only, never FAIL**.
...
Sub-kinds (`OVER_MERGE`, `UNDER_SPLIT`, `NAMING`, `UNSUPPORTED`, `MISSING_US`, `CAP_MULTI_INTENT`, …)
are **tags** on a WARN, not separate verdicts. Full text in
[`references/verdict-taxonomy.md`](references/verdict-taxonomy.md).
...
**⚠️ DON'T:** flag a citation-adjacent span (mandated ADR quotes) as redundancy · assert a phantom on a no-/partial-graph language (→ UNVERIFIABLE) · run the default `--scope all` sweep without the estimate gate.
...
flowchart TD
    ...
    D --> E3["Engine 3 — judgment_engine.py<br/>inference/naming/granularity/capability-intent + refutation (WARN)"]
...
/tkm:audit-synthesis-judgment --scope system --plan-dir plans/260721-1043-foo  # + the plan-dir validator roll-up
```

- `--plan-dir` is optional. Engine 1 passes it to the plan-dir validator subset (`validate_feature_existence`, `validate_id_contiguity`); with no plan dir those two simply do not run and are absent from the roll-up.
