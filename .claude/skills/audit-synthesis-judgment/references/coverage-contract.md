...
**Excluded (16), by design:** `validate_route_list`, `validate_screen_list`, `validate_screen_flow`,
`validate_api_map`, `validate_api_contracts`, `validate_crud_matrix`, `validate_db_catalog`,
`validate_behavior_logic`, `validate_process_flow`, `validate_test_cases`,
`validate_reading_guide_db_impact`, `validate_feature_api_link`, `validate_feature_screen_link`,
`validate_deployment_view`, `validate_traceability_matrix` —
