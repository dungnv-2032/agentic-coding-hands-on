...
```
Resolves the lang-mapped docs root and emits the artifact manifest; every artifact resolves
under that root. `--plan-dir` is accepted for CLI parity and ignored here — Engine 1 is its only
reader. Feeds all three engines.
...
1. `judgment_engine.py prepare --scope <s> [--plan-dir <p>] --out candidates.json`
   → extracts candidates (inference / naming / granularity / capability-intent), each
     pinned to a computed anchor.
