...
# --------------------------------------------------------------- advisory validator roll-up
_STUB_VALIDATOR = '''\
import argparse, json, pathlib
p = argparse.ArgumentParser()
p.add_argument("--summary-out")
a, _ = p.parse_known_args()
pathlib.Path(a.summary_out).write_text(
    json.dumps({"overall_status": "PASS", "totals": {"critical": 0, "warning": 0}}))
'''


class TestValidatorSubset:
    """The roll-up is invoked with `run_validators=False` by every other test here, so its call
    shape is otherwise unexercised — a green suite would not notice a validator being dropped from
    the subset or an extra one creeping back in. These tests stub the rebuild-spec scripts dir so
    the assertion is on the ENUMERATED set itself, exactly, not on a superset."""

    def _stub_scripts(self, tmp_path, monkeypatch, names):
        stubs = tmp_path / "stub-scripts"
        stubs.mkdir()
        for name in names:
            (stubs / f"{name}.py").write_text(_STUB_VALIDATOR, encoding="utf-8")
        monkeypatch.setattr(coverage_engine, "_REBUILD_SCRIPTS", stubs)
        return stubs

    def _all_stub_names(self):
        return (coverage_engine._DOCS_ROOT_VALIDATORS
                + coverage_engine._PLAN_DIR_VALIDATORS
                + ["validate_id_contiguity"])

    def test_docs_root_subset_is_exactly_the_enumerated_set(self, tmp_path, monkeypatch):
        self._stub_scripts(tmp_path, monkeypatch, self._all_stub_names())
        res = coverage_engine._run_validator_subset(tmp_path / "docs", None, tmp_path)
        assert set(res) == set(coverage_engine._DOCS_ROOT_VALIDATORS)
        assert all(v["ran"] is True and v["overall_status"] == "PASS" for v in res.values())

    def test_plan_dir_adds_exactly_the_plan_dir_validators(self, tmp_path, monkeypatch):
        self._stub_scripts(tmp_path, monkeypatch, self._all_stub_names())
        res = coverage_engine._run_validator_subset(tmp_path / "docs", str(tmp_path), tmp_path)
        assert set(res) == set(self._all_stub_names())

    def test_missing_validator_is_recorded_not_raised(self, tmp_path, monkeypatch):
        self._stub_scripts(tmp_path, monkeypatch, [])  # none present on disk
        res = coverage_engine._run_validator_subset(tmp_path / "docs", None, tmp_path)
        assert set(res) == set(coverage_engine._DOCS_ROOT_VALIDATORS)
        assert all(v["ran"] is False and "not found" in v["reason"] for v in res.values())

    def test_run_threads_the_subset_through_with_no_extra_artifact_argument(self, tmp_path, monkeypatch):
        """`run(..., run_validators=True)` must reach the roll-up with the surviving 3-arg shape."""
        root, g = TestCoverageEngineE2E()._corpus(tmp_path)
        self._stub_scripts(tmp_path, monkeypatch, self._all_stub_names())
        res = coverage_engine.run(root, "all", None, None, g, False, None,
                                  do_preflight=False, run_validators=True)
        assert set(res["validators"]) == set(coverage_engine._DOCS_ROOT_VALIDATORS)
