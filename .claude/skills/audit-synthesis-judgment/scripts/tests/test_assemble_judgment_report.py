...
class TestStatusPropagation:
    ...
    def test_granularity_skipped_note_surfaces_in_report_body(self):
        """phase 09: before this phase a v26/half-migrated run just returned zero granularity
        findings — indistinguishable from "ran clean". The report body must say SO, loudly, and
        it must not flip `result` (WARN-only, never a FAIL source)."""
        judgment = {"judgment_status": "OK", "findings": [],
                    "granularity_status": "SKIPPED",
                    "granularity_note": "no functional-spec.md located under /repo/docs/features/*/functional-spec.md"}
        report, data = asm.assemble(_cov(), {}, judgment, "all")
        assert data["granularity_status"] == "SKIPPED"
        assert data["granularity_note"] == judgment["granularity_note"]
        assert data["result"] == "PASS"  # never folded into the FAIL rule
        assert "granularity_status" in report and "SKIPPED" in report
        assert judgment["granularity_note"] in report

    def test_granularity_ok_status_surfaces_without_note_noise(self):
        judgment = {"judgment_status": "OK", "findings": [],
                    "granularity_status": "OK", "granularity_note": "granularity ran on 10 features"}
        report, data = asm.assemble(_cov(), {}, judgment, "all")
        assert data["granularity_status"] == "OK"
        assert "granularity_status: **OK**" in report

    def test_granularity_defaults_to_ok_when_judgment_absent(self):
        _r, data = asm.assemble(_cov(), {}, {}, "all")
        assert data["granularity_status"] == "OK"
        assert data["granularity_note"] is None

    # ----------------------------------------------------- capability-intent (phase 10, FM-5)
    def test_capability_intent_skipped_note_surfaces_in_report_body(self):
        """[FM-5] Mirrors `test_granularity_skipped_note_surfaces_in_report_body` exactly — a
        half-migrated corpus (no § 2 row claims any US) must say SO loudly, and never flip
        `result` (WARN-only, never a FAIL source)."""
        judgment = {"judgment_status": "OK", "findings": [],
                    "capability_intent_status": "SKIPPED",
                    "capability_intent_note": "4 functional-spec.md found under /repo/docs/features/*/"
                                              "functional-spec.md but no § 2 row claims any User Stories"}
        report, data = asm.assemble(_cov(), {}, judgment, "all")
        assert data["capability_intent_status"] == "SKIPPED"
        assert data["capability_intent_note"] == judgment["capability_intent_note"]
        assert data["result"] == "PASS"  # never folded into the FAIL rule
        assert "capability_intent_status" in report and "SKIPPED" in report
        assert judgment["capability_intent_note"] in report

    def test_capability_intent_ok_status_surfaces_without_note_noise(self):
        judgment = {"judgment_status": "OK", "findings": [],
                    "capability_intent_status": "OK",
                    "capability_intent_note": "capability-intent ran on 66 functional-spec.md; "
                                              "10 CAP row(s) claim >= 3 US"}
        report, data = asm.assemble(_cov(), {}, judgment, "all")
        assert data["capability_intent_status"] == "OK"
        assert "capability_intent_status: **OK**" in report

    def test_capability_intent_defaults_to_ok_when_judgment_absent(self):
        _r, data = asm.assemble(_cov(), {}, {}, "all")
        assert data["capability_intent_status"] == "OK"
        assert data["capability_intent_note"] is None
