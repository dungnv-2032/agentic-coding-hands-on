...
def assemble(coverage: dict, boundary: dict, judgment: dict, scope: str) -> tuple[str, dict]:
    ...
    # phase 09: Engine 3's § 2 granularity axis carries its own completion status, distinct from
    # judgment_status (which covers the LLM-judged candidates as a whole) — a v26/half-migrated
    # corpus must say SO, not merely return zero granularity findings indistinguishable from "ran
    # clean". Never folded into `_result()` — WARN-only, same as every other Engine-3 signal.
    granularity_status = judgment.get("granularity_status", "OK") if judgment else "OK"
    granularity_note = judgment.get("granularity_note") if judgment else None
    # phase 10: capability-intent's own completion status, mirroring granularity_status exactly
    # — a half-migrated corpus (§ 2 widened, every US cell unfilled) must say SO rather than
    # return zero CAP_MULTI_INTENT findings indistinguishable from "ran clean". WARN-only, never
    # folded into `_result()`.
    capability_intent_status = judgment.get("capability_intent_status", "OK") if judgment else "OK"
    capability_intent_note = judgment.get("capability_intent_note") if judgment else None
    ...
    fm = [
        ...
        f"inference_warn: {len(judgment_findings)}   # Engine 3 (UNSUPPORTED / granularity / naming / CAP_MULTI_INTENT) → WARN",
        ...
        f"granularity_status: {granularity_status}     # SKIPPED = § 2 axis did not run, "
        "never a verified zero-outlier PASS",
        f"capability_intent_status: {capability_intent_status}     # SKIPPED = no § 2 row claims "
        "any US, never a verified zero-CAP_MULTI_INTENT PASS",
        f"result: {result}",
        "---",
    ]
    ...
    def _warn_section(items):
        ...
        for i, f in enumerate(items, 1):
            ...
            sev = f.get("severity", "")
            out.append(f"- **Severity**: {sev} · **Verdict**: WARN")
            out.append("")
        return "\n".join(out)
    ...
    parts = [
        ...
        f"- granularity_status: **{granularity_status}**"
        + (f" — {granularity_note}" if granularity_status == "SKIPPED" and granularity_note else ""),
        f"- capability_intent_status: **{capability_intent_status}**"
        + (f" — {capability_intent_note}"
           if capability_intent_status == "SKIPPED" and capability_intent_note else ""),
        "",
        "## How to remediate", "",
        ...
        "- `granularity_status: SKIPPED` → the § 2 axis did not run (see the note above); "
        "fill or migrate § 2, then re-audit — this is NOT a verified zero-outlier PASS.",
        "- `capability_intent_status: SKIPPED` → no § 2 row claims any US (see the note above); "
        "fill § 2, then re-audit — this is NOT a verified zero-CAP_MULTI_INTENT PASS.",
        "- WARN → a human weighs each; none blocks.", "",
    ]
    frontmatter_data = {
        ...
        "boundary_status": boundary_status, "judgment_status": judgment_status,
        "granularity_status": granularity_status, "granularity_note": granularity_note,
        "capability_intent_status": capability_intent_status,
        "capability_intent_note": capability_intent_note,
        "result": result,
        "_gate_log": log,
    }
