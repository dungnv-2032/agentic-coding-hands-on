...
### feature-spec fan-out  (Wave 6 — per-feature, 2-file audience-split model)
- Read: scoped `<specsRoot>/*/spec.md` + UserStories + FeatureList for that F###.
- Templates (2 files per feature):
  - `claude/skills/rebuild-spec/templates/functional-spec-template.md`
  - `claude/skills/rebuild-spec/templates/technical-spec-template.md`
- Output per feature: `artifacts/features/{slug}/functional-spec.md`,
  `technical-spec.md`.
  (NOT a single spec.md — the pair splits by AUDIENCE: functional-spec.md is
  the BA/QA half, technical-spec.md is the dev half. The pre-v4 single-file
  template is no longer used; the older, wider per-feature file set from
  before the 27.0.0 audience split is also retired in favor of this pair.)
- `validate_source_citations.py` validates `technical-spec.md` ONLY
  (`functional-spec.md` is skipped by the validator). Every spec-URI citation
  in `technical-spec.md` must resolve under `--specs-root`.
