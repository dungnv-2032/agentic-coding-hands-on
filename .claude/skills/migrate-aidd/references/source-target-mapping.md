...
docs layout (v4): 9 core artifacts + per-feature 2-file audience-split set.
...
```
docs/system/     ← overview.md, permissions.md, glossary.md, architecture.md
...
docs/features/   ← {slug}/functional-spec.md, technical-spec.md
```

There is NO `docs/specs/` in v4. The 2-file audience-split per-feature set
(`functional-spec.md` BA/QA · `technical-spec.md` dev) replaces the old
single `spec.md` output. The `--docs-specs` arg of `promote_drafts.py` is
deprecated/no-op. The former standalone `docs/system/` business-rules narrative
is likewise retired — its content is now carried in `docs/generated/behavior-logic.md`'s
BA-first Index section (see `claude/skills/rebuild-spec/templates/behavior-logic-template.md`).
...
Each reconciled F### produces a **2-file audience-split feature set** (Wave 6):
`artifacts/features/{slug}/functional-spec.md`, `technical-spec.md`.
Promoted to `docs/features/{slug}/` by W9.
