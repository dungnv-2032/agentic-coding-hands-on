...
2. **Detect + locate specs** — run this skill's own `detect_sdd.py`:
   ```
   .claude/skills/.venv/bin/python3 claude/skills/migrate-aidd/scripts/detect_sdd.py \
...
- W6 FeatureSpec fan-out (batches of 5 if >20 F###) — source-agnostic. AS-IS.
  Per-feature output is the **2-file audience-split set**:
  `artifacts/features/{slug}/functional-spec.md`, `technical-spec.md`.
...
| W6 | `researcher` | scoped FeatureList/UserStories + 2 audience-split feature templates | `features/{slug}/functional-spec.md`, `technical-spec.md` |
...
- `claude/skills/rebuild-spec/templates/*` — all 9 core + scout + 2 audience-split feature templates (`functional-spec-template.md`, `technical-spec-template.md`)
...
- `scripts/{detect_sdd,detect_sdd_lib}.py` — SDD detection. Vendored from
  `propose-improvements` when that skill moved to the `extras` kit: this skill ships
  in base, so it cannot call into a path that only exists when extras is installed.
  Keep in sync with the extras copy (both have a `test_detect_sdd.py` suite).

## Output

<!-- layout-exempt: output section — all paths are this skill's own promote targets -->
- `docs/system/{overview,permissions,glossary,architecture}.md` — curated system narratives
...
- `docs/features/{slug}/{functional-spec,technical-spec}.md` — 2-file audience-split per-feature set
- `docs/features/{slug}/contracts/` — verbatim copy of speckit `contracts/` dir when present (P04)
(same canonical home as rebuild-spec v4; promote + stub rules reused. Canonical topic → path
ownership is `claude/skills/_shared/docs-canonical-mapping.md` — this list mirrors it for
convenience and MUST NOT be treated as the source of truth if the two ever disagree.)
