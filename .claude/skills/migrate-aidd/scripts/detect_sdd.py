#!/usr/bin/env python3
"""SDD (Spec-Driven Development) detection CLI — migrate-aidd Preflight step 2.

Detects whether `<spec-folder>` is a Speckit/SDD spec tree and resolves its
`specsRoot`, so migrate-aidd knows what it is migrating before it writes anything.

Vendored from the `propose-improvements` skill, which moved to the `extras` kit.
`migrate-aidd` ships in the base (`engineer`) kit and must not depend on a file
that only exists when extras is installed, so base carries its own copy. The
extras copy (`claude/skills/propose-improvements/scripts/detect_sdd.py`) stays
the reference implementation of `references/sdd-detection.md`; keep the pair in
sync — `propose-improvements-workflow`'s `parity.test.mjs` guards the extras copy
against its JS port, and `tests/test_detect_sdd.py` guards this one.
