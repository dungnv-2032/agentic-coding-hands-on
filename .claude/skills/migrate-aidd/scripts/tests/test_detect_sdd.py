"""Tests for migrate-aidd's vendored SDD detection script.

Base's copy of `detect_sdd.py` / `detect_sdd_lib.py` (see those files' docstrings
for why base carries one). Mirrors the extras-side suite at
`claude/skills/propose-improvements/scripts/test_detect_sdd.py` — keep both green
when either copy changes.
"""
...
# tests/ → scripts/ (this suite lives one level below the script, unlike the
# extras-side copy which sits next to it).
SCRIPT = Path(__file__).resolve().parent.parent / "detect_sdd.py"
