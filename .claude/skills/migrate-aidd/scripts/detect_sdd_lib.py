"""Pure-Python helpers for SDD (Spec-Driven Development) detection.

Vendored alongside `detect_sdd.py` from the `propose-improvements` skill, which
moved to the `extras` kit — see that file's docstring for why base keeps a copy.
The procedure is specified in the extras skill's `references/sdd-detection.md`.
No CLI / I/O orchestration here — see `detect_sdd.py` for the wrapper that
handles stdout, idempotency, and the atomic JSON write.
