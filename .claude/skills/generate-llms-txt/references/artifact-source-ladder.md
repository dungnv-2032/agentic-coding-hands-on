...
## Ladder ranks quality; contract decides inclusion

This ladder answers one question only: **how good is the available source material** (T1 richest,
T4 a rescue rung). It says nothing about *whether a piece of content belongs in an end-user
artifact* — that call belongs to the 7-section content contract and the audience rule in
[`references/end-user-content-contract.md`](./end-user-content-contract.md). A T1 repo's
`docs/system/architecture.md` is top-of-ladder quality and still gets dropped under
`--audience user`, because it describes how the product is built, not how it is used. The mapping
table above (rebuild-spec sections → llms.txt sections) documents the `--audience dev` grouping,
which is v1's `SECTION_ORDER` unchanged; the end-user contract's 7-section shape is a separate,
narrower emission order used only under `--audience user` — see the contract reference for that
table, not here.
