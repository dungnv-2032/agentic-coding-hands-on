...
| `llms-full.txt` | Content inlined (no external links to follow). The agent loads it once and understands everything. Generated **by default**; `--index-only` skips it. |

### Our divergence: curated and capped, not an unbounded dump

The ecosystem's `llms-full.txt` convention is "inline everything" — observed real-world files run
**177k to 3.7M tokens**, well past what fits as attached context in a single turn. This skill's
`llms-full.txt` deliberately diverges: it is governed by the 7-section content contract
(`references/end-user-content-contract.md`) and a token budget (default `50000`, `--budget`
override). Content that does not answer "what does the end user need" is filtered by audience
before it ever reaches the render step, and a fixed trim ladder (`Optional` → API detail →
per-section tightening) brings an over-budget render back under the cap before it ships. The final
size is always reported as an estimate (`chars/4`), including when the cap could not be met.

**This is a deliberate choice, not an oversight.** A future maintainer tempted to "restore" the
ecosystem's full-dump semantics by removing the cap should read this note first: an unbounded
`llms-full.txt` defeats the one property that makes it useful — that an agent can actually load it
in one pass.
...
The two checks below are the format/quality half of the skill's **six gates**; the other four
(self-containment, contract completeness, audience lint, secret scan) are defined in full —
pass condition, failure action, and the audience-lint remediation rule — in
[`references/end-user-content-contract.md`](./end-user-content-contract.md) § The six gates. All
six run before any `--promote`.
...
- [ ] **Self-containment** — `llms-full.txt` has zero relative links and zero relative image
  links; `self_containment.remaining == 0` after budget trimming.
- [ ] **Contract completeness** — every required contract section is `filled` or `gap` with a
  non-null advisory; never silently absent.
- [ ] **Audience lint** — under `--audience user`, no heading in `llms-full.txt` matches the
  dev-content pattern set (tech stack, architecture, CI/CD, …).
- [ ] **Secret scan** — `secret_scan.status == "clean"`; a non-clean scan withholds
  `llms-full.txt` and promotes the index alone.
