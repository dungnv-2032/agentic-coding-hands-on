...
install_system_deps() {
    ...
    # ImageMagick (required for generate-slide PDF export — extras kit)
    install_system_package "imagemagick" "ImageMagick" "magick,convert"

    # Pandoc (required for translate-file skill, extras kit: Markdown → HTML)
    install_system_package "pandoc" "Pandoc" "pandoc"

    # Calibre/ebook-convert (required for translate-file skill, extras kit: HTML → DOCX/PDF)
    if command_exists "ebook-convert"; then
        print_success "Calibre (ebook-convert) already installed"
        track_success "optional" "Calibre"
    elif [[ "$OS" == "macos" ]]; then
        print_info "Installing Calibre..."
        if brew install --cask calibre 2>/dev/null; then
            print_success "Calibre installed"
            track_success "optional" "Calibre"
        else
            print_warning "Calibre: brew install --cask failed"
            track_failure "optional" "Calibre" "brew install --cask failed"
        fi
    else
        install_system_package "calibre" "Calibre (ebook-convert)" "ebook-convert"
    fi
...
# Install optional external Python CLI tools that skills shell out to (isolated, on PATH).
# licenseal (audit-licenses skill): prefer `uv tool`, fall back to pipx. Best-effort —
# never fails the install; the skill still prints an install hint if the tool is absent.
install_python_cli_tools() {
    print_header "Installing Python CLI Tools"

    # licenseal — audit-licenses skill (dependency license compatibility)
    if command_exists licenseal; then
        local lv
        lv=$(licenseal --version 2>&1 | head -n1 || echo "available")
        print_success "licenseal already installed ($lv)"
    elif command_exists uv; then
        print_info "Installing licenseal via uv tool..."
        if uv tool install licenseal >/dev/null 2>&1; then
            print_success "licenseal installed (uv tool)"
        else
            print_warning "licenseal install via uv failed — run 'uv tool install licenseal' manually"
            track_failure "optional" "licenseal" "uv tool install failed"
        fi
    elif command_exists pipx; then
        print_info "Installing licenseal via pipx..."
        if pipx install licenseal >/dev/null 2>&1; then
            print_success "licenseal installed (pipx)"
        else
            print_warning "licenseal install via pipx failed — run 'pipx install licenseal' manually"
            track_failure "optional" "licenseal" "pipx install failed"
        fi
    else
        print_warning "Neither uv nor pipx found — skipping licenseal. Install uv or pipx, then: uv tool install licenseal"
    fi
}
...
install_node_deps() {
    ...
    npm_packages=(
        ...
        "wrangler:wrangler"
        "repomix:repomix"
        "pptxgenjs:pptxgenjs"   # generate-slide skill (extras kit): Sun* PPTX generation
        "@sun-asterisk/sunlint:sunlint"   # lint-code skill: SunLint static analysis
    )
...
# CLI Version Gate
# ============================================================================
# Defense-in-depth for OLD takumi CLIs. The CLI-side gate refuses installs
# before any writes, but an old CLI predating that gate never checks the kit's
# min_cli_version. This script ships fresh with the kit, so it re-checks here:
# the CLI passes its version via TAKUMI_CLI_VERSION; a missing version means the
# CLI is too old to even report it and is treated as unsupported.

# True (0) when semver core A < B. Compares the numeric MAJOR.MINOR.PATCH core
# only (pre-release / build suffixes stripped), matching the CLI's semver.coerce.
_tkm_semver_lt() {
    local a="$1" b="$2"
    local a_maj a_min a_pat b_maj b_min b_pat
    # Strip everything from the first non-[0-9.] char (e.g. "-dev.42"), then split.
    a="${a%%[!0-9.]*}"
    b="${b%%[!0-9.]*}"
    ...
    if [ "$a_maj" -ne "$b_maj" ]; then [ "$a_maj" -lt "$b_maj" ]; return; fi
    if [ "$a_min" -ne "$b_min" ]; then [ "$a_min" -lt "$b_min" ]; return; fi
    [ "$a_pat" -lt "$b_pat" ]
}

# True (0) when the string starts with a digit — a parseable semver core.
_tkm_is_semver() {
    case "$1" in [0-9]*) return 0 ;; *) return 1 ;; esac
}

# Read min_cli_version from the kit metadata.json (sibling of the skills dir).
# grep/sed avoids a jq dependency (matches the rest of this script's JSON reads).
_tkm_read_min_cli_version() {
    local metadata="$SCRIPT_DIR/../metadata.json"
    [ -f "$metadata" ] || return 0
    grep -o '"min_cli_version"[[:space:]]*:[[:space:]]*"[^"]*"' "$metadata" 2>/dev/null \
        | head -1 \
        | sed 's/.*:[[:space:]]*"//; s/"$//'
}

# Refuse to install when the running CLI is below the kit's declared floor.
# Lenient by default: no floor, an unparseable floor, or an unparseable CLI
# version all proceed. Only a present-but-too-low or a missing CLI version (with
# a floor declared) blocks. TKM_SKIP_CLI_VERSION_CHECK=1 overrides (for the
# documented manual `bash install.sh` path, which has no CLI to report a version).
check_cli_version() {
    local min cli
    min="$(_tkm_read_min_cli_version)"
    [ -n "$min" ] || return 0                 # no floor → allow
    _tkm_is_semver "$min" || return 0         # unparseable floor → allow

    case "${TKM_SKIP_CLI_VERSION_CHECK:-}" in
        1|true|yes|TRUE|YES)
            print_warning "CLI version check bypassed (TKM_SKIP_CLI_VERSION_CHECK); this kit expects takumi CLI >= $min"
            return 0
            ;;
    esac

    cli="${TAKUMI_CLI_VERSION:-}"
    if [ -z "$cli" ]; then
        print_error "This kit requires takumi CLI >= $min, but the CLI did not report its version."
        print_info "Your takumi CLI is too old to install this kit safely. Upgrade with:"
        print_info "  npm install -g @sunasteriskrnd/takumi"
        print_info "Then re-run the installer, or set TKM_SKIP_CLI_VERSION_CHECK=1 to override."
        exit 1
    fi

    # A version we can't parse shouldn't hard-block — treat as unknown/allow.
    _tkm_is_semver "$cli" || return 0

    if _tkm_semver_lt "$cli" "$min"; then
        print_error "This kit requires takumi CLI >= $min, but the installed CLI is $cli."
        print_info "Upgrade with:"
        print_info "  npm install -g @sunasteriskrnd/takumi"
        print_info "Then re-run the installer, or set TKM_SKIP_CLI_VERSION_CHECK=1 to override."
        exit 1
    fi
...
main() {
    # Refuse before any work if the installing CLI is below the kit's floor.
    check_cli_version
    ...
    # Optional external CLI tools (licenseal for audit-licenses). Idempotent + best-effort,
    # so it runs every time without its own resume phase — command_exists short-circuits.
    install_python_cli_tools
