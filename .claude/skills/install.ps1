...
function Install-SystemDeps {
    ...
    if (Test-Command "pandoc") {
        Write-Success "Pandoc already installed"
        Track-Success -Category "optional" -Name "Pandoc"
    } else {
        $null = Install-WithPackageManager `
            -DisplayName "Pandoc" `
    ...
    if (Test-Command "ebook-convert") {
        Write-Success "Calibre (ebook-convert) already installed"
        Track-Success -Category "optional" -Name "Calibre"
    } else {
        $null = Install-WithPackageManager `
            -DisplayName "Calibre (ebook-convert)" `
            -WingetId "Calibre.Calibre" `
            -ChocoName "calibre" `
            -ScoopName "calibre" `
            -ManualUrl "https://calibre-ebook.com/download" `
            -Category "optional"
    }
...
# CLI Version Gate
# ============================================================================
# Mirror of install.sh's gate. Defense-in-depth for OLD takumi CLIs that predate
# the CLI-side gate and so never check the kit's min_cli_version. The CLI passes
# its version via $env:TAKUMI_CLI_VERSION; a missing version means the CLI is too
# old to report it and is treated as unsupported.

# Return the numeric MAJOR.MINOR.PATCH core as [version], or $null if unparseable.
# Pre-release / build suffixes (e.g. "-dev.42") are ignored (matches semver.coerce).
function Get-SemverCore {
    param([string]$Version)
    # No leading-`v` handling: match install.sh, which treats a leading non-digit
    # as unparseable (→ lenient allow). Version strings the CLI passes never carry one.
...
# Lenient: no floor / unparseable floor / unparseable CLI version all proceed.
# Only a present-but-too-low or a missing CLI version (with a floor declared)
# blocks. TKM_SKIP_CLI_VERSION_CHECK=1 overrides (manual invocation escape hatch).
function Test-CliVersion {
    $metadata = Join-Path (Split-Path -Parent $ScriptDir) "metadata.json"
    if (-not (Test-Path $metadata)) { return }

    $min = $null
    try {
        $min = (Get-Content -Raw -LiteralPath $metadata | ConvertFrom-Json).min_cli_version
    } catch { return }   # unreadable/malformed metadata -> allow
    if ([string]::IsNullOrWhiteSpace($min)) { return }   # no floor -> allow

    $minCore = Get-SemverCore $min
    if ($null -eq $minCore) { return }   # unparseable floor -> allow

    if ($env:TKM_SKIP_CLI_VERSION_CHECK -in @('1', 'true', 'yes', 'TRUE', 'YES')) {
        Write-Warning "CLI version check bypassed (TKM_SKIP_CLI_VERSION_CHECK); this kit expects takumi CLI >= $min"
        return
    }

    $cli = $env:TAKUMI_CLI_VERSION
    if ([string]::IsNullOrWhiteSpace($cli)) {
        Write-Error "This kit requires takumi CLI >= $min, but the CLI did not report its version."
        Write-Info "Your takumi CLI is too old to install this kit safely. Upgrade with:"
        Write-Info "  npm install -g @sunasteriskrnd/takumi"
        Write-Info "Then re-run the installer, or set TKM_SKIP_CLI_VERSION_CHECK=1 to override."
        exit 1
    }

    $cliCore = Get-SemverCore $cli
    if ($null -eq $cliCore) { return }   # unparseable version -> allow

    if ($cliCore -lt $minCore) {
        Write-Error "This kit requires takumi CLI >= $min, but the installed CLI is $cli."
        Write-Info "Upgrade with:"
        Write-Info "  npm install -g @sunasteriskrnd/takumi"
        Write-Info "Then re-run the installer, or set TKM_SKIP_CLI_VERSION_CHECK=1 to override."
        exit 1
    }
