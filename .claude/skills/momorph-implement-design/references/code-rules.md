...
- **`get_figma_image` fallback (the ONLY allowed call site)**: if a node has the name `mm_media_*` BUT has no row in `assets.md` (because `get_media_files` lacked a URL — file not yet uploaded to Figma cloud), call `mcp__momorph__get_figma_image(fileKey, nodeIds=[nodeId], format='svg' for icons | 'png' for raster)` to pull it directly.
...
3. **Take the actual color from Figma `fills`** of the icon node (or the closest meaningful ancestor) via `mcp__momorph__get_node(screenId, nodeId)` → apply `color: <hex>` on the parent element (or inline style/className).
...
When unsure → query `mcp__momorph__get_node_context(screenId, nodeId, includeSiblingStyles=true)` to view parent + asset together and reconcile their `cornerRadius`.

## 3. Style mapping (auto-layout + alignment)

Get correct styles from `mcp__momorph__get_node(screenId, nodeId)` — DO NOT guess.
