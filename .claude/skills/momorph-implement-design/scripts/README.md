...
Two local helper scripts (asset download + coverage check). Every node/style query goes through live MCP tools directly (`get_overview`, `get_frame_node_tree`, `get_node`, `get_node_context`, ...).
...
# 3. Subagents code in parallel (use embedded branches + MCP get_node directly,
