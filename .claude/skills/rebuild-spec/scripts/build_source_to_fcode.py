...
from _stack_profile_lib import SCHEMA_VERSION as STATE_SCHEMA_VERSION  # noqa: E402

# RT-F4: stamped into .rebuild-state.json on every write so resume can detect a stale state
# and re-run preflight detect. A state written before screen_source existed would resume
# carrying a profile that fail-closes screens to "none"; the bump invalidates it so the
# profile re-resolves. SCHEMA_VERSION is the single source of truth in _stack_profile_lib.py --
# imported, not shadowed, so every consumer of either module sees the same value.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--cursor",
        default="core",
        choices=["core", "feature-specs", "flows", "glossary", "api-contracts", "test-cases"],
        help=(
    ...
    state_data: dict = {
        ...
        "schema_version": STATE_SCHEMA_VERSION,  # resume-invalidation gate; see _stack_profile_lib.SCHEMA_VERSION
