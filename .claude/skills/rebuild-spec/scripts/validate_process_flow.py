...
# v26.2.0 — Trigger Sequence companion: EXPECTED sequenceDiagram fence after ## State Diagram.
# Simple regex over full text (same idiom as the other section scans in this file, which also
# scan raw lines "including mermaid code blocks" — no dedicated fence parser needed).
SEQUENCE_DIAGRAM_RE = re.compile(r"```mermaid\s*\r?\n\s*sequenceDiagram", re.IGNORECASE)
...
def _validate_flow_file(flow_file: Path, root: Path, entity_sms: list[dict] | None = None) -> list[dict]:
    ...
    # v26.2.0 — Trigger Sequence companion: WARN-only, absence never blocks a pass (Decision 1).
    # No corpus today has one; old flows legitimately lack it (no migration script, see contract).
    if not SEQUENCE_DIAGRAM_RE.search(text):
        issues.append(_issue("warning", "ProcessFlow.sequence_diagram.pre_migration",
                             flow_file, root, None,
                             "no `### Trigger Sequence` sequenceDiagram fence found after "
                             "## State Diagram — WARN-first companion (v26.2.0), never blocks "
                             "a pass; re-run --flows to add"))
