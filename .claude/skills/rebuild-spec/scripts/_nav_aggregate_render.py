...
def compute_present_nums(docs_root: str, reading_order: list[dict]) -> set[int]:
    """Return the set of READING_ORDER `num`s whose artifact exists on disk.

    Shared by _nav_index.build_index_readme (docs/README.md table + role-paths)
    and _nav_sidecar_lib.build_reading_sidecar (the JSON sidecar), so the two
    outputs prune against the same set by construction, not by convention.
    """
    return {
