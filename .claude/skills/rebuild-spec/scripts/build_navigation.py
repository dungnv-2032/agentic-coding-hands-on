...
from _nav_docmap_lib import insert_document_map  # noqa: E402
...
from _nav_sidecar_lib import write_reading_sidecar  # noqa: E402
...
def _write_index_readme(docs_root: str, lang: str | None, timestamp: str) -> bool:
    ...
    Returns True iff the full non-aggregate READING_ORDER-driven index was
    written at this root — i.e. neither the write-safety guard, nor the bare
    per-lang-root skip, nor the aggregate thin-pointer path fired. The caller
    (run()) gates the reading-order sidecar write on this return value instead
    of re-detecting bare-root/aggregate itself (phase-01 Q2/Q3 fix, plan
    260826-1601-package-reading-layers-index-pager): the sidecar describes
    READING_ORDER, so it must never be written where the index it claims to
    mirror does not exist (or is a different structure, e.g. the aggregate
    thin pointer built from AGGREGATE_SYSTEM_ORDER).
    """
    ...
    # Reuse the SAME aggregate-root decision build_index_readme makes internally
    # (_is_aggregate_root) — one call, no re-implementation — so the sidecar
    # gate can never drift from what actually got rendered below.
    is_aggregate = _is_aggregate_root(docs_root)
    ...
    # FR-1 (phase-01, 27.11.0): '## Document Map' — a 4-bucket, waterfall-ordered crosswalk
    # spliced into the SAME generated zone, right before GEN_END. Presence-driven; a no-op
    # when nothing is present yet (never a dangling empty heading).
    content = insert_document_map(content, docs_root, lang)
    try:
...
def run(docs_root_arg: str | None, pass_complete: bool, lang: str | None = None,
    ...
    wrote_full_index = _write_index_readme(docs_root, lang, timestamp)

    # Reading-order sidecar (phase-01, 28.2.0): docs/.reading-order.json —
    # machine-readable READING_ORDER/ROLES/QUICK_PATH so --package can consume
    # the single source of truth instead of re-deriving its own grouping.
    # Gated on wrote_full_index (Q2/Q3 fix): only written where the full
    # non-aggregate READING_ORDER-driven index README was actually written —
    # an aggregate thin-pointer root or a bare per-lang root gets no sidecar,
    # so it can never describe an index that does not exist at this root.
    if wrote_full_index:
        write_reading_sidecar(docs_root, lang)
