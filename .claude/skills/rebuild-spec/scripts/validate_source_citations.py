...
from _slug_lib import (  # noqa: E402
    SATELLITE_FEATURE_FILES, assert_under, iter_docs_technical_specs, iter_technical_specs,
...
from _citation_hop_parse_lib import iter_citation_tokens  # noqa: E402
...
# ONE `**Source:**` prefix -- reference 2..N have no prefix of their own, so CITATION_RE
# (anchored on that literal prefix, one path:N/N-M pair) can only ever match reference 1
# via `.search()`. `iter_citation_tokens` walks every reference after the prefix
# regardless of separator, so reference 2..N get the same existence/range/traversal
# treatment reference 1 always got. CITATION_RE itself is kept byte-identical: an
# external contract (`_action_thread_diagram_lib.py` derives `FILE_LINE_RE` from its
# exact pattern text) and existing tests assert its single-match behaviour directly.
...
def _check_citations(spec: Path, root: Path, mode: str = "source",
    ...
    for i, ln in enumerate(lines):
        ...
        if in_fence:
            continue
        tokens = list(iter_citation_tokens(ln))
        if not tokens:
        ...
        # Applies to the whole line -- a multi-reference citation carries one
        # [FROM_CODE] tag for the whole line, not per-reference.
        force_source = "[FROM_CODE]" in ln

        # Every reference on the line (arrow chain / comma line-list / comma-joined
        # citations -- see _citation_hop_parse_lib.py) gets the SAME treatment
        # reference 1 always got: traversal guard, spec-driven branch, existence, and
        # range checks. References 2..N carry no `**Source:**` prefix of their own,
        # which is exactly why a single `.search()` on CITATION_RE used to see only
        # the first one.
        for raw_path, ranges, _m in tokens:
            # path-traversal guard before resolving -- once per path token, not per
            # comma-list range (a path only needs to be judged a traversal once).
            ...
            try:
                ...
                continue
            # A single path token may carry a comma-list of ranges (F2, e.g.
            # `a.rb:115-116,186,188`) -- range-check EVERY one of them, not just the
            # first, since the file is only resolved once but each range is an
            # independent claim about that file's contents.
            for start, end_opt in ranges:
                end = end_opt if end_opt is not None else start
...
def main(argv: list[str]) -> int:
    ...
    if args.docs_root:
        ...
        if single.name in SATELLITE_FEATURE_FILES:
