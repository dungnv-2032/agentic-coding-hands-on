...
STRINGS = {
    ...
    "feature_traversal": [
        "How to read a feature: open its folder and read functional-spec.md first — "
        "what it does, how it behaves, what the screen shows — then technical-spec.md "
        "for how it's implemented (endpoints, pseudocode, Source citations).",
        "To go deeper on a screen listed in functional-spec.md § 5, follow the SCR### "
        "code in its row to docs/screens/SCR###/spec.md (the spec links back via its "
        "**Feature** header).",
    ...
    # line only when its gating entry (the features glob, entry 15) survives pruning,
    ...
    # are the satellite filename(s) (skeleton identity asserted across locales).
    "feature_readme": {
        ...
        "col_route_spec": "Spec",
        "file_purposes": {
            "functional-spec.md": "what it does, how it behaves, what the screen shows",
            "technical-spec.md": "how it's implemented",
    ...
    "artifact_descriptions": {
        ...
        "features": "Per-feature deep specs — functional-spec + technical-spec per feature (see note below)",
        "screens": "Per-screen detailed UI and behavior specifications",
        "traceability_matrix": "One row per F### threading its SCR###/US###/BL###/ROUTE###/PERM### (+TC###) — the end-to-end feature thread",
    },
    # W5 crosswalk (phase-01, 27.11.0) — a second, BA/QA-oriented grouping of the SAME
    # artifacts above, organized by "what kind of question do you have" (waterfall:
    # Requirements -> External Design -> Internal Design -> Test Spec) instead of by
    # reading order. Rendered as '## Document Map' in the SAME generated zone.
    "document_map": {
        "heading": "Document Map",
        "intro": (
            "Every generated artifact, grouped the way a standard document set is grouped — "
            "orient by what kind of question you have, not by filename."
        ),
        "buckets": {
            "requirements": {"label": "Requirements", "who": "Read by: BA / QA."},
            "external_design": {
                "label": "External Design",
                "who": "Read by: BA / QA / Dev — how the system looks and connects from outside.",
            },
            "internal_design": {
                "label": "Internal Design",
                "who": "Read by: Dev / SA — how it is built on the inside.",
            },
            "test_spec": {"label": "Test Spec", "who": "Read by: QA."},
        },
        "entries": {
            "functional_spec": "What must each feature do, and why? (US### user stories, per feature)",
            "screen_list": "What screens does the product have?",
            "screen_flow": "How do screens connect to each other?",
            "entities": "What data entities exist and how do they relate? (ER)",
            "api_map": "What API surface does the backend expose?",
            "feature_list": "What features make up this product?",
            "architecture": "How is the system structured, layer by layer?",
            "technical_spec": "How is each feature implemented? (per feature)",
            "behavior_logic": "What background/business logic runs, and when?",
            "crud_matrix": "Which feature reads or writes which table?",
            "api_contracts": "What are the exact request/response shapes?",
            "test_cases": "How is each feature verified? (TC### test cases, per feature)",
        },
        "trace_pointer": (
            "**Tracing one feature end-to-end?** See [{link}]({link}) — one row per F### "
            "linking its screens, stories, rules, routes, permissions, and tests."
        ),
    },
    # UI chrome for the `--package` client bundle (plan
    # `260826-1601-package-reading-layers-index-pager`, phase-03b). Emitted
    # verbatim as the sidecar's top-level `ui` object — NOT artifact prose,
    # so it lives in its own block rather than folded into an existing one.
    # Consumed by the index/sidebar (phase-04) and the prev/next pager
    # (phase-05); reading-spine.cjs itself uses only `appendix`. Keyed set is
    # a skeleton parity requirement across en/vi/ja (test-asserted).
    "package_ui": {
        "start_here": "Start Here",
        "appendix": "Appendix — Repo Docs",
        # Layer-4 drill sub-labels — keyed by the READING_ORDER entry "key"
        # so a consumer looks them up directly (`ui.drill_labels[entry.key]`),
        # no parallel mapping to keep in sync.
        "drill_labels": {
            "flows": "Flows",
            "features": "Features",
            "screens": "Screens",
        ...
        "pager": {
            ...
            "end_of_spine": "End of the reading order",
        },
