...
def build_index_readme(docs_root: str, lang: str | None, timestamp: str,
    ...
    # prune against the same set of files-on-disk. Shared with the reading-order
    # sidecar (_nav_sidecar_lib.build_reading_sidecar) via compute_present_nums,
    # so docs/README.md and docs/.reading-order.json can never drift apart.
    present_nums = compute_present_nums(docs_root, READING_ORDER)
    lines = [
        f"# {s['title']}", "",
        GEN_START,
        f"<!-- rebuild-spec navigation — generated {timestamp} -->",
    ...
    # new_dev → feature-traversal pointer, gated on the features entry (15) surviving.
    lines += role_path_lines(
        present_nums, ROLES, s["role_labels"], s["roles_heading"],
        role_notes=s.get("role_notes", {}), note_gate=15,
    )
    omitted = False
