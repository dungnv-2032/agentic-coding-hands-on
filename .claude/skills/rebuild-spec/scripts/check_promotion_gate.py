#!/usr/bin/env python3
"""Wave 9 — promotion gate: verify all mandatory feature files (technical-spec.md + functional-spec.md, v27.0.0) exist before doc promotion.
...
def _resolve_summary_path(plan_dir: Path) -> Path | None:
    """Locate validation-summary.json, canonical location first.

    The pipeline writes `artifacts/validation/validation-summary.json` (every
    `--summary-out` in references/pipeline-*.md).  This gate previously looked only
    at `artifacts/validation-summary.json` — one directory too high — so it never
    found the file, took the "absent is acceptable" branch, and could not see an
    overall_status=FAIL.  The flat path is kept as a fallback for older plan dirs.
    """
    for candidate in (
        plan_dir / "artifacts" / "validation" / "validation-summary.json",
        plan_dir / "artifacts" / "validation-summary.json",
    ):
...
def _check_validation_summary(plan_dir: Path) -> str | None:
    ...
    summary_path = _resolve_summary_path(plan_dir)
