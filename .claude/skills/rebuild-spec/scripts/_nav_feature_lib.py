...
# Feature reading order — the 2 mandatory files (BA/QA-first), plus the optional
...
# 3rd entry simply never survives the os.path.isfile() filter below).
FEATURE_FILE_ORDER = [
    _FUNC_SPEC_FILE,
    _TECH_SPEC_FILE,
    "test-cases.md",
...
def build_feature_readme(feature_dir: str, docs_root: str, lang: str | None,
    ...
    Sections: a presence-pruned reading order (functional-spec.md, technical-spec.md,
    + optional test-cases.md sidecar), then a best-effort Screen → SCR### → spec table
    (resolved against generated/screen-list.md by name, or read directly when
    functional-spec.md § 5 already carries an SCR### column).
    """
    ...
    # Best-effort screen table from functional-spec.md § 5, resolved via screen-list.md.
    screens = []
    func_spec_path = os.path.join(feature_dir, _FUNC_SPEC_FILE)
    if os.path.isfile(func_spec_path):
        try:
            screens = parse_screen_names(_read(func_spec_path))
        except OSError:
            screens = []
    if screens:
        name_to_code = {}
