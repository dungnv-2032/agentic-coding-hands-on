...
# `specs` is a per-fcode container, not a flat validator slot; it is counted
# separately below. Every other slot is counted by presence.
AGGREGATE_SLOTS = frozenset({"specs"})


def recalculate_totals(summary: dict) -> None:
    """Sum across validators + count pass/fail specs.

    Flat slots are counted by PRESENCE, never from an allowlist: a validator that
    reports criticals must be able to fail the run the day it is wired up.  The
    previous allowlist ("route_list", "screen_list", "id_contiguity") silently
    dropped every other validator's findings, so a run with criticals from e.g.
    behavior_logic or screen_flow still derived overall_status PASS.
    """
    c = w = p = f = 0
    ...
    for name, slot in summary["validators"].items():
        if name in AGGREGATE_SLOTS or not isinstance(slot, dict):
            continue
        slot_summary = slot.get("summary")
        if not isinstance(slot_summary, dict):
            continue
        c += slot_summary.get("critical") or 0
        w += slot_summary.get("warning") or 0
