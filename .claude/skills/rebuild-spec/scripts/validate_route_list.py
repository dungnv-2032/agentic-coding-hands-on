...
# Template-shipped blockquote contract note, e.g.
# `> **Completeness Contract:** ... FORBIDDEN: approximation markers (`~N`, ...)`.
# These describe the forbidden markers rather than using them.
CONTRACT_NOTE_RE = re.compile(r"^>\s*\*\*[^*]*Contract:?\*\*", re.IGNORECASE)
...
def validate(plan_dir: Path, root: Path, single_file: Path | None = None) -> dict:
    ...
    for lineno, line in enumerate(lines, start=1):
        ...
        elif CONTRACT_NOTE_RE.match(stripped):
            # route-list-template.md ships blockquote contract notes that ENUMERATE the
            # forbidden markers ("FORBIDDEN: ... approximation markers (`~N`,
            # `(+nhiều)`, `see routes.rb`, `etc.`)").  Scanning them made the template's
            # own rule text a critical violation, so every artifact that kept the note
            # failed — the check contradicted the template it polices.  Only blockquote
            # `> **... Contract:**` notes are exempt; route data lives in table rows,
            # which are still scanned in full above.
            continue
        else:
