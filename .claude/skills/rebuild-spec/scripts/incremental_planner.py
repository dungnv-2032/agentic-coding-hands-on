...
def _hydrate(plan_dir: Path, docs_root: Path) -> int:
    ...
    # architecture.md: copy if present, else skip (NOT like system-overview, which forces
    # a full-rebuild fallback when absent). architecture is a newer artifact; a legacy repo
    # lacking it must not be forced into a full rebuild here. No incremental wave regenerates
    # it; it is always hydrated from the layered copy.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if affected_waves:
        _holistic_docs = ["system-overview.md"]
