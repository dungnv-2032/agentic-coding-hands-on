...
Binds the two ID systems both ways and checks each side resolves:
  forward  — every functional-spec.md § 5 Screens row's SCR### ∈ screen-list.md inventory
             (v27.0.0: screens.md is retired; the SCR###/Feature bridge moved here verbatim)
...
# No trailing guard originally over-matched: "SCR0011" resolved as "SCR001" (a wrong
# link.scr_unresolved verdict), since nothing barred a 4th digit from following the
# fixed 3-digit width. (?!\d) ("not followed by another digit") closes that.
_SCR_PREFIX = re.compile(r"\bSCR\d{3}(?!\d)", re.IGNORECASE)
# _F_PREFIX carried the identical over-match shape ("F0011" resolving as "F001"). Closed
# here rather than deferred: same file, same inventory/prefix pair, same 3-digit scheme, and
# a one-token divergence between two adjacent matchers is how this hazard keeps recurring.
_F_PREFIX = re.compile(r"\bF\d{3}(?!\d)", re.IGNORECASE)
...
# v27.0.0 (audience split): the SCR###⇄F### bridge now lives in functional-spec.md § 5
# Screens (screens.md is retired — folded in, see templates/functional-spec-template.md).
# Matches both the new numbered heading ("## 5. Screens") and, tolerantly, the retired
# "## Screen List" text (pre-migration docs written before this split).
_SCREENS_SECTION_HEAD_RE = r"#+\s*(?:\d+\.\s*)?Screens?(?:\s+List)?\b"


def check_screens_md(text: str, scr_inventory: set[str], file_path: str) -> list[dict]:
    """Forward check: each SCR### in functional-spec.md § 5's Screens table resolves
    to the inventory."""
    if _BACKGROUND.search(text) and not _first_table_after(text, _SCREENS_SECTION_HEAD_RE):
        return []  # background-only feature, no UI table — N/A
    table = _first_table_after(text, _SCREENS_SECTION_HEAD_RE)
    if len(table) < 2:
    ...
    if scr_idx is None:
        return [_issue("warning", "link.pre_migration", file_path,
                       "functional-spec.md § 5 Screens has no SCR### column "
                       "(pre-migration); run migrate-feature-screen-ids.py")]
    issues: list[dict] = []
    for raw in table[2:]:
...
def validate(root: Path) -> dict:
    ...
    if sl is None:
        ...
        for func_spec in sorted((root / "features").glob("*/functional-spec.md")):
            issues += check_screens_md(_read(func_spec), scr_inv, str(func_spec))
    if fl is None:
        issues.append(_issue("warning", "link.inventory_absent", str(root / "generated"),
