...
def _semver_tuple(s: str) -> tuple[int, ...] | None:
    """Parse a dotted-int semver string into a tuple for numeric comparison.

    Returns None on anything unparseable (non-string, non-numeric segments) rather than
    raising -- a corrupt version stamp must read as stale, never crash detection.
    """
...
def _state_staleness(root: str) -> dict:
    """Compare `<root>/docs/.rebuild-state.json`'s `schema_version` against the current
    `_stack_profile_lib.SCHEMA_VERSION` (Phase 03, state-schema lockstep).

    Reads the project ROOT's own state file -- deliberately not
    `_reused_root_lib.is_reused_root`, which excludes the scan root by design (it exists to
    find OTHER reused sub-roots below root, not to read root's own checkpoint).

    Semantics (do not conflate these two absent cases):
      - state file absent            -> state_present=False, found=None, stale=False
                                         (a first run has no checkpoint to invalidate)
      - file present, key missing/
        unparseable/corrupt/below     -> state_present=True,  stale=True
        `required`

    Read-only; the try/except guarantees this never raises -- an unreadable or invalid state
    file degrades to "stale", exactly like a missing/legacy schema_version would.
    """
    state_file = os.path.join(root, "docs", ".rebuild-state.json")
    if not os.path.isfile(state_file):
        return {"state_present": False, "found": None, "required": SCHEMA_VERSION, "stale": False}

    found = None
    stale = True  # default: present-but-unreadable/missing-key/corrupt all read as stale
    try:
        ...
        if isinstance(data, dict):
            ...
            if isinstance(raw, str):
                found = raw
                found_tuple = _semver_tuple(raw)
                required_tuple = _semver_tuple(SCHEMA_VERSION)
                if found_tuple is None:
                    stale = True
                elif required_tuple is not None:
                    # Zero-pad to equal length before comparing: bare tuple comparison makes
                    # the shorter equal-prefix tuple smaller, so "22.1" would read as older
                    # than "22.1.0" despite naming the same schema.
                    width = max(len(found_tuple), len(required_tuple))
                    found_p = found_tuple + (0,) * (width - len(found_tuple))
                    required_p = required_tuple + (0,) * (width - len(required_tuple))
                    stale = found_p < required_p
    except (OSError, ValueError, json.JSONDecodeError):
        found = None
        stale = True

    return {"state_present": True, "found": found, "required": SCHEMA_VERSION, "stale": stale}
...
def detect(
    ...
    return {
        ...
        "ui_sniff": ui_sniff,
        "warnings": warnings,
        "state_staleness": _state_staleness(abs_root),
    }
