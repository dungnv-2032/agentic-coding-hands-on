...
THE RULE THE GATE ENFORCES: no rendered artifact may echo a literal secret value in its
prose, table cells, or config/schedule fields. A doc that quotes a real password, token,
or credentialed URL republishes that secret to every reader of the docs tree and to every
downstream consumer of the docs package, so ONE hit is CRITICAL and blocks promotion
outright — it is never downgraded to a warning and there is no override. Live gate
callers: `validate_deployment_view.py` and `extract_service_topology.py`.
`.service`/`.timer` systemd unit files are recognised by
...
# walk picks up .service/.timer pairs. Generic scanning surface, not tied to any one pass:
# reached via `collect_scrubbed_config` from `validate_deployment_view.py` and
# `extract_service_topology.py`.
...
def assert_no_secrets(digest_json: str) -> list[str]:
    ...
    PRECISION-focused hard CRITICAL gate. Because a hit is CRITICAL and unappealable,
    a false positive here permanently blocks promotion of clean docs, so this iterates
