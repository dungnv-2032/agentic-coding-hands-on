...
# code-formats.md mandates `BL###_NameSlug`.  BL_H2_RE above is deliberately
# permissive so a malformed heading is still PARSED (and still gets its
# source/type checks) rather than vanishing from the document silently; this
# stricter pattern is what decides whether the heading HONORS the contract.
# Trailing `(?!\d)` not `\b`: `_` is a word char, so `\b` never matches
# `BL054_Slug` and would exempt exactly the compliant form.
BL_H2_CANONICAL_RE = re.compile(r"^## BL\d{3}(?!\d)_[A-Za-z0-9]")
...
# Scout BL inventory format contract: `- <category>: <path>` (one BL per entry).
INVENTORY_ENTRY_RE = re.compile(r"^\s*-\s+[A-Za-z][A-Za-z-]*:\s*\S")
# Empty-category sentinel emitted by scout; skipped per pipeline Rule C1 step 2a.
INVENTORY_SENTINEL_RE = re.compile(r"\(none found\)")
# Markdown/HTML comment prose that carries no entry (opening line and continuations).
INVENTORY_COMMENT_RE = re.compile(r"^\s*(<!--|-->|§)|-->\s*$")


def _count_inventory_entries(inventory_text: str) -> int:
    """Count BL entries in a scout inventory, tolerating either supported shape.

    Two shapes are in use: the scout contract `- <category>: <path>`, and a plain
    one-entry-per-line list.  Prefer the contract shape whenever the file uses it,
    because a contract-shaped file also carries prose the plain count would swallow
    (HTML-comment header lines counted as entries, inflating the total) plus
    `(none found)` empty-category sentinels that pipeline Rule C1 step 2a requires
    to be SKIPPED.  Either way, sentinels and comment prose never count.
    """
    lines = inventory_text.splitlines()
    contract = [
        ln for ln in lines
        if INVENTORY_ENTRY_RE.match(ln) and not INVENTORY_SENTINEL_RE.search(ln)
    ]
    if any(INVENTORY_ENTRY_RE.match(ln) for ln in lines):
        return len(contract)
    return len([
        ...
        and not ln.startswith("//")
        and not INVENTORY_COMMENT_RE.search(ln)
        and not INVENTORY_SENTINEL_RE.search(ln)
    ])
...
def validate(
    ...
    for bl_sec in bl_sections:
        ...
        # Check: code_format — heading must be `## BL###_NameSlug` per
        # references/code-formats.md.  Warning, not critical: the deviation does not
        # lose content, but any downstream consumer keying on `BL###_` silently
        # skips the non-conforming sections, so it must not stay invisible.
        if not BL_H2_CANONICAL_RE.match(bl_sec["heading"]):
            issues.append(_issue("warning", "BehaviorLogic.code_format", rel_path, line_start,
                                 f"BL heading {bl_sec['heading']!r} does not match the mandated "
                                 f"`BL###_NameSlug` format (references/code-formats.md); "
                                 f"consumers matching `BL###_` will skip this section"))
    ...
    if scout_bl_inventory is not None and scout_bl_inventory.is_file():
        try:
            ...
            inventory_count = _count_inventory_entries(inventory_text)
