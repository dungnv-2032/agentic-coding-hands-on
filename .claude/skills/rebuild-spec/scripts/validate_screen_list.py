...
# A REG definition row: the code is the ENTIRE leading table cell.  Used for
# duplicate detection so cross-references inside descriptive cells do not count.
REG_DEF_ROW_RE = re.compile(r"^\|\s*(REG\d{3}(?:_\w+)?)\s*\|", re.IGNORECASE)
    ...
    for scr_sec in scr_body_sections:
        ...
        for abs_line, line in scr_sec["body_lines"]:
            ...
            if not stripped.startswith("|"):
                continue
            # A REG code's DEFINITION site is the leading cell of the Regions table
            # (``| REG001 | Name | ... |``).  A code named inside a longer cell —
            # ``| #categoriesDeleteModal | modal (region) | Simple-delete (REG001) |`` —
            # is a cross-REFERENCE to that region, and a screen legitimately both
            # defines a region and points at it from its Components table.  Matching
            # every REG occurrence in the row counted those references as redefinitions.
            reg_def = REG_DEF_ROW_RE.match(stripped)
            if reg_def:
                reg_code = reg_def.group(1).upper()
