...
from _spec_block_lib import BLOCK_HEADING_RE, DEC_BLOCK_RE  # noqa: E402

# v27.0.0 (audience split, translation-contract.md Rule 1a): BR/SM/ALG/INT (`###`) and DEC
# (`####`) block headings put prose and skeleton on ONE line --
# "### {sentence} (BR-001)" / "#### {sentence} (DEC-001)". Only the trailing "(<CODE>-###)"
# tag is skeleton; the sentence prefix is prose and is expected to differ per translated
# mirror. BLOCK_HEADING_RE / DEC_BLOCK_RE (canonical shape, _spec_block_lib.py) confirm the
# line is one of these before this regex extracts just the tag.
TRAILING_CODE_TAG_RE = re.compile(r"\(((?:BR|SM|ALG|INT|DEC)-\d{3,4})\)\s*$")
...
def extract_skeleton(lines: list[str]) -> list[tuple[int, str]]:
    ...
    for i, line in enumerate(lines):
        ...
        if stripped.startswith("#"):
            # Rule 1a: a v27.0.0 BR/SM/ALG/INT/DEC block heading carries prose (the sentence)
            # and skeleton (the trailing code tag) on one line. Reduce the skeleton signature
            # to marker + tag so a legitimately translated sentence isn't flagged as drift --
            # a changed tag still differs, and a dropped heading still shrinks/misaligns the
            # skeleton sequence (caught by the length/extra-element checks in `validate()`).
            if BLOCK_HEADING_RE.match(stripped) or DEC_BLOCK_RE.match(stripped):
                tag_match = TRAILING_CODE_TAG_RE.search(stripped)
                if tag_match:
                    marker = stripped.split(" ", 1)[0]
                    skeleton.append((i, f"{marker} ({tag_match.group(1)})"))
                    continue
            skeleton.append((i, stripped))
            continue
