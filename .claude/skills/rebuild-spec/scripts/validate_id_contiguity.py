...
def _check_global_contiguity(
    ...
    # route-list / crud-matrix / the DISC-### discriminators in data-model), its
    # definition site is instead the table cell holding the code ALONE
    # (``| payment_process | DISC-020 | ... |``); a code embedded in longer cell or
    # prose text (``See DISC-020.``, ``(DISC-020)``) is a cross-reference. Without
    # that tier a legitimately cross-referenced DISC code was reported as appearing
    # "5 times". Only when a scheme is neither heading- nor cell-defined do we fall
    # back to all-prose counting, so genuine duplicate rows are still caught.
    pat = re.compile(
    ...
    # A table cell whose ENTIRE content is the code — the definition site for
    # table-defined schemes.  Lookahead on the closing pipe so adjacent cells match.
    cell_pat = re.compile(
        rf"\|\s*{re.escape(prefix)}{re.escape(sep)}(\d{{3}})(?![0-9])\s*(?=\|)"
    )
    ...
    for kind, chunk in segment_text(text):
        if kind == "prose":
            ...
            for li, ln in enumerate(chunk.split("\n")):
                ...
                for cm in cell_pat.finditer(ln):
                    n = int(cm.group(1))
                    cell_counts[n] = cell_counts.get(n, 0) + 1
                    cell_lines.setdefault(n, []).append(prose_line_offset + li + 1)
            prose_line_offset += chunk.count("\n")
        else:
            prose_line_offset += chunk.count("\n")

    # Resolved PER CODE, most authoritative definition site first.  Deliberately not
    # per document: a code with no definition site anywhere must still fall through
    # to prose counting, so "referenced twice, defined never" stays reportable rather
    # than becoming silent — this validator's failure mode is false passes.
    for n, prose_count in prose_counts.items():
        if heading_counts.get(n, 0) >= 1:
            count = heading_counts[n]
            report_lines = heading_lines.get(n)
        elif cell_counts.get(n, 0) >= 1:
            count = cell_counts[n]
            report_lines = cell_lines.get(n)
        else:
            count = prose_count
            report_lines = prose_lines.get(n)
        if count > 1:
            ...
            iss = _issue(
                severity, "contiguity.duplicate", rel_path,
...
def main(argv: list[str]) -> int:
    ...
    if args.summary_out:
        ...
        try:
            ...
            # One slot is shared by every artifact this validator runs over, so a
            # plain assignment let a later artifact's PASS erase an earlier one's
            # FAIL.  Tag each issue with its artifact and merge: re-running the SAME
            # artifact replaces only its own issues (idempotent), while other
            # artifacts' findings survive.
            prior = summary["validators"].get(VALIDATOR, {})
            prior_issues = [
                i for i in prior.get("issues", [])
                if i.get("artifact") != args.artifact
            ]
            merged = prior_issues + [
                {**i, "artifact": args.artifact} for i in result["issues"]
            ]
            merged_crit = sum(1 for i in merged if i["severity"] == "critical")
            merged_warn = sum(1 for i in merged if i["severity"] == "warning")
            summary["validators"][VALIDATOR] = {
                "status": "FAIL" if merged_crit else ("WARN" if merged_warn else "PASS"),
                "summary": {"critical": merged_crit, "warning": merged_warn},
                "issues": merged,
            }
