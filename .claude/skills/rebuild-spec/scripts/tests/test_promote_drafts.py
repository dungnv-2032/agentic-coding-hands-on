...
class TestLayeredPromotion:
    ...
    def test_business_rules_no_longer_promoted(self, tmp_path):
        """rebuild-spec 27.0.0: business-rules.md is deleted — behavior-logic.md absorbs
        it (Phase 03). No longer in CORE_ARTIFACTS, so a full promote must not produce it,
        even if a stale draft is left over from a pre-27.0.0 researcher run."""
        plan_dir, docs_root = _copy_fixture_plan(tmp_path)
