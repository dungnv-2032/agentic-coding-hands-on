...
# ---------------------------------------------------------------------------
# Cross-artifact summary merge — a later PASS must not erase an earlier FAIL
# ---------------------------------------------------------------------------

class TestSummarySlotSharedAcrossArtifacts:
    """Regression: every artifact wrote the same `validators.id_contiguity` slot by
    plain assignment, so running the validator over 7 artifacts kept only the LAST
    one — a clean artifact silently erased an earlier artifact's critical.
    """

    def _summary(self, plan_dir: Path) -> Path:
        return plan_dir / "artifacts" / "validation" / "validation-summary.json"

    def _read(self, path: Path) -> dict:
        return json.loads(path.read_text(encoding="utf-8"))

    def test_later_clean_artifact_does_not_erase_earlier_fail(self, tmp_path):
        plan = tmp_path / "plan"
        _write_artifact(plan, "user-stories", "## US001_A\nx\n\n## US003_C\ny\n")   # US002 gap
        _write_artifact(plan, "behavior-logic", "## BL001_A\nx\n\n## BL002_B\ny\n")  # clean
        out = self._summary(plan)

        rc, _ = _run(plan, "user-stories", summary_out=out, project_root=tmp_path)
        assert rc == 1
        assert self._read(out)["overall_status"] == "FAIL"

        rc, _ = _run(plan, "behavior-logic", summary_out=out, project_root=tmp_path)
        assert rc == 0                     # this artifact is itself clean
        summary = self._read(out)
        assert summary["validators"]["id_contiguity"]["status"] == "FAIL"
        assert summary["totals"]["critical"] == 1
        assert summary["overall_status"] == "FAIL"

    def test_issues_are_tagged_with_their_artifact(self, tmp_path):
        plan = tmp_path / "plan"
        _write_artifact(plan, "user-stories", "## US001_A\nx\n\n## US003_C\ny\n")
        out = self._summary(plan)
        _run(plan, "user-stories", summary_out=out, project_root=tmp_path)
        issues = self._read(out)["validators"]["id_contiguity"]["issues"]
        assert issues and all(i["artifact"] == "user-stories" for i in issues)

    def test_criticals_from_two_artifacts_accumulate(self, tmp_path):
        plan = tmp_path / "plan"
        _write_artifact(plan, "user-stories", "## US001_A\nx\n\n## US003_C\ny\n")
        _write_artifact(plan, "behavior-logic", "## BL001_A\nx\n\n## BL003_C\ny\n")
        out = self._summary(plan)
        _run(plan, "user-stories", summary_out=out, project_root=tmp_path)
        _run(plan, "behavior-logic", summary_out=out, project_root=tmp_path)
        summary = self._read(out)
        assert summary["totals"]["critical"] == 2
        assert {i["artifact"] for i in summary["validators"]["id_contiguity"]["issues"]} == {
            "user-stories", "behavior-logic"}

    def test_rerunning_same_artifact_is_idempotent(self, tmp_path):
        plan = tmp_path / "plan"
        _write_artifact(plan, "user-stories", "## US001_A\nx\n\n## US003_C\ny\n")
        out = self._summary(plan)
        _run(plan, "user-stories", summary_out=out, project_root=tmp_path)
        _run(plan, "user-stories", summary_out=out, project_root=tmp_path)
        assert self._read(out)["totals"]["critical"] == 1

    def test_fixing_an_artifact_clears_only_its_own_issues(self, tmp_path):
        plan = tmp_path / "plan"
        _write_artifact(plan, "user-stories", "## US001_A\nx\n\n## US003_C\ny\n")
        _write_artifact(plan, "behavior-logic", "## BL001_A\nx\n\n## BL003_C\ny\n")
        out = self._summary(plan)
        _run(plan, "user-stories", summary_out=out, project_root=tmp_path)
        _run(plan, "behavior-logic", summary_out=out, project_root=tmp_path)
        # Repair user-stories, re-run it: its issue clears, behavior-logic's remains.
        _write_artifact(plan, "user-stories", "## US001_A\nx\n\n## US002_B\ny\n")
        _run(plan, "user-stories", summary_out=out, project_root=tmp_path)
        summary = self._read(out)
        assert summary["totals"]["critical"] == 1
...
# ---------------------------------------------------------------------------
# Reference-vs-definition for table-defined schemes (DISC-### in data-model)
# ---------------------------------------------------------------------------

class TestCellDefinedSchemeDuplicates:
    """Regression: DISC-### is never a heading, so duplicate detection fell back to
    counting EVERY prose occurrence.  A discriminator defined once in a table and
    cross-referenced four times was reported as "appears 5 times" — 21 such false
    criticals on a real corpus, all of them correct documents.
    """

    def _data_model(self, plan_dir: Path, body: str) -> None:
        _write_artifact(plan_dir, "data-model", body)

    def test_cross_referenced_disc_is_not_a_duplicate(self, tmp_path):
        plan = tmp_path / "plan"
        self._data_model(plan, (
            "## MODEL001_Transaction\n\n"
            "| Column | Type | Notes |\n"
            "|---|---|---|\n"
            "| payment_process | string | See DISC-001. |\n\n"
            "### Discriminators\n\n"
            "| Column | Code | Values | Meaning |\n"
            "|---|---|---|---|\n"
            "| payment_process | DISC-001 | `a`, `b` | selects flow |\n\n"
            "Inclusion uses `a`/`b` (DISC-001) by default.\n"
        ))
        _, data = _run(plan, "data-model", project_root=tmp_path)
        assert "contiguity.duplicate" not in _rule_ids(data)

    def test_genuinely_duplicated_disc_definition_is_still_caught(self, tmp_path):
        """The gate must still FIRE — two definition cells for one code is real."""
        plan = tmp_path / "plan"
        self._data_model(plan, (
            "## MODEL001_Transaction\n\n"
            "| Column | Code | Values |\n"
            "|---|---|---|\n"
            "| payment_process | DISC-001 | `a` |\n"
            "| other_column | DISC-001 | `b` |\n"
        ))
        _, data = _run(plan, "data-model", project_root=tmp_path)
        assert "contiguity.duplicate" in _rule_ids(data)

    def test_disc_duplicate_is_warning_not_critical(self, tmp_path):
        """The DISC check is invoked with severity="warning"; duplicate previously
        hardcoded "critical", overriding the caller's contract."""
        plan = tmp_path / "plan"
        self._data_model(plan, (
            "## MODEL001_Transaction\n\n"
            "| Column | Code | Values |\n"
        ...
        rc, data = _run(plan, "data-model", project_root=tmp_path)
        dup = [i for i in data["issues"] if i["rule_id"] == "contiguity.duplicate"]
        assert dup and all(i["severity"] == "warning" for i in dup)
        assert rc == 0, "a warning-severity duplicate must not halt the pipeline"

    def test_model_duplicate_stays_critical(self, tmp_path):
        """MODEL is owned by data-model and checked at critical — unchanged."""
        plan = tmp_path / "plan"
        self._data_model(plan, "## MODEL001_A\nx\n\n## MODEL001_A\ny\n")
        rc, data = _run(plan, "data-model", project_root=tmp_path)
        dup = [i for i in data["issues"] if i["rule_id"] == "contiguity.duplicate"]
        assert dup and all(i["severity"] == "critical" for i in dup)
        assert rc == 1

    def test_heading_defined_scheme_still_wins_over_cells(self, tmp_path):
        """A heading-defined scheme must keep using headings even when an index
        table lists every code as a sole cell."""
        plan = tmp_path / "plan"
        _write_artifact(plan, "user-stories", (
            "## User Story Index\n\n"
            "| Code | Name |\n|---|---|\n"
            "| US001 | A |\n| US002 | B |\n\n"
            "## US001_A\nx\n\n## US002_B\ny\n"
        ))
        _, data = _run(plan, "user-stories", project_root=tmp_path)
        assert "contiguity.duplicate" not in _rule_ids(data)
