...
def _make_full_feature_dir(tmp_path: Path, frontmatter: str, extra_tech_spec: str = "") -> Path:
    """Create docs/features/F001_Test/ with both required files (v27.0.0 audience split).
    ...
    (feat_dir / "functional-spec.md").write_text(_minimal_functional_spec(), encoding="utf-8")
    return tmp_path


def _minimal_functional_spec() -> str:
    """Return a minimal but structurally valid functional-spec.md body."""
    return """\
# F001_Test — Test Feature
...
def _minimal_tech_spec() -> str:
    ...
    v27.x — human-readable SOT, 5-bucket retaxonomy). `### 5.4 Source References` is
    left with no real citation line — the one deliberate gap this fixture exists to
    exercise (source_refs_empty / block_source_missing, softened to warning for a
    draft+takumi spec, critical otherwise)."""
    return """\
# F001_Test — Test Feature
...
class TestDraftStructuralRulesStayCritical:
    def test_draft_missing_sm_mermaid_still_critical(self, tmp_path):
        ...
        spec.write_text(
            ...
            + "# F001_Test — Test Feature\n\n"
            + "## Source Code References\n\nSome prose only.\n\n"
            + "### A state machine with no fence (SM-001)\n\nNo mermaid fence here.\n",
            encoding="utf-8",
        )
...
class TestF15RoundTrip:
    ...
    def test_implemented_docs_root_table_citation_passes(self, tmp_path):
        """C2 regression: an implemented spec whose ### 5.4 Source References uses the
...
#      functional-spec.md (v27.0.0: absorbs business-context/screens/edge-cases) may
#      be deferred. Its *.missing must downgrade critical -> warning for a takumi
#      draft so the "draft exit 0" contract holds. A non-takumi/implemented dir with
#      the same shape stays critical (exit 1).
# ===========================================================================

def _make_tech_spec_only_dir(tmp_path: Path, frontmatter: str) -> Path:
    """Create docs/features/F001_Test/ with ONLY technical-spec.md (no functional-spec.md)."""
    feat_dir = tmp_path / "docs" / "features" / "F001_Test"
    feat_dir.mkdir(parents=True)
...
class TestDraftSatellitesDeferred:
    _SATELLITE_MISSING = {"func.missing"}
