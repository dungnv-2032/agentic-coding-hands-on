...
# ---------------------------------------------------------------------------
# no_dup_reg — REG definition site vs cross-reference
# ---------------------------------------------------------------------------

REG_CROSS_REFERENCED = """\
# Screen List

## SCR029_SignupLoginSettings

### Components

| Component | Kind | Notes |
|-----------|------|-------|
| signup_info_content | rich-text block | Custom signup page text — REG001 |
| `#categoriesDeleteModal` | modal (region) | Simple-delete confirmation (REG001) |

### Regions

| Code | Name | Feature | Rationale |
|------|------|---------|-----------|
| REG001 | SignupInfoContent | — | distinct save endpoint |
"""


def test_cross_referenced_reg_is_not_a_duplicate(tmp_path):
    """Regression: no_dup_reg matched EVERY REG occurrence in any table row, so a
    region defined once and pointed at from the Components table was reported as a
    duplicate — 24 such false criticals on a real corpus.
    """
    plan = _setup_plan(tmp_path, REG_CROSS_REFERENCED)
    result = validate(plan, tmp_path)
    rule_ids = [i["rule_id"] for i in result["issues"]]
    assert "ScreenList.no_dup_reg" not in rule_ids


def test_two_reg_definition_rows_still_fail(tmp_path):
    """The gate must still FIRE: two leading-cell definitions is a real duplicate."""
