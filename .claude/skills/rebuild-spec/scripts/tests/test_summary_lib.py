...
# ---------------------------------------------------------------------------
# Flat slots are counted by PRESENCE, not by an allowlist
# ---------------------------------------------------------------------------

class TestAllValidatorSlotsCount:
    """Regression: recalculate_totals used a hardcoded allowlist
    ("route_list", "screen_list", "id_contiguity"), so every other validator's
    findings were dropped and a run with criticals still derived PASS.
    """

    def _summary(self, validators):
        return {"totals": {}, "validators": validators}

    def test_unlisted_validator_criticals_reach_totals(self):
        summary = self._summary({
            "behavior_logic": {"status": "FAIL", "summary": {"critical": 5, "warning": 2}, "issues": []},
        })
        recalculate_totals(summary)
        assert summary["totals"]["critical"] == 5
        assert summary["totals"]["warning"] == 2

    def test_unlisted_validator_criticals_fail_the_run(self):
        summary = self._summary({
            "screen_flow": {"status": "FAIL", "summary": {"critical": 9, "warning": 0}, "issues": []},
        })
        recalculate_totals(summary)
        assert derive_overall_status(summary) == "FAIL"

    def test_sums_across_many_unlisted_validators(self):
        summary = self._summary({
            "behavior_logic": {"status": "FAIL", "summary": {"critical": 5, "warning": 2}, "issues": []},
            "api_map": {"status": "FAIL", "summary": {"critical": 3, "warning": 0}, "issues": []},
            "screen_flow": {"status": "FAIL", "summary": {"critical": 9, "warning": 0}, "issues": []},
        })
        recalculate_totals(summary)
        assert summary["totals"]["critical"] == 17
        assert derive_overall_status(summary) == "FAIL"

    def test_specs_container_is_not_double_counted(self):
        """`specs` is a per-fcode container; its counts come from the per-fcode
        walk only, never also as a flat slot."""
        summary = self._summary({
            "specs": {
                "F001_Auth": {"status": "FAIL", "summary": {"critical": 2, "warning": 1}, "issues": []},
            },
    ...
    def test_slot_without_summary_is_ignored_not_crashing(self):
        summary = self._summary({
            "weird": {"status": "PASS"},
            "also_weird": "not-a-dict",
            "behavior_logic": {"status": "FAIL", "summary": {"critical": 1, "warning": 0}, "issues": []},
        })
        recalculate_totals(summary)
        assert summary["totals"]["critical"] == 1

    def test_none_valued_counts_treated_as_zero(self):
        summary = self._summary({
            "behavior_logic": {"status": "PASS", "summary": {"critical": None, "warning": None}, "issues": []},
        })
        recalculate_totals(summary)
        assert summary["totals"] == {"critical": 0, "warning": 0, "passed_specs": 0, "failed_specs": 0}
