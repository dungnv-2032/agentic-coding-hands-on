...
def _make_complete_plan(tmp_path: Path) -> Path:
    """Create a valid 2-file (v27.0.0 audience split) plan dir under tmp_path."""
...
# ---------------------------------------------------------------------------
# Complete 2-file plan: all files present, no .pending → gate PASS
# ---------------------------------------------------------------------------
...
class TestTestCasesSidecarNotGated:
    ...
    def test_gate_passes_with_test_cases_absent(self, tmp_path):
        """Same 2-file plan, no test-cases.md at all — still PASS (sidecar, not mandatory)."""
        plan_dir = _make_complete_plan(tmp_path)
...
# ---------------------------------------------------------------------------
# 1-of-2 files: functional-spec.md missing → gate FAIL (gate.files_incomplete)
# ---------------------------------------------------------------------------

class TestGateFilesIncomplete:
    def test_exit_code_one(self, tmp_path):
        plan_dir = _make_complete_plan(tmp_path)
        (plan_dir / "artifacts" / "features" / "F001_Auth" / "functional-spec.md").unlink()
        code, _ = _run(plan_dir, tmp_path)
        assert code == 1
    ...
    def test_message_mentions_missing_file(self, tmp_path):
        ...
        assert any("functional-spec.md" in i["message"] for i in incomplete)
...
class TestGateValidationSummaryFail:
    ...
    def test_fail_summary_at_canonical_path_produces_warning(self, tmp_path):
        """Regression: the gate read `artifacts/validation-summary.json`, but the
        pipeline writes `artifacts/validation/validation-summary.json`.  It therefore
        never found the file, took the "absent is acceptable" branch, and could not
        see overall_status=FAIL.  The old test used the flat path, so the dead gate
        looked healthy.
        """
        plan_dir = _make_complete_plan(tmp_path)
        canonical = plan_dir / "artifacts" / "validation" / "validation-summary.json"
        canonical.parent.mkdir(parents=True, exist_ok=True)
        canonical.write_text(
            json.dumps({"overall_status": "FAIL", "validators": {}}), encoding="utf-8"
        )
        _, data = _run(plan_dir, tmp_path)
        assert "gate.validation_summary" in _rule_ids(data)

    def test_canonical_path_wins_over_legacy_flat_path(self, tmp_path):
        plan_dir = _make_complete_plan(tmp_path)
        (plan_dir / "artifacts" / "validation-summary.json").write_text(
            json.dumps({"overall_status": "PASS", "validators": {}}), encoding="utf-8"
        )
        canonical = plan_dir / "artifacts" / "validation" / "validation-summary.json"
        canonical.parent.mkdir(parents=True, exist_ok=True)
        canonical.write_text(
    ...
    def test_passing_canonical_summary_does_not_warn(self, tmp_path):
        plan_dir = _make_complete_plan(tmp_path)
        canonical = plan_dir / "artifacts" / "validation" / "validation-summary.json"
        canonical.parent.mkdir(parents=True, exist_ok=True)
        canonical.write_text(
        ...
        _, data = _run(plan_dir, tmp_path)
        assert "gate.validation_summary" not in _rule_ids(data)
