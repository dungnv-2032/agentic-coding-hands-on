...
A4: docs/features/F###_Slug/README.md carries a presence-pruned reading order
(functional-spec.md, technical-spec.md, + optional test-cases.md sidecar) and a
best-effort Screen → SCR### → spec table (resolved by name against
generated/screen-list.md, or read from an SCR### column directly — Phase B).
v27.0.0: the Screens table moved from screens.md to functional-spec.md § 5.
...
def _make_feature_tree(base: Path, satellites=("functional-spec.md", "technical-spec.md"),
                       with_screen_list=True) -> Path:
    docs = base / "docs"
    feat = docs / "features" / "F001_Login"
    feat.mkdir(parents=True)
    for f in satellites:
        if f == "functional-spec.md":
            (feat / f).write_text(FUNCTIONAL_SPEC_MD)
        else:
...
class TestParsers:
    def test_parse_screen_names_basic(self):
        rows = parse_screen_names(FUNCTIONAL_SPEC_MD)
        names = [r["name"] for r in rows]
        assert names == ["Login Form", "Dashboard"]
        assert all(r["scr"] is None for r in rows)  # em-dash SCR cells → unresolved
    ...
    def test_parse_screen_names_background_feature(self):
        assert parse_screen_names("## 5. Screens\n\nN/A — background feature.\n") == []
    ...
    def test_index_screen_list_rejects_four_digit_overmatch(self):
        # Phase 03 (capability-map plan) — _SCR_RE's `\bSCR\d{3}` (no trailing guard)
        # over-matches: "SCR0011" reads as "SCR001" since nothing bars a 4th digit from
        # following, wrongly admitting the row's Code cell as valid.
        # Reaches: (?!\d) lookahead in _SCR_RE rejects the SCR0011 cell outright, so the
        # row is skipped and never indexed — pre-fix it was wrongly indexed.
        screen_list = (
            "# Screen List\n\n## Screen Index\n\n"
            "| Code | Name |\n|------|------|\n"
            "| SCR0011_Ghost | Ghost Screen |\n"
        )
        assert index_screen_list(screen_list) == {}
...
class TestFeatureReadmeContent:
    def test_lists_present_reading_order(self, tmp_path):
        ...
        for f in ("functional-spec.md", "technical-spec.md"):
            assert f in out
        assert "Reading order" in out

    def test_prunes_absent_satellite(self, tmp_path):
        docs = _make_feature_tree(tmp_path, satellites=("technical-spec.md",))
        out = build_feature_readme(str(docs / "features" / "F001_Login"), str(docs), "en", TS)
        assert "functional-spec.md" not in out
    ...
    def test_includes_test_cases_when_present(self, tmp_path):
        ...
        docs = _make_feature_tree(
            tmp_path,
            satellites=("functional-spec.md", "technical-spec.md", "test-cases.md"),
        )
        out = build_feature_readme(str(docs / "features" / "F001_Login"), str(docs), "en", TS)
        assert "test-cases.md" in out
        assert out.index("technical-spec.md") < out.index("test-cases.md")
    ...
    def test_reads_scr_column_directly(self, tmp_path):
        """Phase-B forward-compat: an SCR### column in functional-spec.md § 5 is read
        directly."""
        docs = tmp_path / "docs"
        feat = docs / "features" / "F001_Login"
        feat.mkdir(parents=True)
        (feat / "functional-spec.md").write_text(
            "## 5. Screens\n\n| Screen Name | SCR### | What User Sees |\n"
