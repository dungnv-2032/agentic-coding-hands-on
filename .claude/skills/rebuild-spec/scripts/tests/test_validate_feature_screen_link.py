...
# ---------------------------------------------------------------------------
# forward: functional-spec.md § 5 SCR### column (screens.md is retired, v27.0.0)
# ---------------------------------------------------------------------------
...
class TestInventory:
    ...
    def test_build_inventory_rejects_four_digit_overmatch(self):
        # Phase 03 (capability-map plan) — _SCR_PREFIX's `\bSCR\d{3}` (no trailing guard)
        # over-matches: "SCR0011" reads as "SCR001", so a link check against this
        # inventory would wrongly resolve an unrelated 4-digit code as SCR001.
        # Reaches: (?!\d) lookahead in _SCR_PREFIX rejects the SCR0011 substring outright.
        text = "| SCR0011_Ghost | ... |\n"
        assert v.build_inventory(text, v._SCR_PREFIX) == set()

    def test_build_inventory_rejects_four_digit_overmatch_fcode(self):
        # Phase 03 (capability-map plan) — _F_PREFIX carried the same missing trailing
        # guard as _SCR_PREFIX: "F0011" read as "F001", wrongly resolving an unrelated
        # 4-digit code against the feature inventory.
        # Reaches: (?!\d) lookahead in _F_PREFIX rejects the F0011 substring outright.
        text = "| F0011_Ghost | ... |\n"
        assert v.build_inventory(text, v._F_PREFIX) == set()

    def test_build_inventory_still_extracts_three_digit_fcodes(self):
        # Guard against the fix over-tightening: legitimate 3-digit F-codes, slugged or
        # bare, must still be collected.
        # Reaches: (?!\d) passes because the char after the digits is `_` or end-of-cell.
        text = "| F001_Login | ... |\n| F042 | ... |\n"
        assert v.build_inventory(text, v._F_PREFIX) == {"F001", "F042"}
...
def _make_docs(base: Path, scr_cell="SCR001_LoginForm", feature="F001_Login") -> Path:
    ...
    # v27.0.0: the SCR###⇄F### bridge lives in functional-spec.md § 5 now (screens.md
    # is retired).
