...
class TestBusinessRulesRemoved:
    """[success criteria] rebuild-spec 27.0.0 Phase 03: business-rules.md is deleted —
    behavior-logic.md absorbs it. ARTIFACT_LAYERED must no longer name it."""

    def test_artifact_layered_has_no_business_rules(self):
        assert "business-rules.md" not in ARTIFACT_LAYERED
...
class TestComputeDocShas:
    ...
    def test_system_layer_artifacts(self, tmp_path):
        ...
        (tmp_path / "system" / "glossary.md").write_text("terms")
        shas = _compute_doc_shas(tmp_path)
        assert "permissions.md" in shas
        assert "glossary.md" in shas
