...
# ---------------------------------------------------------------------------
# no_approximation_marker — template contract notes are not violations
# ---------------------------------------------------------------------------

CONTRACT_NOTE_ROUTE_LIST = """\
# Route List

## Backend Routes

> **Completeness Contract:** emit exactly ONE row per leaf route. FORBIDDEN: \
resource-summary tables, approximation markers (`~N`, `(+nhiều)`, `see routes.rb`, \
`etc.`), wildcard paths (`/x/*`).

| Code | Method | Path | Handler | Owner F### |
|------|--------|------|---------|------------|
| ROUTE001 | GET | /users | UsersController#index | F001 |
...
def test_template_contract_note_is_not_an_approximation_marker(tmp_path):
    """Regression: route-list-template.md ships a contract note that ENUMERATES the
    forbidden markers, so the check flagged its own rule text — every artifact that
    kept the template note failed with a critical.
    """
    plan = _setup_plan(tmp_path, CONTRACT_NOTE_ROUTE_LIST)
    result = validate(plan, tmp_path)
    rule_ids = [i["rule_id"] for i in result["issues"]]
    assert "RouteList.no_approximation_marker" not in rule_ids


def test_real_prose_approximation_still_fails(tmp_path):
    """The gate must still FIRE on a genuine compression note outside a contract line."""
    plan = _setup_plan(tmp_path, CONTRACT_NOTE_ROUTE_LIST.replace(
        "Total: 1 route.", "Remaining routes: see routes.rb for the rest."))
    result = validate(plan, tmp_path)
    rule_ids = [i["rule_id"] for i in result["issues"] if i["severity"] == "critical"]
    assert "RouteList.no_approximation_marker" in rule_ids


def test_approximation_in_a_table_cell_still_fails(tmp_path):
    """Route data lives in table rows; the contract-note exemption must not reach them."""
    plan = _setup_plan(tmp_path, CONTRACT_NOTE_ROUTE_LIST.replace(
        "| ROUTE001 | GET | /users | UsersController#index | F001 |",
        "| ROUTE001 | GET | /users | UsersController#index (+nhiều) | F001 |"))
    result = validate(plan, tmp_path)
    rule_ids = [i["rule_id"] for i in result["issues"] if i["severity"] == "critical"]
    assert "RouteList.no_approximation_marker" in rule_ids
