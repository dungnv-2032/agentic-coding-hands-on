...
systemd config surface: .service/.timer recognition + systemd
...
class TestAssertNoSecrets:
    ...
    def test_combined_bearer_and_username_prose_full_repro_clean(self):
        """Both FP shapes in ONE realistic rendered document → zero warnings.

        Distinct from the two single-pattern tests above it: those prove each prose
        shape is individually exempt, this proves they stay exempt when they co-occur
        in a full artifact alongside headings, bold labels and citations — the shape a
        real gate caller (`validate_deployment_view.py`) actually hands to the gate.
        A regression here would block promotion of a clean doc, which is unrecoverable
        without a code change, so this case must never be folded into the two above.
        """
        digest = (
            "# Architecture\n\n"
            "## Deployment View\n\n"
            "**Source**: `docker-compose.yml:22`\n"
            "**Component**: partner-sync\n\n"
            "Calls the partner API using **Authorization**: Bearer "
    ...
    def test_api_key_literal_still_flagged(self):
        """A literal `sk_live_` API key is a leak, not a placeholder — must stay CRITICAL."""
