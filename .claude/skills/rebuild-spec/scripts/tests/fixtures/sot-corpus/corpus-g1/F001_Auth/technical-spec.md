...
#### BR-001_TermsMismatchSignout
**Linked FR:** FR-003
**Source:** `app/controllers/sessions_controller.rb:48-55`
**Applies to:** POST /sessions
**Rule:** After successful Devise authentication, if `community.consent != user.consent` and user is not admin, sign out the user and store temp session vars for deferred acceptance. Redirect to /terms.
...
#### BR-002_InviteOnlyRegistration
**Linked FR:** FR-004
**Source:** `app/controllers/people_controller.rb:65-79`
**Applies to:** POST /people
**Rule:** If `community.join_with_invite_only?` or `params[:invitation_code]` present, validate invitation code via `Invitation.code_usable?`. Abort with flash error and redirect on failure. On success, find invitation by code (upcased).
...
#### BR-003_HoneypotSpamBlock
**Linked FR:** FR-004
**Source:** `app/controllers/people_controller.rb:59-63`
**Applies to:** POST /people
**Rule:** If `params[:person]` is blank or `params[:person][:input_again]` is present (honey pot field), reject as spam and redirect.
...
#### BR-004_EmailAvailabilityCheck
**Linked FR:** FR-005
**Source:** `app/controllers/people_controller.rb:395-413`
**Applies to:** POST /people
**Rule:** Email must not be taken in community (`Email.email_available?`) and must be allowed by community (`community.email_allowed?`). Returns HTTP 302 to error_redirect_path with flash[:error] on failure.
...
#### BR-005_BannedUserBlock
**Linked FR:** FR-002
**Source:** `app/controllers/application_controller.rb:317-329`
**Applies to:** All non-exempt actions for logged-in users
**Rule:** `cannot_access_if_banned` fires on every request. If `@current_user.banned?` (and not global admin), redirect to access_denied_path. SessionsController skips this filter for `destroy` and `confirmation_pending`.
...
#### DEC-001_ReturnToRouting
**subtype:** flow
**Triggers in:** SCR020_LoginPage — `POST /sessions` form submit
**Involved entities:** Session[:return_to], Session[:return_to_content]
**user_visible_outcome:** User is sent to the page they originally requested, or to admin2 path, or to search_path after login.
...
#### DEC-002_RegistrationEmailConfirmSkip
**subtype:** flow
**Triggers in:** SCR021_SignupPage — POST /people success
**Involved entities:** APP_CONFIG.skip_email_confirmation
**user_visible_outcome:** In test/dev environments, registration bypasses email confirmation and redirects directly to search; in production, redirects to confirmation_pending_path.
...
#### DEC-003_ConfirmationEmailAdminRedirect
**subtype:** flow
...
**user_visible_outcome:** Admin users go to admin2 dashboard; normal users go to return_to or search_path; unauthenticated confirmers go to login.
...
#### SM-001_CommunityMembershipStatus
**kind:** entity
...
#### BR-006_UniquePersonPerCommunity
**Linked FR:** FR-001
**Source:** `app/models/community_membership.rb:56-59`
**Applies to:** CommunityMembership.create
**Rule:** `person_can_join_community_only_once` validation prevents duplicate membership for same person_id + community_id combination.
