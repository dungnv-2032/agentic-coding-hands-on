...
**Rule:** reCAPTCHA validation runs via `verify_recaptcha!` with 5-second timeout. Behaviour controlled by `APP_CONFIG.recaptcha_mode`: `:enforce` → return 400 on failure; `:log` → log failure but continue; no recaptcha_secret_key → skip validation entirely.
...
#### BR-002_FormValidation
**Linked FR:** FR-002
**Source:** `app/forms/form.rb:2-12`
**Applies to:** Both creation endpoints
**Rule:** `NewMarketplaceForm` requires: admin_email (format validated), admin_first_name (1–255 chars), admin_last_name (1–255 chars), admin_password (presence), marketplace_country, marketplace_language, marketplace_name, marketplace_type. `marketplace_type` must be one of: `product`, `rental`, `service`, `event`, `free`.
...
#### BR-003_IdentUniqueness
**Linked FR:** FR-003
**Source:** `app/services/marketplace_service.rb:239-251`
**Applies to:** MarketplaceService.create — ident generation
**Rule:** Subdomain (`ident`) derived from marketplace_name via URL-safe slug, truncated to 30 chars. If already taken or in RESERVED_DOMAINS list, numeric suffix appended and retried until unique.
...
#### BR-004_FirstMemberIsAdmin
**Linked FR:** FR-004
**Source:** `app/services/user_service/api/users.rb:71-73`
**Applies to:** `make_user_a_member_of_community`
**Rule:** If `community.community_memberships.count == 0`, the new membership gets `admin=true`. This ensures the marketplace creator is always the first admin, using all-status count (not just accepted) to handle unconfirmed admin edge case.
...
#### BR-005_AdminEmailUniqueness
**Linked FR:** FR-004
**Source:** `app/services/user_service/api/users.rb:10`
**Applies to:** `UserService::API::Users.create_user`
**Rule:** Raises `ArgumentError` if admin email already in use in the community (`Email.email_available?` returns false). This is caught by the StandardError rescue in the transaction block, returning `Result::Error`.
...
#### BR-006_TrialPlanExpiry
**Linked FR:** FR-006
**Source:** `app/controllers/int_api/marketplaces_controller.rb:30-33`
**Applies to:** POST /int_api/create_trial_marketplace only
**Rule:** Trial plan expires at 09:00 local time exactly 31 days from creation. Provisioned via `PlanService::API::API.plans.create_initial_trial`.
...
#### DEC-001_CreationPathRouting
**subtype:** flow
**Triggers in:** SCR019_NewCommunity — form submission
**Involved entities:** Community.full_url, UserService auth_token
**user_visible_outcome:** On success, admin is redirected to their new marketplace's admin2 dashboard with a one-time login token in the URL, authenticating them automatically without requiring a separate login step.
...
**user_visible_outcome:** In enforce mode, failed reCAPTCHA returns a 400 error; in log mode, the failure is logged but creation proceeds; with no key configured, reCAPTCHA is skipped entirely.
...
#### SM-001_MarketplaceCreationFlow
**kind:** ui
...
#### INT-002_TransactionSettingsProvisioning
**Linked FR:** FR-007
