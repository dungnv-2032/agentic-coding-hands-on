...
### Computes when to auto-reject a preauthorized transaction. Gateway expiration period (days) minus 10-minute buffer. If booking present, `preauth_expires_at` takes the minimum of gateway expiry and `booking.final_end`. (ALG-001)
**Linked FR:** FR-002
...
### Reminder scheduled 1 day before preauthorization expiry, but only if `reminder_at > Time.zone.now` — prevents stale reminders for short-duration preauths. (ALG-002)
**Linked FR:** FR-002
...
### Sends queue job to delayed_job queue → AutomaticallyRejectPreauthorizedTransactionJob (INT-001)
**Linked FR:** FR-002
...
### Sends queue job to delayed_job → SendPaymentReceipts job (INT-002)
**Linked FR:** FR-002
...
### Sends api call to PaypalService or StripeService gateway adapter (INT-003)
**Linked FR:** FR-005
