...
# F902_CapabilitiesEmptyCells — Technical Spec
...
| **A1** | `—` | `GET` `/login`,` | FR-001 | — *(read-only)* | § 3.2 |
| **A2** | `—` | `GET` `/signup` | FR-001 | — *(read-only)* | § 3.2 |
| **A3** | `—` | `POST` `/sessions` | FR-003 | — *(no DB write observed)* | § 3.2 |
| **A4** | `—` | `POST` `/people` | FR-004 | — *(no DB write observed)* | § 3.2 |
| **A5** | `—` | `POST` `/sessions/request_new_password` | US023_RequestPasswordReset | — *(no DB write observed)* | § 3.2 |
| **A6** | `—` | `DELETE` `/sessions/:id` | US028_Logout | — *(no DB write observed)* | § 3.2 |
| **A7** | `—` | `GET` `/community_memberships/access_denied` | US030_ViewAccessDenied | — *(read-only)* | § 3.2 |
| **A8** | `SendWelcomeEmail#perform` *(background, no FE)* | queue · `Delayed::Job` | — | — *(no DB write observed)* | § 3.2 |
| **A9** | `CommunityJoinedJob#perform` *(background, no FE)* | queue · `Delayed::Job` | — | — *(no DB write observed)* | § 3.2 |
| **A10** | `UNFILLED#perform` *(background, no FE)* | queue · `Delayed::Job` | — | `{UNFILLED SCAFFOLD}` | § 3.2 |

## 3. Actions

### 3.1 CAP-01 — {Capability name}

### 3.2 CAP-02 — {Capability name}
...
| Feature List | [feature-list.md](../../feature-list.md) | F902_CapabilitiesEmptyCells | [x] |
