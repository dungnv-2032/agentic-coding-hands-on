...
# F901_CapabilitiesHeaderOnly — Technical Spec
...
| **A1** | `—` | `GET` `/login`,` | FR-001 | — *(read-only)* | § 3.1 |
| **A2** | `—` | `GET` `/signup` | FR-001 | — *(read-only)* | § 3.1 |
| **A3** | `—` | `POST` `/sessions` | FR-003 | — *(no DB write observed)* | § 3.1 |
| **A4** | `—` | `POST` `/people` | FR-004 | — *(no DB write observed)* | § 3.1 |
| **A5** | `—` | `POST` `/sessions/request_new_password` | US023_RequestPasswordReset | — *(no DB write observed)* | § 3.1 |
| **A6** | `—` | `DELETE` `/sessions/:id` | US028_Logout | — *(no DB write observed)* | § 3.1 |
| **A7** | `—` | `GET` `/community_memberships/access_denied` | US030_ViewAccessDenied | — *(read-only)* | § 3.1 |
| **A8** | `SendWelcomeEmail#perform` *(background, no FE)* | queue · `Delayed::Job` | — | — *(no DB write observed)* | § 3.1 |
| **A9** | `CommunityJoinedJob#perform` *(background, no FE)* | queue · `Delayed::Job` | — | — *(no DB write observed)* | § 3.1 |
| **A10** | `UNFILLED#perform` *(background, no FE)* | queue · `Delayed::Job` | — | `{UNFILLED SCAFFOLD}` | § 3.1 |

## 3. Actions

### 3.1 CAP-01 — F901_CapabilitiesHeaderOnly
...
| Feature List | [feature-list.md](../../feature-list.md) | F901_CapabilitiesHeaderOnly | [x] |
