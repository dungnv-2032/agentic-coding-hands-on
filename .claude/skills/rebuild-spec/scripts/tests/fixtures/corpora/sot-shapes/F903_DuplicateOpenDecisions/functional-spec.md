...
<!-- SA-3 fixture marker: DEC-002 (declared in § 5, tagged on the registration-bypass
rule) is deliberately left off CAP-02's Business Rules cell, unlike F900's otherwise
identical table. This makes `cap.code_unclaimed` reachable and observable on this
fixture: if the stray "## 2. Open Decisions" heading below ever regressed `is_pre_sot`
back to muting section-bound checks (the SA-3 bug this fixture pins), that one
critical would silently vanish instead of firing. -->

| ID | Capability | What the user can do | User Stories | Requirements | Business Rules | Screens |
|----|------------|------------------------|---------------|---------------|-----------------|---------|
| CAP-01 | Sign in with credentials | Submit email/password and reach the post-login destination | US021 | FR-001, FR-002, FR-003, FR-006, FR-007 | BR-001, DEC-001, DEC-003, SM-002 | SCR020 |
| CAP-02 | Register a new account | Fill in the sign-up form, with an invite code where the marketplace requires one | US020 | FR-004, FR-005 | BR-002, BR-003, BR-004, BR-006, SM-001 | SCR021 |
| CAP-03 | Reset a forgotten password | Request a reset link by email and complete the reset | US023 | FR-008 | N/A | SCR025 |
| CAP-04 | Log out of the current session | Click logout and end the active session | US028 | FR-009 | N/A | N/A |
| CAP-05 | View the access-denied notice | See why access was blocked and navigate away | US030 | FR-010 | BR-005 | N/A |
...
```mermaid
journey
    title F903_DuplicateOpenDecisions user journey
...
<!-- SA-3 fixture marker: the heading below is a STRAY duplicate, deliberately placed outside its normal document position, pinning the mute-abuse shape reports/red-team-260819-1300-adjudication.md calls SA-3. It carries no real open-decision content. -->

## 2. Open Decisions

N/A -- this heading is a fixture defect, not a real decision list.
