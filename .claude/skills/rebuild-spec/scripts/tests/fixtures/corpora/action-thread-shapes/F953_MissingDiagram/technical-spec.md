...
# F953_MissingDiagram — Technical Spec
**Priority**: P1
**Type**: ui
...
|---|------------------|---------------|-------|--------|--------|
| **A0** | *cross-cutting — belongs to no single action* | — | DEC-001, DEC-002, DEC-003, FR-001, FR-101, FR-401, FR-601 | — | § 4.4 |
| **A1** | `ManageListingsController#index` | `GET` `/admin/listings/manage-listings` | FR-201, FR-202, US127 | — *(read-only)* | § 3.1 |
| **A2** | `ManageListingsController#update` | `PATCH` `/admin/listings/manage-listings/update` | BR-001, BR-004, FR-204, FR-205, US128 | `listings` | § 3.1 |
| **A3** | `ManageListingsController#close` | `PATCH` `/admin/listings/manage-listings/close` | BR-002, FR-206 | `listings` | § 3.1 |
| **A4** | `ManageListingsController#delete` | `DELETE` `/admin/listings/manage-listings/delete` | BR-003, FR-207 | `listings` | § 3.1 |
| **A5** | `ManageListingsController#export` | `GET` `/admin/listings/manage-listings/export` | FR-203, FR-402, INT-001 | `export_task_results` | § 3.1 |
| **A6** | `ManageListingsController#export_status` | `GET` `/admin/listings/manage-listings/export_status` | BR-006, FR-203, SM-002 | — *(read-only)* | § 3.1 |
| **A7** | `ManageListingsController#approve` | `PUT` `/admin/listings/manage-listings/:id/approve` | FR-204, SM-001 | — *(unreachable)* | § 3.1 |
| **A8** | `ManageListingsController#reject` | `PUT` `/admin/listings/manage-listings/:id/reject` | FR-205 | — *(unreachable)* | § 3.1 |
| **A9** | `ExportListingsJob#perform` *(background, no FE)* | queue · `Delayed::Job` | BR-005, US160 | `export_task_results` | § 3.1 |

## 3. Actions

### 3.1 CAP-01 — Review, search, and moderate marketplace listings

#### A1 · Paginated listings table with title/provider/dates/category/status

`GET /admin/listings/manage-listings` → `ManageListingsController#index`
`FR-201` `FR-202` `US127`

**BE** · **FR-201** Paginated listings table with title/provider/dates/category/status — `#index` renders `Listing::ListPresenter#listings` via `will_paginate` (Provider column only shown when `presenter.admin_mode`) [`app/controllers/admin2/listings/manage_listings_controller.rb:5`, `app/views/admin2/listings/manage_listings/index.haml:1-23`]; **FR-202** Search/filter by text and status — `ListPresenter#resource_scope` filters on `q` and `status[]` params (`q` matches title/category/author via SQL `LIKE`; statuses OR-merged) [`app/presenters/listing/list_presenter.rb:83-109`]; **US127** Search, filter, and export listings — `#index` + `#export`/`#export_status` (Independent Test in § 5.1) [`app/controllers/admin2/listings/manage_listings_controller.rb:5,35-49`]
**Result** · — **read-only**.
**Source:** `app/controllers/admin2/listings/manage_listings_controller.rb:5`

---

#### A2 · Approve/reject only apply while pending

`PATCH /admin/listings/manage-listings/update` → `ManageListingsController#update`
`BR-001` `BR-004` `FR-204` `FR-205` `US128`

**BE** · **FR-204** Admin approves a pending listing — `Admin2::ListingsService#approve` via `#update` (Sets `state`, increments `approval_count`, mails, notifies followers) [`app/services/admin2/listings_service.rb:33-38`]; **FR-205** Admin rejects a pending listing — `Admin2::ListingsService#reject` via `#update` (Sets `state`, mails rejection) [`app/services/admin2/listings_service.rb:40-43`]; **BR-001** Approve/reject only apply while pending — `ListPresenter#show_approval_link?` gates the UI; `ListingsService#update` dispatches by submitted state regardless (See DEC-001; service itself does not re-validate current state before applying) [`app/presenters/listing/list_presenter.rb:53-59`; `app/services/admin2/listings_service.rb:15-23`]; **BR-004** First approval notifies followers, later ones don't — `notify_followers` returns early unless `approval_count == 1` (Guards a `Delayed::Job.enqueue(NotifyFollowersJob...)`) [`app/services/admin2/listings_service.rb:83-87`]; **US128** Moderate a listing's status — `#update`/`#close`/`#delete`/`#approve`/`#reject` (Independent Test in § 5.1) [`app/controllers/admin2/listings/manage_listings_controller.rb:7-33`]
**Rule** ·
...
```
**Result** · UPDATE `listings` state, approval_count — `state` literal from submitted param; `approval_count` derived as current+1; UPDATE `listings` state — literal from submitted param
**Source:** `app/controllers/admin2/listings/manage_listings_controller.rb:7-10` → `app/services/admin2/listings_service.rb:33-38` → `app/services/admin2/listings_service.rb:40-43`

---

#### A3 · Closing does not delete

`PATCH /admin/listings/manage-listings/close` → `ManageListingsController#close`
`BR-002` `FR-206`

**BE** · **FR-206** Admin closes a listing — `Admin2::ListingsService#close` via `#close` (`listing.update!(open: false)`; no soft-delete) [`app/services/admin2/listings_service.rb:25-27`]; **BR-002** Closing does not delete — `open: false` update only, `deleted` untouched (Listing remains in `exist` scope and exportable) [`app/services/admin2/listings_service.rb:25-27`]
**Rule** ·
...
```
**Result** · UPDATE `listings` open — literal `false`
**Source:** `app/controllers/admin2/listings/manage_listings_controller.rb:12-15` → `app/services/admin2/listings_service.rb:25-27`

---

#### A4 · Delete is permanent and independent of close

`DELETE /admin/listings/manage-listings/delete` → `ManageListingsController#delete`
`BR-003` `FR-207`

**BE** · **FR-207** Admin permanently deletes a listing — `Admin2::ListingsService#delete` via `#delete` (`listing.update!(deleted: true)`; only action with a rescue) [`app/services/admin2/listings_service.rb:29-31`; `app/controllers/admin2/listings/manage_listings_controller.rb:27-33`]; **BR-003** Delete is permanent and independent of close — `deleted: true` update, no dependency on `open` (Soft-delete flag; filtered out via `Listing.exist` scope everywhere else) [`app/services/admin2/listings_service.rb:29-31`; `app/models/listing.rb:118`]
**Rule** ·
...
```
**Result** · UPDATE `listings` deleted — literal `true`
**Source:** `app/controllers/admin2/listings/manage_listings_controller.rb:27-33` → `app/services/admin2/listings_service.rb:29-31`

---

#### A5 · CSV export runs async, page polls for download

`GET /admin/listings/manage-listings/export` → `ManageListingsController#export`
`FR-203` `FR-402` `INT-001`

**FE** · **FR-402** Export disabled while filtered — `_filter.haml` appends `is-disabled` class when `params[:status]`/`params[:q]` present (Client-visual only; `#export` itself does not check filter params) [`app/views/admin2/listings/manage_listings/_filter.haml:7`]
**BE** · **FR-203** CSV export runs async, page polls for download — `#export` enqueues `ExportListingsJob`; `#export_status` returns token/status/url (See INT-001 and BL017 (behavior-logic.md) for the job itself) [`app/controllers/admin2/listings/manage_listings_controller.rb:35-49`]
**Result** · INSERT then UPDATE `export_task_results` status — `status` defaulted `'pending'` on create, then set `'started'`/`'finished'` by the background job
**Source:** `app/controllers/admin2/listings/manage_listings_controller.rb:35-38` → `app/models/export_task_result.rb:32-37`; `app/jobs/export_listings_job.rb:17,25`

---

#### A6 · Download link expires 10s after issue

`GET /admin/listings/manage-listings/export_status` → `ManageListingsController#export_status`
`BR-006` `FR-203` `SM-002`

**FE** · **SM-002** Export button polling status — `app/assets/javascripts/admin2/export.js` `initializeExportPolling` (See § 3.3) [`app/assets/javascripts/admin2/export.js:1-30`]
**BE** · **BR-006** Download link expires 10s after issue — `ExportTaskResult::AWS_S3_URL_EXPIRES_SECONDS` used in `expiring_url(...)` (See RISK-03 (functional-spec § 11)) [`app/models/export_task_result.rb:30`; `app/controllers/admin2/listings/manage_listings_controller.rb:44`]
**Rule** ·
...
```
**Result** · — **read-only**.
**Source:** `app/controllers/admin2/listings/manage_listings_controller.rb:41-49`

---

#### A7 · Admin approves a pending listing

`PUT /admin/listings/manage-listings/:id/approve` → `ManageListingsController#approve`
`FR-204` `SM-001`

**BE** · **SM-001** Listing moderation lifecycle — `Listing` `enum :state, APPROVALS` + `Admin2::ListingsService#approve`/`#reject` (See § 3.3; same field as DISC-012 in entities.md) [`app/models/listing.rb:144-149`; `app/services/admin2/listings_service.rb:33-43`]
**Result** · — **unreachable**.
**Source:** `app/controllers/admin2/listings/manage_listings_controller.rb:17-20` — `[INFERRED]` declared and implemented but no view/JS links to it; see RISK-01

---

#### A8 · Admin rejects a pending listing

`PUT /admin/listings/manage-listings/:id/reject` → `ManageListingsController#reject`
`FR-205`

**Result** · — **unreachable**.
**Source:** `app/controllers/admin2/listings/manage_listings_controller.rb:22-25` — `[INFERRED]` same as above

---

#### A9 · Export always covers the full community

queue · `Delayed::Job` → `ExportListingsJob#perform`
`BR-005` `US160`

**BE** · **BR-005** Export always covers the full community — `ExportListingsJob#generate_csv_content` reads `community.listings.for_export`, ignoring any request params (`for_export` scope = `exist.order('created_at DESC')`, no filter args) [`app/jobs/export_listings_job.rb:19,42`; `app/models/listing.rb:141`]; **US160** Export community data to CSV — `ExportListingsJob#perform`/`#generate_csv_rows` (System-type story; independent test in § 5.1) [`app/jobs/export_listings_job.rb:14-79`]
**Rule** ·
...
```
**Result** · UPDATE `export_task_results` file_file_name, file_content_type, file_file_size, file_updated_at — derived from the generated CSV filename/content via the Paperclip attachment
**Source:** `app/jobs/export_listings_job.rb:22-25`
...
| A1, A2, A3, A4, A5, A6, A7, A8 | 2 | ManageListingsController | `app/controllers/admin2/listings/manage_listings_controller.rb:1-61` | HTTP entry points for all 8 routes |
...
| A9 | 5 | ExportListingsJob | `app/jobs/export_listings_job.rb:1-106` | background CSV generation (BL017) |
...
| Feature List | [feature-list.md](../../generated/feature-list.md) | F953 | [x] |
...
| Permissions Matrix | [permissions-matrix.md](../../generated/permissions-matrix.md) | — (PERM001 gates this screen via the blanket admin2 `ensure_is_admin` filter — cross-feature, not owned by F953, see FR-001/FR-601; feature-list.md's PERM016 assignment does not govern this screen — PERM016 only guards the public `ListingsController#move_to_top`/`#show_in_updates_email` actions, see permissions-matrix.md § PERM016) | [ ] |
