...
|------|------|--------------------------|------------------|--------|
| FR-001 | System validates credentials | `POST /login` via `LoginController::store` | | `app/Http/Controllers/LoginController.php:45-55` |
...
<!-- v27.0.0 (D-f): the Source field is intentionally omitted below — subtype/Triggers
in/Involved entities/user_visible_outcome are all present (user_visible_outcome is
forward-compatible per D-f) to prove Source is the field that still fails the block,
isolated from every other field. -->
**subtype:** render
**Triggers in:** SCR001_LoginForm — form submit event
**Involved entities:** User.status
**user_visible_outcome:** User is redirected to dashboard on success or shown error on failure.
