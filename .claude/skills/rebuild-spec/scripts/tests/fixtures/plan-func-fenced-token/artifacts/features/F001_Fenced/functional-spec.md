...
**Problem:** Users need to sign in. Historically, clients called `POST /api/login` directly.
**Detail:** The legacy flow lived in `src/auth.ts:42` before the rewrite.
**Aside:** Internally ``the endpoint was POST `secured` behind a gateway`` for a while.
