...
# Technical Spec — F017_TransportAndCookieSecurity
...
### 4.1 Force HTTPS and migrate the legacy session-cookie name
...
| Feature List | [feature-list.md](../../generated/feature-list.md) | F017 | [x] |
| API Map | [api-map.md](../../generated/api-map.md) | — (none owned by F017; applies to every route) | [ ] |
| Entities | [entities.md](../../generated/entities.md) | — (none owned by F017) | [ ] |
...
1. **File:** `config/application.rb:105-113` — start here: shows exactly where and in what
   order both middleware are inserted relative to `Rack::Sendfile` and
   `ActionDispatch::Cookies`, which explains why order matters at all.
2. **File:** `lib/rack_middleware/enforce_ssl.rb:1-32` — next: the full HTTPS-enforcement
   middleware, read top to bottom (it is 32 lines).
3. **File:** `lib/rack_middleware/custom_cookie_renamer.rb:1-14` — next: the full cookie-rename
   middleware.
4. **File:** `config/config.defaults.yml:7,155,244-245` — last: the four config values
   (`domain`, `always_use_ssl`, `session_key`, `cookie_session_key`) both middleware read.
...
Rack request
  -> EnforceSsl#call
       always_use_ssl && !ssl? -> 301 redirect (BR-001, BR-002)
       else                    -> @app.call(env)  [continues down the stack]
  -> ... (Rack::Sendfile, Rack::MethodOverride, ...)
  -> CustomCookieRenamer#call
       HTTP_COOKIE present && new key absent -> rename in place (BR-003)
       else                                   -> @app.call(env)  [unchanged]
  -> ActionDispatch::Cookies -> MarketplaceLookup -> SessionContextMiddleware -> Rails routing
```
...
N/A — read-only feature, no DB writes. Both middleware operate entirely on the in-memory Rack
`env` hash (redirect response / cookie-header rewrite); neither touches ActiveRecord or issues
any SQL.
