...
class TestIdempotent:
    def test_running_twice_does_not_double_insert(self, tmp_path):
        ...
        # v27.0.0: heading form is now a trailing `(BR-NNN)` tag, not an anchored
        # prefix, so count real blocks via _spec_block_lib.find_blocks rather than
        # a literal "### BR-" substring (which no longer appears in the new form).
        import sys
        sys.path.insert(0, str(SCRIPTS_DIR))
        from _spec_block_lib import find_blocks
        assert content.count("**Linked FR:** FR-???") == len(find_blocks(content))
