...
# ---------------------------------------------------------------------------
# [D-g] business-rules.md removal (rebuild-spec 27.0.0 Phase 03): the CORE_ARTIFACTS
# copy loop's `is_file()` guard (promote_drafts.py — verified present, not added) must
# tolerate BOTH the now-normal absence of business-rules.md AND a stale leftover copy
# from a pre-27.0.0 researcher run, without raising. Full suite green either way.
# ---------------------------------------------------------------------------

class TestBusinessRulesRemovalRegression:
    def test_full_promote_without_business_rules_completes(self, tmp_path):
        """[D-g] The fixture no longer ships business-rules.md (researcher stopped
        producing it in 27.0.0). A full --scope core promote must complete without
        raising and without ever attempting business-rules.md."""
        plan_dir, docs_root = _make_scope_plan("dg_no_business_rules")
        assert not (plan_dir / "artifacts" / "business-rules.md").exists(), \
            "fixture must not ship business-rules.md post-27.0.0"
        r = _run_promote(
            ["--plan-dir", str(plan_dir), "--docs-root", str(docs_root),
             "--mode", "full", "--scope", "core"],
            cwd=REPO_ROOT,
        ...
        assert r.returncode == 0, r.stderr
        assert not (docs_root / "system" / "business-rules.md").exists()

    def test_full_promote_with_stale_business_rules_present_completes(self, tmp_path):
        """[D-g] A pre-27.0.0 docs tree may still have a stale business-rules.md draft
        sitting in artifacts/ (left over from before the upgrade). Since it is no
        longer in CORE_ARTIFACTS, the copy loop must simply never look at it — no
        raise, no promotion, no crash. Tolerated until Phase 06 migrates it away."""
        plan_dir, docs_root = _make_scope_plan("dg_stale_business_rules")
        (plan_dir / "artifacts" / "business-rules.md").write_text(
            "# Business Rules\n\nStale pre-27.0.0 draft.\n", encoding="utf-8",
        )
        r = _run_promote(
            ["--plan-dir", str(plan_dir), "--docs-root", str(docs_root),
             "--mode", "full", "--scope", "core"],
        ...
        assert r.returncode == 0, r.stderr
        # Not in CORE_ARTIFACTS anymore — never copied, regardless of its presence.
        assert not (docs_root / "system" / "business-rules.md").exists(), \
            "business-rules.md must never be promoted post-27.0.0, even if the stale " \
            "draft is present in artifacts/"

    def test_core_artifacts_list_has_no_business_rules_literal(self):
        """[success criteria] CORE_ARTIFACTS no longer names business-rules.md."""
        sys.path.insert(0, str(SCRIPTS_DIR))
        from promote_drafts import CORE_ARTIFACTS  # noqa: PLC0415

        assert "business-rules.md" not in CORE_ARTIFACTS

    def test_layered_path_map_has_no_business_rules_literal(self):
        """[success criteria] _layout_lib.LAYERED_PATH_MAP no longer names business-rules.md."""
        sys.path.insert(0, str(SCRIPTS_DIR))
        from _layout_lib import LAYERED_PATH_MAP  # noqa: PLC0415

        assert "business-rules.md" not in LAYERED_PATH_MAP
