...
class TestMarkPass:
    ...
    @pytest.mark.parametrize("retired_pass", ["jobs", "design-intent"])
    def test_retired_pass_is_not_registered(self, tmp_path, retired_pass):
        """The `--jobs` and `--design-intent` passes are RETIRED — they must stay OUT of
        PASS_NAMES/PASS_PREREQS. Leaving either registered resurrects a zombie: the batch
        driver's next_pending_pass() would report it pending forever, because no surviving
        caller can ever reach mark_pass_done() for it. Inverted from the v26.1.0 F1
        registration regression this replaces."""
        assert retired_pass not in plib.PASS_NAMES
        assert retired_pass not in plib.PASS_PREREQS
        comps = _make_components(1)
        manifest_path = _emit(tmp_path, comps)
        _mark_core_done(manifest_path, [comps[0]["path"]])
        entries = mlib.load_manifest(manifest_path)
        with pytest.raises(ValueError):
            plib.next_pending_pass(entries, retired_pass)
        with pytest.raises(ValueError):
            plib.mark_pass_done(manifest_path, comps[0]["path"], retired_pass)
