...
Split out of _nav_feature_lib to keep each module under the 200-LOC invariant.
  ...
  - parse_screen_names()  — Screens table of a feature's functional-spec.md § 5 (column-aware)
...
# digits), so we anchor only on the leading boundary. That leaves the OTHER edge open,
# though: with no guard at all, "SCR0011" over-matches as "SCR001" (a wrong nav row
# bound). (?!\d) ("not followed by another digit") closes that without reintroducing the
# slug-defeated trailing \b.
...
# v27.0.0 (audience split): the Screens table lives in functional-spec.md § 5 now
# (screens.md is retired). Matches the new numbered heading ("## 5. Screens") and,
# tolerantly, the retired "## Screen List" text (pre-migration docs).
_SCREENS_SECTION_HEAD_RE = r"#+\s*(?:\d+\.\s*)?Screens?(?:\s+List)?\b"


def parse_screen_names(screens_md: str) -> list[dict]:
    """Parse the Screens table of a feature's functional-spec.md § 5 (column-aware).
    ...
    """
    table = _first_table_after(screens_md, _SCREENS_SECTION_HEAD_RE)
    if len(table) < 2:
        return []
