...
# v27.0.0 (audience split): feature dir 4 files -> 2. business-context.md/screens.md/
# edge-cases.md are retired; functional-spec.md (BA/QA) absorbs all three. technical-spec.md
# (dev/QA/SA) stays. See plans/260814-1106-rebuild-spec-audience-split/.
FEATURE_FILES = ("technical-spec.md", "functional-spec.md")

# The satellite file(s) a BA/QA reader visits alongside technical-spec.md — i.e.
# FEATURE_FILES minus the developer-only technical-spec.md itself. Used wherever a check
# is scoped to the non-technical-spec sibling(s) only (e.g. citation validation, which is
# technical-spec-only and skips this file). Single source: derived, not re-typed.
SATELLITE_FEATURE_FILES = tuple(f for f in FEATURE_FILES if f != "technical-spec.md")

# Phase 06 (migration): the three v26 satellite files functional-spec.md absorbed and
# retired. Needed by migrate_feature_audience_split.py to RECOGNIZE the pre-migration
# (v26) shape of a feature dir it has not touched yet — single-sourced here (not
# re-typed as a tuple/list/set literal elsewhere; see test_spec_constants_single_source.py
# guard (a)).
RETIRED_V26_SATELLITE_FILES = ("business-context.md", "screens.md", "edge-cases.md")
