...
Every draft this run produces (the `feature-list.md` from Step 0a AND every feature's 2-file set) is
...
- **"Approve — proceed with this feature set"** (Recommended) → delete the sentinel (`rm plans/<plan_dir>/.rp-1.5a-pending`), then run the scaffolder (sentinel now ABSENT → proceeds), then fan out Steps 1–2.5 per feature.
  ...
  **Scaffolder invocation (SYSTEM, APPROVED branch only — runs once after sentinel delete, before fan-out):**
  ...
  `--feature-names`. The scaffolder derives kebab slugs, creates all feature dirs + 2-file sets,
...
**Run the scaffolder BEFORE spawning the researcher.** The scaffolder creates the 2-file set with
...
`plans/<plan_dir>/spec/<slug>/` 2-file set. For a SINGLE feature this is one spawn (N = 1).
...
- `plans/<plan_dir>/spec/<slug>/technical-spec.md`, `functional-spec.md` — fill each file's
  sections; frontmatter and H2 structure are already correct
...
# 2. functional-spec.md canonical Screens heading — researcher may overwrite with wrong heading.
#    Number-agnostic on purpose (P12, target-shape-spec.md renumbered old '## 5. Screens' to
#    '## 6. Screens' and this literal '^## 5\.' grep went stale the moment that landed — it
#    printed FAIL on every correctly-shaped draft). `_nav_table_parse_lib.py` and
#    `validate_feature_screen_link.py` already match Screens this way, which is exactly why
#    they survived that renumber untouched while this lint did not — same shape here so the
#    next renumber doesn't re-break it.
grep -qE '^##\s*(?:[0-9]+\.\s*)?Screens$' "$d/functional-spec.md" && echo "OK  functional-spec.md 'Screens' heading present" \
  || echo "FAIL  functional-spec.md missing its 'Screens' H2 (found 'Screen Route Table' or other?)"
# 3. technical-spec.md required headings present + deprecated absent — researcher may remove/add.
#    Artifact References moved from a standalone H2 to '### 5.5 Artifact References' (H3, under
#    '## 5. Verification & Technical Notes') in the P08 5-bucket retaxonomy — matched
#    number-agnostically for the same reason as the Screens check above.
grep -qE '^###\s*(?:[0-9]+\.[0-9]+\s*)?Artifact References$' "$d/technical-spec.md" && echo "OK  Artifact References present" \
  || echo "FAIL  technical-spec.md missing '### N.N Artifact References' under § Verification & Technical Notes (validator-critical at promote)"
grep -qE '^## (Functional Requirements|Requirements|Business Rules|Success Criteria)$' "$d/technical-spec.md" \
  && echo "FAIL  deprecated standalone H2 in technical-spec.md (FRs/BRs are stated in functional-spec.md; technical-spec.md maps codes under § 2 / § 4)" \
...
if ls "$d"/screens/*/spec.md >/dev/null 2>&1; then
  grep -rlE '^## (Layout & Component Tree|Form Fields|User-Visible Copy|Constraints)$' "$d"/screens/*/spec.md \
    && echo "FAIL  screen spec uses ad-hoc headings — follow screen-spec-template.md (## 2. Screen Layout → ### Layout Sketch / ### Layout Regions; ## 5. UI States; ## 6. Validation & Feedback)" \
...
a non-canonical `functional-spec.md` Screens heading, a missing `Artifact References` sub-section, a deprecated standalone
...
SYSTEM:
```
⚒ Specs drafted — N features in plans/<plan_dir>/spec/ (feature-list.md + N×2-file, status: draft, not yet in docs/)
```
