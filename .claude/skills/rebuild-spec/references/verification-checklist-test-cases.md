...
of record), `docs/features/{fcode}/functional-spec.md` (negative-path source AND UAT source — the
former `edge-cases.md`/`screens.md`/`business-context.md` triple merged into this single file).
...
code/file:line/functional-spec.md; UAT → functional-spec.md section), coverage-gap report
...
- [ ] **TC-S2 UAT citation is genuinely a functional-spec.md section:** a UAT row whose Traces-to
  is a code dressed up as a section reference (e.g. `BR-001 (see functional-spec.md)`) is a
  smuggled code-only citation — critical (defeats the citation-source split's purpose).
...
- [ ] **TC-S4 `functional-spec.md` reuse is faithful:** a UT/IT row citing functional-spec.md's
  Edge Cases section (§8) must match that row's actual Scenario/What-Happens content, not a
...
- **Trap — citation-source laundering:** a UAT row citing a code, or a UT/IT row citing only a
  functional-spec.md section that is not actually the Edge Cases (§8) content, defeats the entire
  point of the citation-source split (risk (b) from the design phase) → critical, not a style nit.
