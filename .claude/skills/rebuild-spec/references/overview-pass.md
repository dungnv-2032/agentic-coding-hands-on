...
Prerequisite chain (single source of truth: SKILL.md § Pass ordering & prerequisites).
**Exactly one artifact is mandatory — `docs/generated/feature-list.md`** — matching both the ABORT
gate below and SKILL.md's table. A completed core pass normally also promotes the artifacts below;
each feeds specific sections and an absent one degrades its section rather than blocking the pass:

- `docs/system/overview.md`, `docs/system/architecture.md`, `docs/system/permissions.md`
- `docs/generated/screen-list.md`, `screen-flow.md`, `entities.md`, `permissions-matrix.md`, `api-map.md`, `behavior-logic.md`
...
`docs/features/*/{technical-spec,functional-spec}.md` (§5 detail, §11 open issues),
...
| 2 | Purpose & Business Value | overview + behavior-logic.md (As-Is/To-Be merged → purpose; no "old-system" narrative) |
