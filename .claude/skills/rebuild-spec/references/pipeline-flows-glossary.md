...
**Requires (shared ABORT gate):** `docs/features/*/technical-spec.md` must exist for at least one feature (else ABORT). **This line is the shared gate only, not the full per-pass prerequisite:** `--flows` additionally requires `docs/generated/entities.md`, and `--glossary` requires `docs/features/*/functional-spec.md` + `docs/generated/entities.md` — see the pass-specific preflight below and SKILL.md § Pass ordering & prerequisites.
...
const upstreams = {
  ...
  behaviorLogic:  "docs/generated/behavior-logic.md",      // BL cross-ref source (business-rules.md merged in, v27.0.0)
  featureSpecs:   "docs/features/*/technical-spec.md",   // FL.1 SM-### source
  functionalSpecs:"docs/features/*/functional-spec.md",  // GL.1 term source
}
...
3. docs/generated/behavior-logic.md — scheduled jobs, events (BL### trigger source), guards, invariants
...
Also EMIT \`### Trigger Sequence\` (a sequenceDiagram) after \`## State Diagram\`, per templates/process-flow-template.md — actor→entry point→system→DB hops for the SAME triggers as the state diagram. Every arrow implying a call MUST be source-cited (reuse the matching Transitions row's Source cell) or marked "derived from Transitions table above" (same anti-hallucination bar). WARN-first companion (v26.2.0) — absence never blocks a pass, but always emit it on fresh output.

OUTPUT: plans/<active-plan>/artifacts/flows/<slug>.md (one per qualifying entity, FLOW### code).
Collision: human-curated (docs/flows/<slug>.md with \`status: human-curated\`) → preserve, emit \`[INFO] flow_preserved\`; ai-draft → overwrite. Name collision → suffix -2/-3, emit \`[WARN] flow_slug_suffixed\`. (Marker strings moved from SKILL.md — meaning unchanged, only their home.)
...
// [v27.12.0] Reconcile advisory — Trigger Sequence companion coverage (non-gating). Counts
// promoted Tier-1 flow files still missing the `### Trigger Sequence` sequenceDiagram fence;
// system-flow.md and confidence-report_*.md sidecars excluded. Advisory only — FL.2 already
// WARNed per-file; nothing to migrate (old flows legitimately lack it, no auto-author possible).
bash: MISSING_SEQ=$(for f in ${docs_root}/flows/*.md; do \
  case "$(basename "$f")" in system-flow.md|confidence-report_*) continue ;; esac; \
  grep -q "sequenceDiagram" "$f" || echo "$f"; \
done | wc -l)
...
If `$MISSING_SEQ` > 0: `Note: $MISSING_SEQ Tier-1 flow(s) lack ### Trigger Sequence — re-run /tkm:rebuild-spec --flows to add.`
...
|------|----------|-------|--------|
| FL.1 | `researcher` | `docs/generated/entities.md` + `docs/generated/screen-flow.md` + `docs/generated/behavior-logic.md` + `docs/features/*/technical-spec.md` + `process-flow-researcher-contract.md` + `process-flow-template.md` | `plans/<active>/artifacts/flows/*.md` (FLOW### per qualifying entity) + `flows/.completed` + `flows/system-flow.md` (Tier-2, when ≥2 Tier-1 flows) |
...
Synthesizes the project glossary from data-model entity names + feature functional-spec prose.
**Requires (glossary-specific):** `docs/features/*/functional-spec.md` must exist (functional-spec needed for term sourcing).
...
```js
const functionalSpecFiles = glob("docs/features/*/functional-spec.md")
if (functionalSpecFiles.length === 0) {
  throw new Error(
    `ABORT — No functional-spec.md files found under docs/features/. ` +
...
Synthesize Glossary from docs/generated/entities.md + functional-spec files across all feature specs.
Output: plans/<active-plan>/artifacts/glossary.md

SOURCES (read in order — all from docs/, not artifacts/):
1. docs/generated/entities.md — entity names, field names, technical terms
2. docs/features/*/functional-spec.md — plain-language term usage
...
- Include domain terms that appear in >=2 functional-spec files
...
| Wave | Subagent | Input | Output |
|------|----------|-------|--------|
| GL.1 | `researcher` | `docs/generated/entities.md` + `docs/features/*/functional-spec.md` | `plans/<active>/artifacts/glossary.md` + `TaskUpdate(status=completed)` |
