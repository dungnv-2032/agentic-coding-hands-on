...
| permissions-matrix / permissions / system/* / future | — (no unit) | source-file count (crude) or default single | — | single (no post-merge check) | — | — |
...
## `functional-spec.md` shard seam (v27.0.0, prose rule only — no mechanism)

`functional-spec.md` has **no Descriptor Table row and gains none.** The audience split's
1-line-per-rule density (plan Validation Decision V1, `validation-log.md`) cuts artifact length
roughly 10× versus the old 4-file layout, so the `estimate_artifact_loc.py` `--max-loc` threshold
this table gates on may never be reached in practice for this file — wiring a real shard path
now would be speculative (YAGNI).

**If** an oversized `functional-spec.md` ever occurs, the documented seam (plan Validation
Decision V3) is **FR number-band boundaries** — the same bands `§ 3 Requirements` already groups
under (`0xx` foundation / `1xx` navigation / `2xx`-`3xx` per-screen / `4xx` interaction / `6xx`
security): `FR-0xx`/`1xx` → shard 1, `2xx`/`3xx` → shard 2, and so on. Bands are already the
semantic reading unit, so a shard seam there falls on a boundary a BA/QA reader already expects.

This is a **documented rule only** — no `estimate_artifact_loc.py` row, no validator, no
Descriptor Table entry, no merge-recipe wiring. An implementer who adds a sharding code path
here for `functional-spec.md` has exceeded this contract's scope.
