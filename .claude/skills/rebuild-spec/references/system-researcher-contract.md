...
## Deployment View (optional, v27.13.0)

`architecture.draft.md`'s topology narrative MAY include a `## Deployment View` subsection when a
component's own `architecture.md` already carries one (system-tier template,
`templates/architecture-template.md`) with genuinely cross-service placement/connectivity —
otherwise omit it entirely; this section is never mandatory at the aggregate tier. When you do
include one:

- Follow `references/deployment-source-patterns.md` for what counts as a citable IaC source.
- Carry over the mandatory honesty label verbatim: `> Derived from repository infrastructure-as-code
  — not verified against production.`
- Same citation invariant as the system tier (FR-7): every node/edge you draw must trace to a
  `file:line` a component's own Deployment View already cites — never infer a new host or edge
  the components' docs did not themselves cite.
- Scrub every value through `_credential_scrub_lib.py` before it becomes a node/edge label; a
  component doc that already leaked one is a defect in that component's docs, not license to repeat
  it here.
