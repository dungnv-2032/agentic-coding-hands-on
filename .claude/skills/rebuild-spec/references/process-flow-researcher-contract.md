...
- `behavior-logic.md` — BL### scheduled jobs, events, observers (scheduled/event trigger source);
  also the guard/invariant source (v27.0.0: the former separate `business-rules.md` system doc
  is retired — its plain-language guard/invariant content is now part of this same artifact).
...
## Trigger Sequence Companion (EXPECTED, WARN-first)

Each Tier-1 flow file SHOULD also emit `### Trigger Sequence` (a `sequenceDiagram`) immediately after `## State Diagram`, per `templates/process-flow-template.md`. The state diagram documents *what states, what triggers*; the sequence diagram documents *who calls whom* — actor -> entry point -> system -> DB hops — for those same triggers. It is an additive companion, not a replacement for `stateDiagram-v2` under `## State Diagram`.

**Same anti-hallucination obligation as the Citation Rule above:** every sequence arrow that implies a call MUST either carry a source citation (reuse the matching Transitions row's `Source` cell) or be marked "derived from Transitions table above" when it is a direct restatement of an already-cited transition. Never fabricate a hop that has no basis in the Transitions table or source code.

Absence of this companion is a **WARN**, never a CRITICAL — `scripts/validate_process_flow.py` rule `ProcessFlow.sequence_diagram.pre_migration`. Old flows written before v26.2.0 legitimately lack it; there is no migration script (nothing to auto-author from). Emit it on every fresh `--flows` run going forward.
...
Apply this forbidden-tokens regex to all prose (description, state meanings, trigger descriptions)
— v27.0.0: inlined here (formerly sourced from the now-retired `templates/business-context-template.md`;
its ban-all-codes rule does not carry over to `functional-spec-template.md`, which inverts it and
allows FR/BR/SM/DEC/SCR/US codes by design, so FLOW### prose keeps its own copy instead):
...
Any match outside a fenced code block = CRITICAL; rewrite in plain language.
