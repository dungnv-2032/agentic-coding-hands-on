...
// --- W9.5a: Deployment View gate (v27.13.0) — runs PRE-PROMOTE, on the draft ---
// Deliberately before promote_drafts.py, not after: `## Deployment View` is derived from repository
// IaC, and IaC files (`environment:` blocks, connection strings) are the likeliest place in a repo
// for an embedded secret. Gating the DRAFT means a leaked credential never reaches docs/ at all —
// and `--package` later hands docs/ to an external client, so a post-promote check would already be
// too late.
// POSTURE (phase-03 FR-5/NFR): WARN-first. A missing section, a missing IaC source, an uncited node,
// a split diagram — all WARN, exit 0, pass proceeds. Absence is expected for legacy corpora and for
// stacks whose profile carries no `deployment_sources` (cobol/delphi-vcl/oracle-plsql), so this gate
// adds NO new blocking condition for any corpus that merely lacks the section.
// The ONE hard CRITICAL is a detected secret literal — exit 1 → HALT. A leaked credential value is
// the one defect an LLM review cannot be trusted to catch and the one that cannot be walked back
// once promoted, so it gets a deterministic check; that is why this file is gated at all rather
// than left to the W7a semantic review, which is an LLM read, not a check.
bash: .claude/skills/.venv/bin/python3 \
  claude/skills/rebuild-spec/scripts/validate_deployment_view.py \
  --plan-dir plans/<active-plan> \
  --summary-out plans/<active-plan>/artifacts/validation/deployment-view-validation-summary.json

// exit 0 → PASS or WARN → proceed to promote
// exit 1 → FAIL (secret literal in the Deployment View) → HALT; surface the JSON issue; the fix is
//          to omit the value and re-run, NEVER to render a partially-masked string
// exit 2 → internal validator error (e.g. path outside repo root) → surface stderr; halt
...
// Canonical core paths: system/(overview, architecture, permissions) +
...
// v27.0.0: system/business-rules.md retired — merged into generated/behavior-logic.md (already listed).
...
const coreDocsToAssert = [
  ...
  `${docs_root}/system/permissions.md`,
...
// --- W9.5b: Traceability matrix (v27.11.0 W6) — MUST run BEFORE W9.6 ---
// Re-projection only: reads the PROMOTED docs/ artifacts (feature-list, screen-list, user-stories,
// behavior-logic, route-list, permissions-matrix, + test-cases when that pass ran) and
// joins them on F###. Introduces zero new detection; every ID it prints already exists and already
// validated in its source-of-record.
// ORDER IS LOAD-BEARING: build_navigation.py's `## Document Map` renders its "Tracing one feature
// end-to-end?" pointer only when docs/generated/traceability-matrix.md is ON DISK
// (_nav_docmap_lib.py gates on os.path.isfile). Run this AFTER promote and BEFORE W9.6, or the
// pointer silently never appears and READING_ORDER #19 always reads as absent.
bash: .claude/skills/.venv/bin/python3 \
  claude/skills/rebuild-spec/scripts/build_traceability_matrix.py --docs-root ${docs_root}

// WARN-first re-projection check: every printed ID must resolve in its source-of-record, and the
// matrix must nowhere be an ID's first appearance. Never a pass failure (exit 0 always).
bash: .claude/skills/.venv/bin/python3 \
  claude/skills/rebuild-spec/scripts/validate_traceability_matrix.py --docs-root ${docs_root}
...
headless run promotes neither — do not treat an absent draft as an error):
  system/    ← overview.md, architecture.md, permissions.md
