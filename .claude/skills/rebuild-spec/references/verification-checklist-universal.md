...
- Per-feature drafts: each feature dir must contain 2 files (technical-spec.md, functional-spec.md) AND no `.pending` marker. Missing any → MISSING verdict; counts toward review-report `failed`.

Counting: `critical` → `failed`; `warning` → `warnings`; `MISSING` (see § Pending Marker Rule) → `missing` (counts toward `failed` for Wave 9 gate).

### Pending Marker Rule

`.pending` is a zero-byte sentinel written by Wave 5 and removed by the FS.1 researcher on successful 2-file write (see `references/canonical-fcode-schema.md § Folder Lifecycle`). When the FS.5 reviewer encounters `.pending` in `artifacts/features/{slug}/`:
