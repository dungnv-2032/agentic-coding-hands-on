...
3. **[RT-M2] Hydrate canonical upstream paths** via `claude/skills/_shared/docs-canonical-mapping.md`:
   ...
   - behavior-logic → `docs/generated/behavior-logic.md` (if present)
...
Generate DETAILED spec for ${fcode} — 2 audience-aware files.
...
TEMPLATES: technical-spec-template.md, functional-spec-template.md.
DRAFT: plans/<active-plan>/artifacts/features/${fcode}/{technical-spec,functional-spec}.md (BOTH required).
POST-WRITE: Run rm plans/<active-plan>/artifacts/features/${fcode}/.pending ONLY after BOTH files written and non-empty.
...
2. Grep each code's section from its canonical artifact (US→docs/generated/user-stories.md, SCR→docs/generated/screen-list.md, BL→docs/generated/behavior-logic.md, MODEL→docs/generated/entities.md, ROUTE→docs/generated/route-list.md).
3. Read in full: docs/generated/behavior-logic.md, any docs/flows/<slug>.md relevant to this feature.
...
CAPABILITY-CHUNKED DRAFTING (v27.7.0, action-thread reshape — replaces the retired 400-line
"SIZE-ADAPTIVE DRAFTING" line-count rule): before drafting, run
\`.claude/skills/.venv/bin/python3 claude/skills/rebuild-spec/scripts/decide_action_chunking.py --feature-dir plans/<active-plan>/artifacts/features/${fcode}\`
(falls back to \`--actions <N> --capabilities <M>\` when the feature dir does not exist yet on a
first pass). Its JSON \`{chunk, slices, reason}\` is the ONLY gate — do NOT restate its predicate
in prose here (a prose-only rule is "a rule believed to be running," not a running gate — this
is exactly the defect the script exists to close). \`chunk: false\` → today's single-task
behavior, unchanged. \`chunk: true\` → dispatch ONE researcher per capability in \`slices\`; each
writes ONLY its capability's § 3 action blocks (plus, for the FIRST slice only, §§ 1-2 and the
skeleton of § 4/§ 5) as a fragment file:
\`plans/<active-plan>/artifacts/_fragments/technical-spec/${fcode}/NN-<cap>.md\` (NN = ordinal
matching the slice's position in \`slices\`, so \`ls | sort\` reproduces CAP- order). After ALL
fragment tasks for this feature complete, the orchestrator merges:
\`ls plans/<active-plan>/artifacts/_fragments/technical-spec/${fcode}/*.md | sort\` → join →
inject into the feature's technical-spec.md draft → \`rm -rf
plans/<active-plan>/artifacts/_fragments/technical-spec/${fcode}/\`. This is the SAME
fragment/merge mechanism as \`plans/260602-2230-rebuild-spec-chunked-artifact-generation/\`
(the \`ls|sort → join → inject into {POPULATED_BY_*} → rm -rf\` recipe) — reused, not
reinvented; see that plan's phase-01 for the full recipe if a merge-step detail is unclear.
Output stays ONE \`technical-spec.md\` per feature either way (D6, LOCKED) — chunking slices
generation, never the artifact. **Out of scope, deliberately:** \`estimate_artifact_loc.py\` /
\`docs.maxLoc\` is a DIFFERENT mechanism (core-artifact/api-contracts LOC-based sharding) —
feature specs never went through it, and this reshape does not touch it; do not "helpfully"
align the two thresholds.
FUNCTIONAL-SPEC CROSS-REFS: for functional-spec.md § 1 Overview, cite the flow slugs from docs/flows/ this feature participates in.

FEATURE ENTRY POINTS (fragment file pattern — mandatory):
After writing both files, emit fragment: bash: mkdir -p plans/<active-plan>/artifacts/_feature-entry-points/
...
- Canonical upstreams: entities.md (data-model), feature-list.md, user-stories.md, screen-list.md, behavior-logic.md (if present), docs/flows/*.md relevant to each feature.
- TEMPLATES: technical-spec-template.md, functional-spec-template.md (2 files per feature — BOTH required).
- DRAFT: plans/<active-plan>/artifacts/features/{slug}/{technical-spec,functional-spec}.md
- POST-WRITE: run \`rm plans/<active-plan>/artifacts/features/{slug}/.pending\` ONLY after BOTH files exist and non-empty.
- FEATURE ENTRY POINTS: emit plans/<active-plan>/artifacts/_feature-entry-points/{fcode}.md per feature.
- CAPABILITY-CHUNKED DRAFTING: run \`decide_action_chunking.py --feature-dir {feature-dir}\` per feature before drafting; \`chunk: true\` fans out one fragment researcher per capability into \`_fragments/technical-spec/{fcode}/\`, same recipe as the small-codebase WaveFS1 block above.
...
// v24.0.0 — feature↔screen ID-link validator (WARN-capable, NON-halting). Checks
// functional-spec.md § 6's SCR### column (Screens; SOT reshape renumbered § 5 → § 6 —
// `_SCREENS_SECTION_HEAD_RE` matches the heading number-agnostically so this validator
// survived that renumber untouched) resolves to screen-list.md, and (once screen-specs
// exist) the screen-spec **Feature** backlink resolves to feature-list.md. Pre-migration
// docs (no column / no backlink) emit WARN `link.pre_migration` and exit 0 — never block
// a build. A NON-zero exit means real drift (a code that does not resolve) → fix before
// promote.
bash: .claude/skills/.venv/bin/python3 \
...
// technical-spec.md/behavior-logic.md bare ROUTE### citations (never braced) resolve to
// route-list.md's Code column, route-list.md's Owner F### cell(s) resolve to feature-list.md, and
...
// v26.0.0 — A3 Source Walkthrough validator (WARN-capable, NON-halting). Covers
// screen-spec spec.md's A3 ONLY as of v27.8.0 — B4 (DB Impact per Event) and A3's
// technical-spec.md side both retired that release (see ADR-0007); this invocation no
// longer walks features/*/technical-spec.md at all. Mirrors validate_feature_screen_link.py's
// FS.2-only wiring shape, including its scope limitation (a --screen-specs-only run without
// ever running --feature-specs never exercises this gate; same as the v24 precedent). Absent
// section on an un-migrated screen doc emits WARN `reading_guide.pre_migration` and exits 0 —
// never blocks a build. A non-zero exit means a present-but-malformed section → fix before
// promote.
bash: .claude/skills/.venv/bin/python3 \
...
Promoted: docs/features/{F###}/ (technical-spec, functional-spec) for ${resolvedTargetFcodes.length} feature(s)
...
Test cases (optional, off by default): /tkm:rebuild-spec --test-cases  # QA-facing docs/features/{F###}/test-cases.md — opt-in pass, never runs automatically
Migrating an existing v26 repo (4-file feature dirs)? See references/migration-audience-split.md for migrate_feature_audience_split.py's modes A/B/C + LLM compaction contract.
...
## Pass-level edge cases

<!-- Moved from SKILL.md § Edge Cases — meaning unchanged, only its home. Legacy plan layout is authored here for the first time. -->
<!-- layout-exempt: pass edge cases — docs/ and plan paths are this pass's own targets -->
...
|------|----------|-------|--------|
| FS.1 | `researcher` (1/feature in chained waves of ≤5, or 5/batch sequential) | scoped artifact sections (Grep per F###) + `feature-spec-researcher-contract.md` + 2 audience templates + canonical upstreams | `plans/<active>/artifacts/features/{slug}/{technical-spec,functional-spec}.md` (2 files) + `TaskUpdate(status=completed)` |
