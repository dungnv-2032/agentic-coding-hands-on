...
4. **Capability intent** — not a feature-level check; see `code-formats.md § Capability-Level Intent (authority)` for CAP-count judgment.
...
6. **Vague naming**: F### name is too broad to scope a sprint
   ...
   - Scope: this check is about a name being too VAGUE, not about a name covering too MUCH. A name that pairs a generic noun with a specific qualifier passes here even when it declares two outcomes (`A & B`) — that shape belongs to check #8's multi-outcome enumeration, which is `critical`. Do not report it under #6 and do not treat #6 passing as clearance for #8.
   - → warning
...
8. **Grouping coherence**: do F-code assignments reflect logical feature boundaries?
   ...
   - Criteria: `code-formats.md § Feature Clustering Rule (authority)` — apply that rule; do not substitute one of your own, and cite the bullet a finding rests on
   - **Multi-outcome names — enumerate, do not skim.** List EVERY F### whose name declares more than one outcome (`A & B`, `A and B`, comma-joined) and give each a one-line verdict: the single outcome that explains all its US, or SPLIT. A name-shaped scan is cheap and the names are the author's own signal, so an unenumerated one reads as unexamined. This directs attention only — the conjunction is NOT itself a finding (see the multi-outcome-name bullet in the authority, and its UNMEASURED marking); a name with no conjunction gets no free pass either, since the axis of grouping decides, not the wording.
...
---
\`\`\`
Severity: Group A checks (1-3) and check 8 are critical (issues count); Group B checks 5-7 are warnings. Halt threshold: passed=false only when issues > 0.
