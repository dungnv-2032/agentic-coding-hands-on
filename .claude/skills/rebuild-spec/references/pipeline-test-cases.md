...
`functional-spec.md` (the merged Edge Cases / Screens / User Journey source for UAT) — an
EXPAND-DETAILS pass, not a new detection surface: it **re-projects** facts already stated and
already validated in the feature specs, and adds zero re-detection of its own — a test case whose
behaviour is not already cited there does not belong in this artifact. Output is a 3rd per-feature file, `docs/features/{slug}/test-cases.md`,
**SIDECAR** — never joins the hard-gated `FEATURE_FILES` tuple or the promotion gate (see
`references/feature-spec-researcher-contract.md` § Sidecar Files).
...
ABORT). `functional-spec.md` is promoted alongside `technical-spec.md` by the same FS.7 promote
unit, so its presence is implied — not re-checked independently.
...
// Incremental cursor check — pass-level: ONE cursor for the whole pass (NOT FS.1's per-fcode reverse-index;
...
2. docs/features/${fcode}/functional-spec.md § 8 Edge Cases — already test-case-shaped
   negative-path rows → UT/IT
3. docs/features/${fcode}/functional-spec.md § 5 Screens + § User Journey — What User Sees /
   What User Can Do → UAT scenarios
...
NOT touch the other files already in that folder). Zero derivable test cases for this feature
...
// validator runs — marker-check style, applied per-feature: the validator refuses to run on a
// partial fan-out, so a crashed researcher HALTS loudly instead of yielding a silently short set.
...
functional-spec.md section, not a smuggled code-only citation).
...
`test-cases.md` files only. Max 3 cycles — mirrors FS.6.
...
other files are not re-drafted this pass), an incremental `--scope features` promote for the
affected fcodes copies test-cases.md into `docs/features/{fcode}/` alongside the pre-existing
...
// [27.11.0 W6] This pass changes TC### membership (features/*/test-cases.md), which is a traceability-matrix
// COLUMN — refresh the matrix BEFORE the nav call below, so the Document Map's trace pointer
// and the matrix itself agree with what this pass just promoted.
bash: .claude/skills/.venv/bin/python3 \
  claude/skills/rebuild-spec/scripts/build_traceability_matrix.py --docs-root ${docs_root}

// [27.11.0 / FR-6 RT-3] Refresh the README index + `## Document Map` crosswalk so this pass's
// artifact (features/*/test-cases.md) is listed. WITHOUT this, the crosswalk omits an artifact the
// corpus actually has — which reads as authoritative ABSENCE, strictly worse than no crosswalk.
// Same invocation as W9.6 / FS.7 / SS.3; idempotent, preserves hand-written README content.
...
per-fcode reconcile shape, not a single-artifact pass's one-path shape).

## Test-Cases Pass — Subagent contracts

| Wave | Subagent | Input | Output |
|------|----------|-------|--------|
| TC.1 | `researcher` (1/feature in chained waves of ≤5, or 5/batch sequential) | `docs/features/{fcode}/{technical-spec,functional-spec}.md` + `test-cases-researcher-contract.md` + `test-cases-template.md` | `plans/<active>/artifacts/features/{fcode}/test-cases.md` + `TaskUpdate(status=completed)` |
