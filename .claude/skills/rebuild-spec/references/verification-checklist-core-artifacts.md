...
**Required sections:** per `architecture-template.md` — `# Architecture` → `## System Architecture` (Mermaid graph) → tech-stack listing → `## Deployment View` (v27.13.0 — honesty label + Mermaid `flowchart` with cited nodes/edges, OR the `N/A — no infrastructure-as-code found in repository.` degradation line; see `deployment-source-patterns.md`).

**Format checks:** at least one Mermaid diagram present and syntactically valid; tech stack entries match the codebase manifests; `validate_deployment_view.py` (WARN-first — see below).

**Critical edge cases:**
- Missing Mermaid diagram (System Architecture) → critical
- Technology documented but not in codebase → critical
- Technology in codebase but not documented → critical
- Mermaid syntax invalid → warning

**Deployment View — WARN-only, never critical (`validate_deployment_view.py`):**
- `## Deployment View` section absent entirely → warning (legacy corpora / a stack with no `deployment_sources` — no forced migration)
- Honesty label missing/altered immediately under the H2 → warning
- Node or edge with no `file:line` citation → warning (per FR-7 it should have been omitted, not rendered uncited)
- Two or more Mermaid diagrams inside the section → warning (infra + network must be ONE combined diagram, not split)
- A leaked credential value anywhere in the rendered section → **critical** (the one hard gate this validator carries — a credential that reaches a promoted doc cannot be walked back, and `--package` later hands `docs/` to an external client)
...
**Valid feature criteria (all 3):** Clear Flow (input→process→output), Independently Testable, Agent Implementable — capability-count judgment is NOT a feature criterion, see `code-formats.md § Capability-Level Intent (authority)`. Same name+different F### or >50% keyword overlap = critical.
...
- Feature with no clear flow → critical
