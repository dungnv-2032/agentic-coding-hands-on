...
**Check 2 — DISC-### scope** (this gate scopes `data-model.md` ONLY, at **critical**. The per-feature `technical-spec.md § 4.2` equivalent is `FeatureSpec.disc_boolean` in `validate_feature_spec.py`, at **warning** — two deliberately different checks on two different artifacts, not a severity drift)**:**
...
Schema: references/code-formats.md (canonical 10 BL types + Source File/Source Symbol field schema).`,
  ...
  if (needsW2a1_combined) {
    ...
    // W2a.1 sibling gate — screen-flow is written by the SAME task as screen-list.
    // Wired 2026-08-25: verification-checklist-core-artifacts.md already labelled this
    // validator "(pre-W7a)" — i.e. it told the W7a reviewer to SKIP its rule_ids as
    // already-checked — while no step ran it. That is worse than an unwired validator:
    // it suppressed the human check meant to compensate. Same exit contract as its
    // sibling gate: 0 → PASS/WARN proceed · 1 → FAIL, HALT · 2 → internal error, halt.
    bash: .claude/skills/.venv/bin/python3 \
      claude/skills/rebuild-spec/scripts/validate_screen_flow.py \
      --plan-dir plans/<active-plan> \
      --summary-out plans/<active-plan>/artifacts/validation/validation-summary.json
  } else {
...
Generate BehaviorLogic. Template: behavior-logic-template.md. Schema: references/code-formats.md (canonical 10 BL types + Source File/Source Symbol field schema). Context: screen-list.md (service-call references extracted by Wave 2a). CARDINALITY CONTRACT (read template § Cardinality Contract before writing any BL item): (1) Read scout-report.md § Background Logic Source Inventory — this is the authoritative file list; (2) For each inventory entry emit exactly 1 BL item (Mode A: 1 file = 1 BL; Mode B: 1 decorator hit = 1 BL; multiple hits in same file = multiple BL items); (2a) Sentinel handling — entries shaped \`- {category}: _(none found)_\` (value field is the sentinel) are scout markers for empty categories; SKIP them, never emit a BL; (3) Set Source File = inventory entry path and Source Symbol = class or method name (single symbol only — see template Rule C2 for forbidden multi-symbol delimiters); (4) Aggregation is a critical violation — do NOT combine multiple source files into one BL item; (5) For per-stack signal context (what counts as each BL type per stack) read references/bl-source-patterns.md. Draft: plans/<active-plan>/artifacts/behavior-logic.md`,
  ...
  // W2b behavior-logic structural gate (produce path).
  // Wired 2026-08-25: verification-checklist-core-artifacts.md already labelled this
  // validator "(pre-W7a)" — i.e. it told the W7a reviewer to SKIP its rule_ids as
  // already-checked — while no step ran it. That is worse than an unwired validator:
  // it suppressed the human check meant to compensate. Same exit contract as its
  // sibling gate: 0 → PASS/WARN proceed · 1 → FAIL, HALT · 2 → internal error, halt.
  bash: .claude/skills/.venv/bin/python3 \
    claude/skills/rebuild-spec/scripts/validate_behavior_logic.py \
    --plan-dir plans/<active-plan> \
    --summary-out plans/<active-plan>/artifacts/validation/validation-summary.json
...
Generate BehaviorLogic (no screen-list upstream — read scout-report.md § Background Logic Source Inventory directly). Template: behavior-logic-template.md. Schema: references/code-formats.md. Draft: plans/<active-plan>/artifacts/behavior-logic.md`,
  ...
  // W2b behavior-logic structural gate (non-web profile path — behavior-logic is universal).
  // Wired 2026-08-25 alongside the produce-path gate: the checklist labelled this validator
  // "(pre-W7a)" while nothing ran it, which suppressed the W7a reviewer's own check.
  // 0 → PASS/WARN proceed · 1 → FAIL, HALT · 2 → internal error, halt.
  bash: .claude/skills/.venv/bin/python3 \
    claude/skills/rebuild-spec/scripts/validate_behavior_logic.py \
    --plan-dir plans/<active-plan> \
    --summary-out plans/<active-plan>/artifacts/validation/validation-summary.json
...
Template: templates/crud-matrix-template.md. Then the orchestrator runs validate_crud_matrix.py (exit 1 → HALT).`,
    addBlockedBy: [scoutTaskId] })
  // Gate (validate_crud_matrix.py): promoted from a comment to a real invocation, 2026-08-25.
  // It was born as a comment in v11.1.0 (4b49cf0d) and stayed one — 2 of 152 gate mentions
  // in the pipeline files used this form, and the command in the comment was not even
  // runnable (no venv interpreter, no full path). Same exit contract as W1.1's route-list
  // gate: 0 → PASS/WARN proceed · 1 → FAIL, HALT · 2 → internal error, halt.
  bash: .claude/skills/.venv/bin/python3 \
    claude/skills/rebuild-spec/scripts/validate_crud_matrix.py \
    --plan-dir plans/<active-plan> \
    --summary-out plans/<active-plan>/artifacts/validation/crud-matrix-validation-summary.json
...
Template: templates/db-object-catalog-template.md. Then the orchestrator runs validate_db_catalog.py (exit 1 → HALT).`,
  ...
  // Gate (validate_db_catalog.py): promoted from a comment to a real invocation, 2026-08-25.
  // It was born as a comment in v11.1.0 (4b49cf0d) and stayed one — 2 of 152 gate mentions
  // in the pipeline files used this form, and the command in the comment was not even
  // runnable (no venv interpreter, no full path). Same exit contract as W1.1's route-list
  // gate: 0 → PASS/WARN proceed · 1 → FAIL, HALT · 2 → internal error, halt.
  bash: .claude/skills/.venv/bin/python3 \
    claude/skills/rebuild-spec/scripts/validate_db_catalog.py \
    --plan-dir plans/<active-plan> \
    --summary-out plans/<active-plan>/artifacts/validation/db-catalog-validation-summary.json
...
### Wave 3 — Permissions (matrix · curated)
...
DOUBLE OUTPUT (order matters — write matrix FIRST, then derive curated from it):
...
<!-- v27.0.0 (Phase 03): the former 3rd output, business-rules.md (DRAFT) — a plain-language
     summary of behavior-logic BR codes — is retired. behavior-logic.md itself (Wave 2b) is now
     BA-first (## Behavior Logic Index) + Dev Appendix in ONE file, so no separate plain-language
...
1. SHELL STEP = GROUPING ONCE (global, single agent sees everything):
   ...
   - CLUSTERING RULE (authority, do not restate): references/code-formats.md § Feature Clustering Rule (authority) — read it before this pass.
...
Template: feature-list-template.md. Schema: references/code-formats.md. Draft: plans/<active-plan>/artifacts/feature-list.md`,
  ...
  var featureListTaskId = resolveWaveTaskId("Wave5: feature-list", () =>
    TaskCreate({ subject: "Wave5: feature-list",
      description: `Session context: read \`plans/<active-plan>/artifacts/_session-context.md\` FIRST.
Generate FeatureList from all prior drafts. Template: feature-list-template.md. Schema: references/code-formats.md. Draft: plans/<active-plan>/artifacts/feature-list.md.
CLUSTERING RULE (authority, do not restate): references/code-formats.md § Feature Clustering Rule (authority) — read it before assigning any US to an F###.
