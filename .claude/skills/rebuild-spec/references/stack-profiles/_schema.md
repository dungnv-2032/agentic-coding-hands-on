...
| `deployment_sources` | string[] | no | **[v27.13.0]** Basename/content patterns (from `references/deployment-source-patterns.md`'s table — e.g. `"docker-compose*.y*ml"`, `"Dockerfile*"`, the content-sniffed pseudo-pattern `"k8s-manifest"`) that are meaningful IaC sources for this stack's `## Deployment View` section in `architecture.md`. Consumed by the system researcher, never by pipeline dispatch logic (no conditional-production gate like `screen_source` — Deployment View is WARN-first, never a hard skip/produce switch). **Absent or `[]` → the section degrades to the FR-5 `N/A — no infrastructure-as-code found in repository.` line for that stack.** `web-js-ts`/`generic-source` carry the full pattern set; `cobol`/`delphi-vcl`/`oracle-plsql` carry `[]` — these legacy/desktop/DB stacks legitimately ship no IaC and MUST NOT have fabricated patterns invented here to look complete (see "generic-source fallback" below for the same fail-closed posture applied to a different field). |
...
`api-map`, `permissions`, `permissions-matrix`, `user-stories`, `feature-list`,
