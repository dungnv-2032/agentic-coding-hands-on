...
**v27.0.0 → human-readable SOT reshape (`plans/260818-1332-rebuild-spec-human-readable-sot`):**
`templates/screen-spec-template.md` and `references/screen-spec-researcher-contract.md` now
generate each screen spec directly in the 10-numbered-§ + `## Technical Appendix` (6 H2-sibling)
SOT shape (`## 1. Overview` … `## 10. Responsive Behavior`, then `## Implementation Mapping`
through `## Source Walkthrough`) — the same target order `compose_screen_sot` composes an
existing corpus into via the `--migrate --only screen-sot` step (`references/pipeline-migrate.md`).
`### Layout Sketch` lives in `## 2. Screen Layout`; `### Layout Regions` moved to the appendix.
This pass's dispatch, batching, and promotion logic below is unaffected — researchers and
reviewers read section order from the template/contract files linked in each task description,
not from this file.
