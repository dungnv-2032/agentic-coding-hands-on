...
|-------|------|-----------|
| `schema_version` | string | State-file schema version (semver), kept in sync with `_stack_profile_lib.SCHEMA_VERSION`. On resume, if absent or numerically less than `_stack_profile_lib.SCHEMA_VERSION` (currently `"22.1.0"`; compare numerically, not lexically — `"9.0.0" > "22.1.0"` lexically), the run reports the state as stale (**advisory**) and continues; Preflight step 2 re-resolves the profile from scratch on every run regardless, and no stack profile is persisted in this file for the gate to invalidate — see `SKILL.md` § Preflight 2.2 for the scope statement; `probe_gate` is preserved verbatim — it is entirely prose-managed (no Python script reads it) and carries no dependency on any profile schema field. General rule: a state stamped below the current profile schema is invalidated, whatever the reason for that schema's bump. History, for context only: a pre-v11.0.0 state could not carry a `profile_id`/encoding at all; a v11.0.0–v20.x state predated the `screen_source` profile field (added at v21.0.0) and would otherwise resume carrying a profile that fail-closes screens to `"none"`. Written on every state write at the current skill version. Absent key = pre-profile legacy state. |
