...
## Invariants — every pass, every flag

<!-- Moved from SKILL.md § Edge Cases — meaning unchanged, only its home. -->
<!-- layout-exempt: relocated invariants — docs/ paths are rebuild-spec's own targets -->

These hold on **every** invocation — the core pass, every standalone pass, and every flag combination. Nothing here is core-only.
...
**Dispatch and fallback rules (all five, plus OOB-edit semantics):**
...
- When `--package` flag is set → `pipeline-package.md` (EXPORT-tier, deterministic build script — not a wave chain; like `--overview`/`--api-doc`, stateless and re-runnable)
...
const coreSystemDocs = [
  ...
  // v27.0.0: system/business-rules.md retired — its content merged into
  // generated/behavior-logic.md (already listed in coreGeneratedDocs below), no replacement entry needed here.
]
...
| Feature specs | `plans/<active-plan>/artifacts/features/<F###>/{technical-spec,functional-spec}.md` | `docs/features/<F###>/{...}.md` | **(feature-specs pass)** |
