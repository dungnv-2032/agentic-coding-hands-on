...
- inline occurrences of provisional codes in the body of `technical-spec.md`, `functional-spec.md`
  (2-file set) and any `phase-XX-*.md` body prose
...
`spec-authoring-contract.md` § Forward-Authored System Docs). These are **SINGLE files, NOT 2-file
...
file and runs the REDUCED registration subset below. The other feature file (`functional-spec.md`)
does not exist yet.
...
| 7 (2-file existence check) | SKIP — partial set is the sanctioned fast-mode state |
...
run removes the marker when it completes the 2-file set.
