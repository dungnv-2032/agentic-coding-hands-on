...
<!-- EXPECTED companion (WARN if absent, never blocks a pass — v26.2.0). The state diagram above
     answers "what states, what triggers"; this sequence diagram answers "who calls whom" for the
     SAME triggers: actor -> entry point -> system -> DB hops. Participants: actor from
     screens/behavior (who initiates), entry point from source (controller/service/job), DB from
     the subject entity's table. Label each message with the Transitions-table trigger it maps to
     (T1, T2, ...). Every arrow implying a call MUST be source-cited (reuse the matching
     Transitions row's Source cell) or marked "derived from Transitions table above" when it just
     restates an already-cited transition — never fabricate a hop. -->

```mermaid
sequenceDiagram
    actor {Actor}
    participant {EntryPoint} as {Controller/Service}
    participant DB as {Entity} table
    {Actor}->>{EntryPoint}: {trigger label, e.g. T1}
    {EntryPoint}->>DB: {state mutation}
    DB-->>{EntryPoint}: {result}
```
