...
the reset boundary; NOT file-global like `F###`).

**Citation-source split**: `UT`/`IT` rows cite a `BR-###`/`SM-###`/`DEC-###`/`DISC-###` code, a
`` `file:line` ``, or a `functional-spec.md` § 8 Edge Cases row. `UAT` rows cite a
`functional-spec.md` § 5 Screens / User Journey section — NEVER a bare code (UAT is less
code-traceable by design; see `references/test-cases-researcher-contract.md`).
...
| {TC003} | {UAT} | {user starting point} | {user action} | {what the user sees/can do next} | {functional-spec.md § User Journey step N} |
