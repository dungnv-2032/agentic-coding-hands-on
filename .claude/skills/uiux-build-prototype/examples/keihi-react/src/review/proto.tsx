  ...
  return (
    <>
      ...
      <div className="mb-3.5 flex flex-wrap items-center gap-1.5">
        {spec.screens.map((s) => (
          <Pill key={s.id} active={s.id === id} onClick={() => goProto(s.id)}>{s.name}</Pill>
        ))}
      ...
      {/* 遷移元 → 現在 → 遷移先 */}
      <div className="mb-3.5 rounded border border-[#ECECEF] bg-white px-4 py-2.5 text-xs shadow-[0_1px_6px_rgba(0,0,0,.06)]">
        <span className="text-[10px] text-zinc-400">遷移元 </span>
        {transitionsTo(id).length
          ? transitionsTo(id).map((t, i) => (
              <Chip key={i} onClick={() => goProto(t.from)}>
                {(t.condition ? `${t.condition}→ ` : "") + (screensById[t.from]?.name ?? t.from)}
              </Chip>
            ))
          : <Chip>—</Chip>}
        <b className="mx-1.5">→</b>
        <Chip active>{meta.name}</Chip>
        <b className="mx-1.5">→</b>
        <span className="text-[10px] text-zinc-400">遷移先 </span>
        {transitionsFrom(id).length
          ? transitionsFrom(id).map((t, i) => (
              <Chip key={i} onClick={() => goProto(t.to)}>
                {(screensById[t.to]?.name ?? t.to) + (t.condition ? `（${t.condition}）` : "")}
              </Chip>
            ))
          : <Chip>—</Chip>}
      </div>
