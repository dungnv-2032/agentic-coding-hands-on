...
- **kensan** (`extras` kit) → cover · TL;DR · 🔥 hot topics (table) · per-topic cards · deep-dive sections
  (how-it-works, evidence, community pulse, what-to-learn). Render hot-topic heat and scores as mono labels.
  This anchor stays here rather than moving with the skill: `kensan` reads this file cross-kit, and the anchors
  are one list so the presets stay visually consistent.
