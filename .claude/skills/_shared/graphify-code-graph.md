...
**`affected` on doc-derived edges needs `--relation`.** Its default relation set is
code-oriented (`calls`, `imports`, `inherits`, `implements`, …), so edges extracted from
documents — `shares_data_with`, `conceptually_related_to`, `cites`, `rationale_for` — are
skipped and the command prints `No affected nodes found`. That is not evidence of no
impact. Pass the relations explicitly when the graph's doc layer matters:

```bash
graphify affected "<node>" --depth 2 --relation shares_data_with --relation references --relation calls
```
...
- **Docs** (`.md`, `.mdx`, `.rst`, `.txt`, `.pdf`) are NOT refreshed automatically. They need
  a semantic re-extraction that costs LLM, and **nothing in the kit runs it on its own** —
  `graphify update .` is code-only by design, so neither the hook nor `rebuild-spec`'s
  preflight adds doc edges. Only `tkm:index-docs` does, and only when a user triggers it.
  The SessionStart hook merely **nudges** when docs changed since `built_at_commit`; it never
  spends tokens.

So treat code edges as current, but treat doc edges as absent unless `tkm:index-docs` has
been run — and verify doc-derived edges against source when freshness matters (see Use
Rule 4). A graph built only by the hook contains file and heading nodes for documents, no
entities extracted from their prose. Disable everything via config `graphify.enabled=false` (CLI /
