...
`docs/` has 5 namespaces: `system/` (curated narratives), `flows/` (AI-drafted cross-feature journeys; user owns post-generation), `features/{slug}/` (**2** audience-aware files per feature — `functional-spec.md` BA/QA · `technical-spec.md` dev; v27.0.0, down from 4), `generated/` (raw inventories, free regen), `decisions/` (human-only ADRs). `manage-docs` owns top-level narrative files (`project-roadmap.md`, `code-standards.md`, etc.).
...
| Feature tech-spec | `docs/features/{slug}/technical-spec.md` | rebuild-spec | Per feature; dev/QA/SA audience |
| Feature functional-spec | `docs/features/{slug}/functional-spec.md` | rebuild-spec | Per feature; BA/QA audience. v27.0.0 — replaces the former `business-context.md`+`screens.md`+`edge-cases.md` trio (10 H2 sections: Overview·Open Decisions·Requirements·Business Rules·Screens·User Stories·Scenarios·Edge Cases·Edge Behaviours to Verify·Configuration) |
| Flows (cross-feature) | `docs/flows/{slug}.md` | rebuild-spec | AI draft; user may rename |
| Feature test-cases | `docs/features/{slug}/test-cases.md` | rebuild-spec | Per feature, SIDECAR (opt-in `--test-cases`); UT/IT/UAT derived from the already-cited technical-spec.md/functional-spec.md — NOT in the mandatory 2-tuple, never gates promotion |
| Route inventory (raw) | `docs/generated/route-list.md` | rebuild-spec | Free regen |
| API map | `docs/generated/api-map.md` | rebuild-spec | Routes + background entrypoints |
...
| Behavior logic (curated + inventory) | `docs/generated/behavior-logic.md` | rebuild-spec | BL### inventory. v27.0.0 — absorbs the former `docs/system/business-rules.md` curated per-rule content (business-rules.md is DELETED) |
...
| Product profile (end-user metadata) | docs/product-profile.md | generate-llms-txt | Human-owned; interview-seeded; never regenerated over |
| Traceability matrix | `docs/generated/traceability-matrix.md` | rebuild-spec | Free regen; F###-keyed re-projection of SCR###/US###/BL###/ROUTE###/PERM### (+ opt-in TC###) already stated elsewhere — invents nothing (v27.11.0, W6) |
...
**Disambiguation (behavior-logic vs architecture, v27.0.0):** `docs/system/business-rules.md` is **DELETED** (v27.0.0) — its per-rule As-Is/To-Be behavior is absorbed into `docs/generated/behavior-logic.md`, which now owns that content; `docs/system/architecture.md` owns structure/stack description.
...
| `docs/features/*/technical-spec.md` | YES | BR/SM/ALG/INT/DEC table edits |
| `docs/features/*/functional-spec.md` | YES — **per-section**, not per-file (v27.0.0, D-d) | Authoritative per-section table lives in `claude/agents/doc-writer.md` § Functional-Spec Section Guardrails — do not restate it here (default-deny: a section not listed there as human-editable is machine-owned) |
...
PR `2026-07-07` (v26.1.0 — B6 `--test-cases` standalone pass). ADDS one output path per feature
...
PR `2026-08-18` (generate-llms-txt v2 phase 1 — new `docs/product-profile.md` end-user metadata
config: name + meaning, URL(s), department, owner, support contact, summary, surfaces, optional
`audience_default`). ADDS one new topic row under `docs/`; no existing consumer's resolved read/
write path changes — the file is human-owned, interview-seeded, and never regenerated over. Per
the proportionate-bump rule this is **minor** for `generate-llms-txt` (owner gains a new topic)
and **no bump** for any existing consumer:

| Skill / agent | From | To |
|---|---|---|
| `generate-llms-txt` | 1.0.0 | pending minor bump (1.1.0) — applied when SKILL.md is next edited (phase 3/5) |
| `rebuild-spec` | — | unchanged |
| `takumi` | — | unchanged |
| `manage-docs` | — | unchanged |
| `doc-writer` (agent) | — | unchanged |

PR `2026-08-25` (rebuild-spec 27.11.0 — W5 crosswalk `## Document Map` in the existing
`docs/README.md` generated zone + W6 `docs/generated/traceability-matrix.md`). Both ADD to
existing output (a new section in an already-generated file; one new file under the
already-translation-covered `generated/` namespace); no existing consumer's resolved
read/write path changes. Per the proportionate-bump rule this is **minor** for rebuild-spec
(owner gains a new section + one new output) and **no bump** elsewhere:

| Skill / agent | From | To |
|---|---|---|
| `rebuild-spec` | 27.10.0 | 27.11.0 |
| `takumi` | — | unchanged |
| `manage-docs` | — | unchanged |
| `doc-writer` (agent) | — | unchanged |
