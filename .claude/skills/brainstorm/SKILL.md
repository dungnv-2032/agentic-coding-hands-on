...
`--level` also selects a **capability tier** when the consultation runs as the `brainstormer` agent: the
caller spawns `brainstormer-lite` (`low`), `brainstormer` (`medium`/`high`), or `brainstormer-max` (`max`)
per the "Model & Effort per Level" table in `_shared/processing-levels.md`. Run inline from this skill, the
consultation keeps the session's own model; the depth rows above still apply either way.
