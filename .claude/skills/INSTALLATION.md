...
Most skills use only the Python standard library. Skills that need external packages ship their own `scripts/requirements.txt` (e.g. `devops`, `pack-codebase`, document-processing skills — plus `audio-transcribe` and `translate-file` from the `extras` kit, which this installer discovers automatically). Common packages across these:
