...
one is governed by [`migrate-aidd/scripts/detect_sdd_lib.py`](../../migrate-aidd/scripts/detect_sdd_lib.py) — `resolve_specs_root()`.
...
1a. **Resolve `specsRoot`** (Specs layer can be one of two layouts). Pick the FIRST that exists with
   ...
   [`migrate-aidd/scripts/detect_sdd_lib.py`](../../migrate-aidd/scripts/detect_sdd_lib.py)
   `resolve_specs_root()` (single source of truth; do not re-derive the logic):
