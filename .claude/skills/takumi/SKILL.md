...
argument-hint: "[task|plan-path] [--interactive|--fast|--parallel|--auto|--no-test] [--e2e-test-first]"
metadata:
...
**MoMorph test policy** is independent of discipline:
- `visual-contract`: static/presentational mapping, including hover/focus/responsive-only work; never claim RED/TDD
- `e2e-red-first`: auto-selected for form validation/navigation/modal/state-transition behavior, or forced by `--e2e-test-first`; a tester-owned executable screen test must produce a valid assertion RED before UI/backend implementation

Any `e2e-red-first` selection and `--no-test` are incompatible. A missing web E2E runner
is blocking; never auto-install, scaffold, or silently downgrade. Strict
E2E is web-only; mobile is BLOCKED as unsupported in this version. See
`.claude/rules/momorph/momorph-development.md` for the full contract.

**Examples:**
```
/tkm:takumi "Add user authentication to the app" --fast
/tkm:takumi path/to/plan.md --auto
/tkm:takumi "Implement this MoMorph screen" --e2e-test-first
```
...
flowchart TD
    ...
    MM -->|Yes| MG["Clarify + select test policy + optional tester RED"]
    MM -->|No| F[Load Blueprint]
    MG --> PA["Spawn MoMorph UI agents + forge RED-first backend"]
    PA --> TV["Tester GREEN + visual validation"]
    TV --> INT["Integrate verified UI + backend"]
    INT --> I
    D --> Sf[Spec — minimal]
    Sf --> MM
    F --> G[Rest Point]
...
- **Delivery (MANDATORY — never skip, never reorder):**
  ...
  1. `delivery-tracker` subagent → sync all completed stages back to `phase-XX-*.md` and `plan.md`
  ...
  **Delivery Manifest — emit verbatim to the user before the commit prompt:**
  ...
  - [x] delivery-tracker — plan.md & phases reconciled
...
| "The plan is already in sync — skip `delivery-tracker`." | Stale checkboxes from earlier stages are the most common drift. Spawn it. |
...
| **MoMorph UI** | **`momorph-ui-implementer` (background, 1 per screen)** | **After clarification and selected-policy gate; never use generic `implementer`** |
| Tempering | `tester`, `debugger` | **MUST** spawn |
| Inspection | `reviewer` | **MUST** spawn |
| Delivery | `delivery-tracker`, `doc-writer`, `git-manager` | **MUST** spawn all 3 |
