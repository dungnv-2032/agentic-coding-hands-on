...
Discipline and MoMorph test policy are separate decisions. The
`--e2e-test-first` flag never changes the selected discipline.

## MoMorph Test-policy Detection

Run this only when MoMorph context is detected:

```
FUNCTION resolveMomorphTestPolicy(input, activePhase, resolvedDesignContext):
  IF input contains "--e2e-test-first":
    policy = "e2e-red-first"
  ELSE IF activePhase.test_policy exists:
    IF value is "visual-contract" OR "e2e-red-first":
      policy = value
    ELSE:
      STOP — invalid persisted policy
  ELSE IF resolvedDesignContext contains form validation, navigation,
          modal behavior, or another state transition:
    policy = "e2e-red-first"
  ELSE:
    policy = "visual-contract"
  IF input selects "--no-test" AND policy is "e2e-red-first":
    STOP — strict policy and no-test are incompatible
  RETURN policy
```

Run this resolution after reading the downloaded specs/test cases and completing
clarification. Hover, focus, pressed, and responsive-only requirements remain
`visual-contract`; they do not trigger strict E2E. `visual-contract` never claims
RED/TDD. `e2e-red-first` is a
pre-implementation gate, not Stage 4 tempering: the exact project runner must
produce an assertion-caused RED before either MoMorph track begins. Never infer
a runner from Playwright MCP, install one, or silently fall back. Strict E2E is
web-only; selecting it for mobile is BLOCKED as unsupported.
...
"/tkm:takumi implement MoMorph screen --e2e-test-first"
→ Discipline: interactive; MoMorph test policy: e2e-red-first
```
