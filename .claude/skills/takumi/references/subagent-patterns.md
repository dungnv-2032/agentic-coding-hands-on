...
Rules:
  - Split by the rebuild-spec criteria in `code-formats.md § Valid Feature Requirements`:
    Clear Flow, Independently Testable, Agent Implementable. A feature MAY span several
    capabilities — do NOT force one intent per feature. Capability-level intent is governed by
    `code-formats.md § Capability-Level Intent (authority)`.
...
Run the GREENFIELD subset:
  - KEEP: Check 4 F-code uniqueness (needs only the feature-list itself),
  ...
  - NOT A GATE: Check 5 is a pointer slot, not an evaluated check. Do NOT fail a feature for spanning
    several capabilities — capability-intent judgment lives in
    `claude/skills/rebuild-spec/references/code-formats.md § Capability-Level Intent (authority)`.
...
Per-file FORMAT templates — match each file's H2 structure EXACTLY (greenfield deltas in the contract):
  ...
  - functional-spec.md → claude/skills/rebuild-spec/templates/functional-spec-template.md
...
Output paths (write directly to the plan dir — NEVER docs/):
  ...
  - plans/<plan_dir>/spec/<slug>/functional-spec.md
...
Rules:
  ...
  - H2 STRUCTURE — each file MUST match its template's required H2 set and order. Specifically:
      ...
      • functional-spec.md: `## 1. Overview` → `## 2. Open Decisions` → `## 3. Requirements` →
        `## 4. Business Rules` → `## 5. Screens` → `## 6. User Stories` → `## 7. Scenarios` →
        `## 8. Edge Cases` → `## 9. Edge Behaviours to Verify` → `## 10. Configuration` (10 numbered
        H2 sections, greenfield deltas per the contract). `## 5. Screens` carries the `## Screen List`
        table (NOT "Screen Route Table") + `### User Journey` — the former `screens.md` content, folded in.
...
## Plan Sync-back (Delivery Tracker)

```
Task(subagent_type="delivery-tracker", prompt="Run full sync-back in [plan-path]: reconcile completed tasks with all phase files, backfill stale completed checkboxes across all phases, update plan.md status/progress, and report unresolved mappings.", description="Sync plan")
```
