...
A feature draft is a DIRECTORY of **two files** (v27.0.0 audience split — down from four), never a
single `spec.md`:

```
plans/<plan_dir>/spec/<slug>/technical-spec.md     # FR/BR/SM/ALG/INT full detail, key entities, dev-facing user stories
plans/<plan_dir>/spec/<slug>/functional-spec.md    # BA/QA: overview, open decisions, one-line FR/BR/SM/DEC, screens, user stories, scenarios, edge cases, configuration
```
...
1. ❌ Authoring a single bespoke `spec.md` instead of the 2-file set. (`spec.md` is the *screen* shape — `spec/<slug>/screens/SCR-<name>/spec.md` — never the feature shape.)
...
to confirm/merge/split — **BLOCKING even in `--auto`**. Then fans out one 2-file set per confirmed
...
creates the 2-file set with correct frontmatter + H2 skeleton before the researcher spawns —
...
> "⚒ minimal spec only — run `/tkm:rebuild-spec --features F###` for the full 2-file spec."
...
**Trigger:** Any source listed in `.claude/rules/momorph/momorph-awareness.md`: the user input, active plan, or spawned-task prompt contains a MoMorph URL, explicit `fileKey`/`screenId`, or a MoMorph/Figma screen reference.

**When triggered, BEFORE entering Stage 2:**
1. Verify the extras kit provides both `momorph-implement-design` and `momorph-ui-implementer`. If either is absent, STOP and instruct the user to run `tkm init --kit extras`; never route MoMorph work to generic `implementer`.
2. Collect all screen identifiers from the request or active plan (may be 1 or N screens).
3. Fetch per screen in parallel: `get_frame(screenId)` + `download_specs(screen_id, "csv")` + `download_test_cases(screen_id, "csv")`
4. **Complete the Clarification Gate** in `momorph-development.md`: deep-read and cross-reference both CSVs, resolve every gap with the user, and write `clarifications.md`. Neither UI nor backend implementation may start before this completes.
5. Resolve `test_policy` deterministically after reading the design context: explicit `--e2e-test-first`, else active-plan value, else auto-select `e2e-red-first` for form validation/navigation/modal/state-transition behavior, else `visual-contract`. Hover/focus/pressed/responsive-only work remains `visual-contract`. Reject invalid values and any strict-policy + `--no-test` combination.
6. Preflight the selected policy:
   - `visual-contract`: no RED claim; continue after recording `redEvidence: not-applicable (visual-contract)`.
   - `e2e-red-first`: web needs an existing executable project E2E runner. Missing runner is BLOCKING — do not install, scaffold, or downgrade. Mobile strict E2E is unsupported and BLOCKING even if a runner exists. For supported web work, spawn `tester` to author/run the durable screen test and accept only assertion-caused RED evidence.
7. **Spawn one background `momorph-ui-implementer` PER SCREEN** by default. Its prompt carries the complete field contract from `momorph-development.md`, including the selected policy and read-only RED evidence. It uses the extras `momorph-implement-design` skill and owns presentational UI only. The orchestrator may partition independent sections into a bounded set of the same agent in `section` mode with disjoint files; section workers never spawn recursively, and Track A never uses generic `implementer`.
8. Once the gate (and strict RED) is satisfied, continue to Stage 2 → Stage 3 without waiting for UI agents. Generic `implementer` keeps RED-first ownership of backend/behavior work.
9. As each screen completes, spawn `tester` to rerun the same strict test GREEN when applicable and to own Playwright/simulator visual validation for both policies. Integrate only verified UI interfaces; no hard Track A/Track B merge point is required.
...
**Hard ordering rule (Stage 6 trap):** Both subagents in step 6.1 (`delivery-tracker` AND `doc-writer`) must be spawned BEFORE any commit-related `AskUserQuestion`. The commit prompt is NOT the seal — the Delivery Manifest is. If you find yourself about to ask "Ready to commit?" and have not yet spawned both, STOP and spawn them first. No exceptions for "tests passed and review approved" — that is precisely when the trap fires. Full anti-rationalization table: SKILL.md → "Delivery Anti-Rationalization".
...
1. **MUST** spawn these subagents in parallel:
   - `Task(subagent_type="delivery-tracker", prompt="Run full sync-back for [plan-path]: reconcile all completed Claude Tasks with all phase files, backfill stale completed checkboxes across every phase, then update plan.md frontmatter/table progress. Do NOT only mark current phase.", description="Update plan")`
...
2. Delivery-tracker sync-back MUST include:
...
6. **Emit the Delivery Manifest** to the user (see SKILL.md → "Delivery Manifest"). This is a printed emission, not a private check. All pre-commit boxes (gen-gate, delivery-tracker, orchestrator, doc-writer, TaskUpdate, evidence-gate) must read `[x]` from real subagent/gate verdicts. If any reads `[ ]`, complete that step NOW. The commit prompt is forbidden until the Manifest passes.
...
- (b) all 3 subagents spawned (`delivery-tracker`, `doc-writer`, `git-manager`) — `doc-writer` is spawned regardless of perceived doc impact; its verdict is what determines whether files change,
- (c) the evidence gate run at `--stage hard` with a real exit-0 SEALED,
- (d) Delivery Manifest emitted verbatim to the user with all pre-commit boxes (gen-gate, delivery-tracker, orchestrator, doc-writer, TaskUpdate, evidence-gate) = `[x]`,
...
- **MANDATORY DELEGATION:** Stages 4, 5, 6 MUST spawn subagents via Task tool. DO NOT perform directly.
  ...
  - Stage 5: `reviewer`
  - Stage 6: `delivery-tracker`, `doc-writer`, `git-manager`
