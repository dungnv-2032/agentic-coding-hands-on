...
Store logs for analysis in `<project>/.claude/automate-browser/logs/<session>/`:

```bash
# Create session directory
SESSION="$(date +%Y%m%d-%H%M%S)"
mkdir -p .claude/automate-browser/logs/$SESSION

# Capture and store
node "$SKILL_DIR/console.js" --url https://example.com --duration 10000 > .claude/automate-browser/logs/$SESSION/console.json
node "$SKILL_DIR/network.js" --url https://example.com > .claude/automate-browser/logs/$SESSION/network.json

# View errors
jq '.messages[] | select(.type=="error")' .claude/automate-browser/logs/$SESSION/console.json
```
