...
metadata:
  ...
  version: "1.1.0"
module: automation-browser
triggers: ["screenshot", "scrape", "automate browser", "Puppeteer", "web automation", "performance test"]
---
...
SKILL_DIR=""
if [ -d ".claude/skills/automate-browser/scripts" ]; then
  SKILL_DIR=".claude/skills/automate-browser/scripts"
elif [ -d "$HOME/.claude/skills/automate-browser/scripts" ]; then
  SKILL_DIR="$HOME/.claude/skills/automate-browser/scripts"
fi
...
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output ./.claude/automate-browser/snapshots/page.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
Store snapshots for analysis in `<project>/.claude/automate-browser/snapshots/`:

```bash
# Create snapshots directory
mkdir -p .claude/automate-browser/snapshots

# Capture and store with timestamp
SESSION="$(date +%Y%m%d-%H%M%S)"
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output .claude/automate-browser/snapshots/$SESSION.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
All in `.claude/skills/automate-browser/scripts/`:
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
For complex automation, write scripts to `<project>/.claude/automate-browser/tmp/`:

```bash
# Create tmp directory for test scripts
mkdir -p $SKILL_DIR/.claude/automate-browser/tmp

# Write a test script
cat > $SKILL_DIR/.claude/automate-browser/tmp/login-test.js << 'EOF'
...
# Run the test
node $SKILL_DIR/.claude/automate-browser/tmp/login-test.js
```
...
Store screenshots for analysis in `<project>/.claude/automate-browser/screenshots/`:

```bash
# Basic screenshot
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./.claude/automate-browser/screenshots/page.png

# Full page
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./.claude/automate-browser/screenshots/page.png --full-page true

# Specific element
node "$SKILL_DIR/screenshot.js" --url https://example.com --selector ".main-content" --output ./.claude/automate-browser/screenshots/element.png
```
...
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./.claude/automate-browser/screenshots/page.png --max-size 3

# Disable compression
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./.claude/automate-browser/screenshots/page.png --no-compress
```
...
Capture/filter console logs, store per-session under `.claude/automate-browser/logs/`,
and correlate JS errors with network failures for root-cause analysis: full
recipes in `references/console-log-analysis.md`.
...
node "$SKILL_DIR/screenshot.js" --output ./.claude/automate-browser/screenshots/debug.png
...
Lazy-load, scroll-triggered, and Intersection Observer animation patterns: `references/screenshot-troubleshooting.md`

## Authentication & Cookies

Five methods to access protected pages (inject cookies, browser extension export, Chrome profile, remote debugging, interactive OAuth login): `references/authentication-methods.md`
