...
triggers: ["Word doc", "PDF", "PowerPoint", "Excel", "office document"]
---
...
> For presentation slides from content or outlines, prefer `tkm:generate-slide` (HTML/PPTX via pptxgenjs) — it ships in the `extras` kit (`tkm init --kit extras`). Use `tkm:pptx` when editing or extracting from an **existing** `.pptx` file, or when `extras` is not installed.
