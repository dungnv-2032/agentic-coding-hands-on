...
## Document Translation — needs the `extras` kit

**When:** "translate this document/spreadsheet/book", "dịch file này", "localize this manual", "翻訳"

> `/tkm:translate-file` ships in the `extras` kit, not this one. Install it with
> `tkm init --kit extras`; the commands below then work as written.
...
```
/tkm:document-skills  [extract text from the file]   # base kit
→ /tkm:translate-file <file-path> --target <lang>    # extras kit
```
