...
**Right choice:** `/tkm:estimate` (pass spec document or directory) — ships in the
`extras` kit; install with `tkm init --kit extras`.
...
Reaching for a base skill because `estimate` isn't installed is the same mistake — install `extras`.
