...
## In the `extras` kit (`tkm init --kit extras`)

Not installed by this kit. Install `extras` and these resolve under the same names.
...
| Learning briefing / deep dive | `/tkm:kensan` | From your trusted watchlists |
| Per-component UI specs | `/tkm:generate-ui-specs` | From MoMorph screen / screenshot |
| Transcribe audio / video | `/tkm:audio-transcribe` | → SRT, domain-aware correction |
| Customer-ready improvement proposal | `/tkm:propose-improvements` | Technical + business tracks |

Plus the `/tkm:ki-*` (KPI Insight) and `/tkm:pm-*` (project management) suites — see
`extras`' own `README.md`.
