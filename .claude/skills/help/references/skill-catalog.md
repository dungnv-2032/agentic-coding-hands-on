...
| `/tkm:index-docs` | Index project DOCUMENTS (specs, RFPs, .md/.pdf) into the Graphify knowledge graph — the only skill that adds doc/semantic edges; the session hook covers code only | "index docs", "index tài liệu", "add docs to graph", "build knowledge graph from documents", "graph my specs" |
...
| `/tkm:generate-llms-txt` | Generate llms.txt / llms-full.txt (llmstxt.org standard) — a self-contained, budget-capped, end-user `llms-full.txt` (`--audience user` default, excludes build internals) plus the spec-correct `llms.txt` index; docs-first with a fallback ladder; `--audience dev` restores v1's technical, uncapped output | "generate llms.txt", "llm.txt", "llms.txt", "agent-first doc", "file for agents" |
...
## Takumi-Specific (Project/Client Tools) — `extras` kit

These are **not in this kit.** They ship in the `extras` kit — install with
`tkm init --kit extras`, then invoke them exactly as written below. If the user asks
for one of these and it does not resolve, the fix is to install `extras`, not to
substitute a base skill.
...
| `/tkm:bidding-proposal` | SVN Proposal PPTX from a supplied markdown profile or the Clio Knowledge Graph | "bidding proposal", "SVN proposal", "proposal PPTX", "Clio proposal" |
| `/tkm:generate-testcases` | QA testcases from MoMorph screen or spec | "testcases", "QA cases", "test case sheet" |
| `/tkm:momorph-implement-design` | Pixel-perfect code from MoMorph/Figma screen | "implement this design", "MoMorph screen", "pixel perfect" |

`extras` also ships the **KPI Insight** suite (`/tkm:ki-*`, read-only, needs the
`kpi-insight` MCP server) and the **PM** suite (`/tkm:pm-*` — requirements, stories,
tasks, DoD, schedule, risks, reporting). Run `/tkm:help` after installing `extras`
to see them, or read that kit's `README.md`.
