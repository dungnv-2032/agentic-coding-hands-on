...
The skills below are NOT in this kit — they ship in `extras` (`tkm init --kit extras`).
Route to them by these triggers anyway: if the skill does not resolve, tell the user to
install `extras` rather than substituting a base skill.
