...
- **Graph-aware (optional):** if `graphify-out/graph.json` exists, load
  [`../_shared/graphify-code-graph.md`](../_shared/graphify-code-graph.md) and use
  `graphify query` / `explain` to surface features repeated across documents and
  dependencies between them before reading everything. Specs remain the authority — treat
  graph output as `graph-suggested` until the document confirms it. No graph → extract as
  normal; never block on it. For a doc-heavy bid worth indexing first, `/tkm:index-docs`
  builds the doc/semantic layer (`graphify update` alone covers code only).
...
When a graph exists, `graphify affected` / `path` expose coupling the documents state only
implicitly — a feature many others depend on carries more schedule risk than its size
suggests. On a doc-built graph, pass `--relation shares_data_with --relation references
--relation calls`; `affected`'s default relations are code-only and would report no impact
where impact exists (see the shared rule). Brownfield estimates can also use the code graph to size the blast radius of a
change. Graph findings are leads; the estimate still rests on the documents.
