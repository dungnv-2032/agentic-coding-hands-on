...
class TestMainCli:
    ...
    def test_functional_spec_only_tree_yields_nonzero_units(self, tmp_path):
        """Zero-match glob = silent audit death (phase-04 top risk): a v27.0.0 features tree
        holding ONLY functional-spec.md (no technical-spec.md, no legacy screens.md) must still
        discover at least one doc unit — a glob returning zero matches must fail this test, not
        pass quietly."""
        feat = tmp_path / "docs" / "features" / "F001_Login"
        feat.mkdir(parents=True)
        (feat / "functional-spec.md").write_text(
            "## 1. Overview\n\nLogin feature.\n\n"
            "**Source:** `src/auth/login.py:1-10`\n",
            encoding="utf-8",
        )
        out = tmp_path / "units.json"
        rc = sdu.main(["--project-root", str(tmp_path), "--out", str(out)])
        ...
        data = json.loads(out.read_text())
        assert len(data) > 0
        assert any(u["artifact"] == "functional-spec" for u in data)
