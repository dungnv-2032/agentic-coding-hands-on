...
_DISCOVERY_MAP = [
    # Per-feature functional-spec (BA/QA half, v27.0.0 audience split — decision D-c: this is its
    # OWN doc-unit type, deliberately NOT folded into the technical-spec unit by
    # _merge_screen_spec_into_technical below. functional-spec prose IS in scope for the parity
    # audit as its own unit (sealed default) — it is not narrative-only like business-context.md was.
    ("features/*/functional-spec.md",  "functional-spec", "feature"),
    # Per-feature technical-spec
    ("features/*/technical-spec.md",   "technical-spec",  "feature"),
    # Per-feature screen-spec — the old alias name. `screens.md` (the file this used to alias, folded
    # into functional-spec.md § 5 Screens in v27.0.0) is REMOVED from this map: a glob with zero
    # matches is not an error, it is a silent stop-auditing bug, so the old per-feature screens file
    # must not still be probed for after the layout deleted it.
...
def _infer_artifact(path: Path) -> str:
    ...
    if "functional-spec" in name or "functional_spec" in name:
