...
|------------------|---------------|-------------|
| `docs/features/{F###_Slug}/functional-spec.md` | `functional-spec` | one unit per feature (v27.0.0, decision D-c — its own doc-unit type, NOT folded into the technical-spec unit; in scope for field-diff as its own unit) |
| `docs/features/{F###_Slug}/technical-spec.md` | `technical-spec` | one unit per feature |
| `docs/features/{F###_Slug}/screen-spec.md` | `screen-spec` | folded into the matching technical-spec unit |
...
**v27.0.0 note:** the former `docs/features/{F###_Slug}/screens.md` alias is REMOVED from this map —
that file is deleted (folded into `functional-spec.md` § 5 Screens); a glob with zero matches is not
an error, it is a silent stop-auditing bug, so a deleted alias must not still be probed for.
...
Narrative docs (`system-overview.md`, `architecture.md` prose) are **out of scope** — no structured
field set → no field-diff possible (design § Coverage). `functional-spec.md` is NOT narrative-only
(unlike the deleted `business-context.md` it absorbed): it carries structured field sets of its own
(FR/BR/SM/DEC one-liners, the Screens table, Edge Cases table) — see decision D-c above — so it stays
in scope as its own doc-unit, not folded into this out-of-scope narrative bucket.
