...
description: >
  ...
  delivery data is needed, it delegates to the delivery-tracker (EM) agent via Task and integrates the result.
  Invoked from the REP menu of project-manager, and can be run repeatedly over time to update.
  Owns the PM document tree only — requires project/01_management/ to exist. If the repository
  ...
  progress update, pm-report-project, REP (PM menu).
...
- `plans/project-management/pm-memory.md` — the status of the `REP` row, `## What To Do Next`, and `Last updated` (Step 3)
...
- When **internal delivery data** (each task's actual progress, completion status, blockers) is needed, obtain it by delegating to the `delivery-tracker` (EM) agent via `Task`, and re-integrate it for the customer. Writing to Issues or tasks is left to the EM side; this skill never does it.
...
- `plans/project-management/pm-memory.md` — the state of the `REP` row and `## GitHub Settings` (the repository). **This file is not shipped with the kit** — the `project-manager` agent creates it on first activation. If it is absent, treat every skill as "not started", carry on, and create it from the skeleton in the agent definition when you write the state back
...
- **When the actual progress of internal tasks is needed**: confirm via Block B of `references/hearing.md`, then use `Task` to ask the `delivery-tracker` (EM) agent for "the task progress, completion status and blockers for the target WBS/period", and integrate the result.
...
- The `delivery-tracker` (EM) agent does not respond or returns nothing → put the delegated portion on hold and update what the existing documents show. State the items awaiting the EM as unsettled in the completion summary. Do not block
...
- `plans/project-management/pm-memory.md` — the `REP` state is updated
