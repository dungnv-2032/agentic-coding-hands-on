...
description: |
  ...
  read-only). Invoked from the WF menu of project-manager.
allowed-tools:
  - Read
  - Edit
  - Write
...
The skill that takes the deliverables of the requirements and WBS phases (the function list, the user stories) as input and produces **low-fidelity screen wireframes**, **2–3 options per screen**, for the user to choose from. Invoked from the `WF` menu of the `project-manager` agent.
