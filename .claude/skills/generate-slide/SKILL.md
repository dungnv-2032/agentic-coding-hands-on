...
description: "Generate polished presentation slides from content or outline. Two modes: --html for modern scroll-snap HTML slides (13 style presets incl. 1 official Sun* preset, opens in browser), or --sun/--pptx for Sun* brand-compliant PowerPoint decks (.pptx, optional --image mode). Use when user says 'tạo slide', 'làm presentation', 'gen deck', 'create slides', 'プレゼン作って', or provides an outline/markdown to turn into a deck. NOT for quick dev-internal explanations (use tkm:preview-output --slides)."
category: content
...
**Two output modes:**
- `--html` *(default)* — Self-contained HTML file, scroll-snap navigation, 13 style presets (12 inspired + 1 official brand-locked Sun* preset), opens directly in browser. Best for internal demos, stakeholder walkthroughs, developer-facing content.
...
**Precedence (mode vs. the Sun* preset #13, read top to bottom — explicit flags always win):**
1. Explicit `--sun`/`--pptx` → PPTX mode. Always. This row never changes, regardless of preset #13's existence.
2. Explicit `--html` → HTML mode. Only *within* HTML mode does the Sun* signal ("Mentions Sun*, STVC, BrSE, client, khách hàng") additionally reserve the specialty preview slot for preset #13 — see HTML Phase 2.
3. No mode flag at all, but the Sun* signal is present → still auto-selects `--sun` (row 2 above, unchanged). The unflagged Sun* signal is a PPTX route, full stop; it does not imply HTML mode, and preset #13 is never offered without explicit `--html`.
4. `--html --auto` + Sun* signal → select preset #13 directly, no previews. `--auto` alone (no explicit `--html`) still resolves mode via rows 1-3 first — the existing Sun* → PPTX auto-route is unaffected.
...
**Guaranteed specialty slot for Sun\* Official (#13) — HTML mode only, after explicit `--html` selection.**
This rule fires only once HTML mode is already chosen (normally an explicit `--html`; per Intent
Detection row 3, the unflagged Sun* signal alone still routes to PPTX, never here). When the content/
audience contains the same Sun* signal used for the PPTX auto-route ("Mentions Sun*, STVC, BrSE,
client, khách hàng") — reuse that exact phrase, don't invent a second detection rule — preset #13
**fills the specialty preview slot**, guaranteed, not silently auto-picked and not competing with the
other specialty candidates. The dark and light slots stay contextually selected from presets 1-12 as
usual. This is a fixed rule, not "pick whichever preset seems most relevant" — do not substitute a
different specialty preset when the Sun* signal is present.
- **`--html --auto` + Sun* signal:** select preset #13 directly, skip the 3-preview step entirely.
- **`--auto` with no explicit `--html`:** mode detection (Intent Detection rows 1-3) runs first and
  keeps the existing Sun* → PPTX route — this guaranteed-slot rule never overrides that.
- No Sun* signal present: behave exactly as before (3 contextually-picked previews from presets 1-12,
  preset #13 not offered).
...
- **If preset #13 (Sun\* Official) was selected:** also load `references/sun-official-components.md` —
  it is the full component catalog for this preset (23 slide layouts, both variants, the 2 brand asset
  data URIs, the CSS bg-art layer, the image contract) and is required reading, not optional context.
  The generated `<html>` element **must** carry `data-preset="sun-official"` (every selector in that
  file is scoped to it) plus `data-variant`/`data-footerlogo`/`data-pageno`/`data-artwork`/`data-anim`
  per that file's attribute contract. Pick component snippets from its catalog per content shape —
  it's a picker menu like `layouts.md`, not a fixed 23-slide deck.
...
| Sun\* Official preset ships React/bundler leftovers | `<image-slot>`, `<deck-stage>`, `data-deck-active`, or a bundler asset UUID in output — none belong; see "Not ported" in `sun-official-components.md` |
| Sun\* Official CSS collides with base template | Every selector must be scoped under `html[data-preset="sun-official"]` — an unscoped `.slide`/`.title`/`.badge` rule will bleed into other presets |
...
| `references/sun-official-components.md` | HTML Phase 3, only when preset #13 (Sun\* Official) is selected — full 23-component catalog |
