...
## Stagger Delay Modifiers (`.d1`–`.d6`)

Add alongside `.reveal` (or any preset's own reveal class) when children should fade in one after
another instead of all at once — e.g. a bulleted list, a card grid, an agenda. Pair with a matching
`class="reveal d2"` etc. rather than relying on `:nth-child`, so stagger order can differ from DOM order
(e.g. two elements in the same visual "wave" both get `d2`).

```css
.reveal.d1, .reveal-scale.d1, .reveal-left.d1, .reveal-blur.d1 { transition-delay: 0.05s; }
.reveal.d2, .reveal-scale.d2, .reveal-left.d2, .reveal-blur.d2 { transition-delay: 0.14s; }
.reveal.d3, .reveal-scale.d3, .reveal-left.d3, .reveal-blur.d3 { transition-delay: 0.24s; }
.reveal.d4, .reveal-scale.d4, .reveal-left.d4, .reveal-blur.d4 { transition-delay: 0.34s; }
.reveal.d5, .reveal-scale.d5, .reveal-left.d5, .reveal-blur.d5 { transition-delay: 0.44s; }
.reveal.d6, .reveal-scale.d6, .reveal-left.d6, .reveal-blur.d6 { transition-delay: 0.54s; }
```

Used by the Sun* Official preset (`references/sun-official-components.md`) in place of a bespoke stagger
class — available to any other preset that wants the same six-step cadence.
