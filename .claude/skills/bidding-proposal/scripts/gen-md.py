...
def _emit_cover(p: ProjectProfile) -> str:
    cover = p.cover
    if not (cover.company or cover.date):
        return ''
    parts = ['## Cover']
    if cover.company:
...
def _emit_agenda(p: ProjectProfile) -> str:
    agenda = p.agenda
    if not (agenda.chapters or agenda.cost_breakdown):
        return ''
    parts = ['## Agenda']
    if agenda.chapters:
        parts.append(f'### Chapters\n{_emit_list(agenda.chapters)}')
    if agenda.cost_breakdown:
        parts.append(f'### Cost Breakdown\n{_emit_list(agenda.cost_breakdown)}')
    return _nonempty(parts)
