...
| open (`filter-menu-hashtag` / `filter-menu-department`) | Bấm nút lọc (component `KudosFilterMenu`, dùng chung cho cả hai) | Option căn giữa, hover đổi con trỏ pointer + nền nhạt; option nhận focus bàn phím hiện glow `#FAE287` (FR-215); **cả hai** khối cuộn trong hộp cao tối đa 348px (6 dòng) — mặc định của `KudosFilterMenu`, không đẩy layout trang (FR-214) | chọn option (đóng dropdown ngay), hoặc chọn lại option đang chọn để bỏ lọc | `app/kudos/_components/kudos-filter-menu.tsx` |
