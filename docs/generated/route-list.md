...
| /awards-information | `Page` → `<ComingSoon />` (`app/awards-information/page.tsx`) | ROUTE005 | F002 | ƒ dynamic | Public |
...
**Why every route is ƒ dynamic**: the five `ComingSoon` placeholders (`/awards-information`,
`/kudos`, `/standards`, `/profile`, `/admin`) call `getPageContext()`
(`app/_components/coming-soon.tsx:18`), which calls `cookies()` (`app/_page-context.ts:28`) —
`cookies()` opts a route out of static generation. `/` reaches the same call via `app/page.tsx:24-25`.
