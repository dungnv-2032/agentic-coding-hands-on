...
2. Trên mọi route dùng `HomeHeader` — `/`, `/awards-information`, `/kudos`, `/kudos/new`, `/profile`, `/standards`, và 2 route `ComingSoon` còn lại (`/admin`, `/kudos/[id]`) — `AccountMenu` nhận `signOutAction` truyền từ page (chính là `signOut` import từ `app/_actions/auth.ts`) và đặt trong `<form action={signOutAction}>` cuối menu. `app/page.tsx:1,37`, `app/awards-information/page.tsx:3,74`, `app/_components/coming-soon.tsx:1,29`, `app/_components/account-menu.tsx:108-112`. `F002` `US006`. *Từ 2026-09-10 (F009): `/kudos/secret-box` không còn dùng `HomeHeader`/`AccountMenu` — màn mới là một thẻ đơn không shell (`docs/screens/SCR009_OpenSecretBox/spec.md`), nên route này rời khỏi danh sách trên.*
