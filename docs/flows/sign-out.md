...
2. Trên `/` (và mọi route dùng `ComingSoon`), `AccountMenu` nhận `signOutAction` truyền từ `HomePage`/`ComingSoon` (chính là `signOut` import từ `app/_actions/auth.ts`) và đặt trong `<form action={signOutAction}>` cuối menu. `app/page.tsx:1,37`, `app/_components/coming-soon.tsx:1,29`, `app/_components/account-menu.tsx:108-112`. `F002` `US006`
