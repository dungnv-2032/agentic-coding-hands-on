...
10. Khối quảng bá Kudos điều hướng tĩnh tới `/kudos` — một liên kết cố định, không có decision logic; từ 2026-09-07 render màn hình Kudos Live Board thật (F004), đã thay `ComingSoon`. `app/_components/kudos-promo.tsx:46-48`. Nút widget nổi (R7) — từ 2026-09-10 không còn hai liên kết trực tiếp: bấm pill (`fab-trigger`) mở một menu ba lựa chọn (F008_FloatingActionButton) — `Thể lệ` → `/standards` (màn Thể lệ thật, F007, từ 2026-09-09), `Viết KUDOS` → `/kudos/new` (khác `/kudos` — bị `proxy.ts` gác, khách chưa đăng nhập bị chuyển hướng `/login`), và `Hủy` đóng menu, trả focus về pill. `app/_components/floating-widget.tsx`, chi tiết ở `docs/features/F008_FloatingActionButton/functional-spec.md`.
