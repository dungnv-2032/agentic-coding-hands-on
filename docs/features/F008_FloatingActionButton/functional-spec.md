...
- **FR-205** Nút `Hủy` đóng nhóm nút, quay về pill, và trả focus về pill qua callback `closeAndRestoreFocus` riêng của nó — không dùng chung đường bấm-ra-ngoài.
...
- `Hủy` và `Escape` đều trả focus về pill — `Hủy` qua callback `closeAndRestoreFocus` riêng, `Escape` qua hợp đồng `useDismissOnOutside`; bấm ra ngoài thì không di chuyển focus (BR-004)
...
- **FR-205** → Bấm `Hủy` đóng menu, quay về pill, và trả focus về pill.
