...
| **A3** | `ComingSoon#render` | `GET` `/awards-information`, `/kudos`, `/standards`, `/profile`, `/admin` | FR-002, US005 | — *(read-only)* | § 3.1 |
...
`GET` `/awards-information` \| `/kudos` \| `/standards` \| `/profile` \| `/admin` → `` `ComingSoon#render` ``
`FR-002` `US005`

**Who** · Bất kỳ ai điều hướng ra từ trang chủ (nav, CTA, thẻ giải thưởng, widget, footer, mục "Admin Dashboard" role-gated)
**FE** · Một component `ComingSoon` (`app/_components/coming-soon.tsx`) dùng chung cho cả 5 route, tái dùng `HomeHeader`/`SiteFooter` thật (không phải trang tĩnh cô lập) để giữ trải nghiệm điều hướng nhất quán — kể cả bộ chọn ngôn ngữ và (nếu đã đăng nhập) chuông/tài khoản.
...
**Rule** · Không có decision logic — 5 route này là shell placeholder theo tuyên bố (`clarifications.md` ORCH-11), không mô phỏng hành vi màn hình đích thật.
**Result** · Chỉ render — không ghi dữ liệu.
**Source:** `app/_components/coming-soon.tsx`, `app/awards-information/page.tsx`, `app/kudos/page.tsx`, `app/standards/page.tsx`, `app/profile/page.tsx`, `app/admin/page.tsx`
...
| A3 | Người dùng bấm liên kết ra khỏi trang chủ (nav/CTA/thẻ/widget/footer/menu tài khoản) | Luôn tới một trong 5 route thật, dùng chung `ComingSoon` — không có liên kết 404 (test ID-59) |
...
| `ComingSoon` | Shell placeholder dùng chung cho 5 route đích | A3 | `app/_components/coming-soon.tsx` |
...
- **SC-006** *(A3)* Tải cả 5 route placeholder → không có lỗi 404, mỗi route render `ComingSoon` với header/footer thật (covers FR-002)
...
- *(A3)* `ComingSoon` là shell thật (không phải stub) — mọi assertion trên trang chủ chạy trên code thật của A1/A2, không phụ thuộc vào nội dung của A3.
...
| A3 | `app/_components/coming-soon.tsx`, `app/{awards-information,kudos,standards,profile,admin}/page.tsx` | toàn file |
