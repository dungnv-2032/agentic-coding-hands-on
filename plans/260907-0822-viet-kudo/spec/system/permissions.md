...
(`TBD (draft)`) — khách vãng lai bị đẩy về `/login`.
...
- **`insert` trên `kudos`, `kudos_hashtags`, `kudos_attachments`, `sunners` — mới, `TBD (draft)`.**
  ...
  Đây chính là thứ chặn việc gửi kudos mạo danh người khác — client có gửi `sender_id` gì cũng
  không vượt được. `kudos_hashtags` / `kudos_attachments` kiểm quyền sở hữu dòng `kudos` cha.
  `sunners` mở `insert` để một người vừa đăng nhập tự tạo được danh tính của chính mình
  (`with check` khớp `auth_user_id` = `auth.uid()`), không tạo hộ ai khác.
...
- **Bucket Storage cho ảnh đính kèm** có policy riêng: `authenticated` ghi được, đọc công khai (ảnh
  hiện trên bảng công khai). Kiểu file được kiểm **cả ở server**, vì client không phải biên.
