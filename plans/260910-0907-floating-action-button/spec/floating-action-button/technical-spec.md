...
**FE** · Nút tròn đỏ `Hủy` (`data-testid="fab-close"`), 56×56 bo tròn hết cỡ, nền `#D4271D`, icon 24×24 — tái dùng nguyên `public/images/rules/close-icon.svg` (đã đúng màu trắng trên nền đỏ), không tạo bản sao. Gắn `app/_components/use-dismiss-on-outside.ts` nguyên trạng cho `Escape` và bấm-ra-ngoài — đúng hợp đồng đang phục vụ chuông thông báo, account menu và language selector.
**Rule** · **BR-004 — `Escape` trả focus về pill; bấm ra ngoài thì không.** Cùng hợp đồng `useDismissOnOutside` đã chứng minh ở ba popover khác trên header — dùng lại nguyên, không viết lại logic dismiss riêng. *(inline)*
...
- **SC-003** *(A2)* `fab-close`, `Escape`, và bấm ra ngoài đều đưa `aria-expanded` về `"false"`; riêng `Escape` trả focus về `fab-trigger` (covers FR-205, FR-401, BR-004)
...
|----------|------|------------|----------|
| Feature List | [feature-list.md](../../../../docs/generated/feature-list.md) | F008 (provisional — allocated at promote) | [ ] |
