...
> **Curated, plain-language view.** Forward-draft viết trước khi code Kudos Live Board (SDD). Mã
> `PERM###` chính thức nằm ở [`docs/generated/permissions-matrix.md`](../generated/permissions-matrix.md);
> trang này dùng lại mã đã có và **không tự cấp phát mã mới** — policy RLS mới ghi là
> `TBD (draft)` cho tới khi Core pass gán mã thật.
...
**Ranh giới ở tầng database (RLS — mới)** — `TBD (draft)`:
- `select` trên toàn bộ bảng Kudos mở cho `anon` và `authenticated`. Dữ liệu trên màn hình này là
  lời cảm ơn công khai trong nội bộ Sun*; không có row nào riêng tư theo người xem.
- `insert` / `delete` trên `kudos_likes` chỉ mở cho `authenticated`, và `with check` /
  `using` bắt buộc `(select auth.uid()) = user_id`. Người dùng không tạo hay xoá được like của
