...
# F000_HomepageSaa — Technical Spec
...
Trang chủ công khai SAA 2025 tại `/`, thay thế trang boilerplate `create-next-app`. Server Component đọc cookie `NEXT_LOCALE` (tái dùng cơ chế của F001_Login) và phiên đăng nhập (nếu có) để render header/hero/nội dung tĩnh; countdown và hai menu header (thông báo, tài khoản) là tương tác phía client, không có round-trip HTTP mới. Bốn route placeholder dùng chung một `ComingSoon` component để không có liên kết gãy.
...
| **A1** | `HomePage#render` (planned) | `GET` `/` | FR-101, FR-102, FR-201, FR-202, FR-203, FR-204, FR-205, FR-206, FR-207, FR-401, FR-402, FR-405, DEC-001, US001 | — *(read-only)* | § 3.1 |
| **A2** | header notification/account menu *(planned, client-only)* | *(client state — no HTTP)* | FR-403, FR-404, FR-601, BR-002, DEC-002, US002 | — *(client state only)* | § 3.1 |
| **A3** | `ComingSoonPage#render` (planned) | `GET` `/awards-information`, `/kudos`, `/standards`, `/profile` | FR-002, US001 | — *(read-only)* | § 3.1 |
...
#### A1 · Render trang chủ
`GET` `/` → `` `HomePage#render` `` (planned)
`FR-101` `FR-102` `FR-201` `FR-202` `FR-203` `FR-204` `FR-205` `FR-206` `FR-207` `FR-401` `FR-402` `FR-405` `DEC-001` `US001` · `SCR-homepage`

**Who** · Khách truy cập (đã đăng nhập hay chưa đều xem được)
**FE** · Server Component render 7 khối theo thứ tự thiết kế: header (logo, 3 mục nav, bộ chọn ngôn ngữ tái dùng F001_Login), hero (keyvisual "ROOT FURTHER", nhãn "Coming soon", 3 ô đếm ngược DAYS/HOURS/MINUTES, thông tin sự kiện, 2 CTA), khối nội dung "Root Further", lưới 6 thẻ giải thưởng, khối quảng bá Sun* Kudos, nút widget nổi, footer. Bộ đếm ngược là Client Component con: server render ô `00` placeholder, client tự tính lại trong `useEffect` sau mount (tránh lệch giờ server/client, không cần `suppressHydrationWarning`).
**Request** · không có tham số — trang tĩnh, không dùng `searchParams`/`cookies()` ngoài cookie ngôn ngữ đã có ở F001_Login.
**BE** · không có — thuần render phía server, nội dung giải thưởng/Kudos là dữ liệu tĩnh lấy từ thiết kế (mock data source theo clarifications.md, không phải bảng DB).
**Rule** · Quyết định cuộn lên đầu hay điều hướng khi bấm mục nav:

| DEC | subtype | Condition | What the user sees | Source |
|---|---|---|---|---|
| **DEC-001** | interaction | `navItem.isSelected` (mục "About SAA 2025" đang ở trang chủ) | Bấm mục đang chọn → cuộn mượt lên đầu trang thay vì điều hướng lại | `TBD (draft)` |
...
#### A2 · Chuông thông báo và menu tài khoản *(planned, client-only)*
*(client state — no HTTP)* → header notification/account menu (planned)
`FR-403` `FR-404` `FR-601` `BR-002` `DEC-002` `US002`

**Who** · Người dùng đã đăng nhập (không hiện với khách chưa đăng nhập)
**FE** · Chuông thông báo (Client Component) toggle một panel hiển thị "Không có thông báo mới" khi bấm; biểu tượng tài khoản (Client Component) toggle một menu Profile / Sign out / Admin Dashboard (điều kiện). Cả hai đều là state cục bộ (`useState`), không gọi API.
**Request** · không có — không có HTTP request nào phát sinh từ hai tương tác này.
**BE** · không có — Sign out gọi lại server action đăng xuất hiện có của F001_Login (đã chuyển tới `app/_actions/auth.ts`), không phải logic mới của action này.
**Rule** · Quyết định hiện mục Admin Dashboard:

| DEC | subtype | Condition | What the user sees | Source |
|---|---|---|---|---|
| **DEC-002** | render | `session != null` AND `user.app_metadata.role === "admin"` | Thêm mục "Admin Dashboard" vào cuối menu tài khoản | `TBD (draft)` |

**BR-002 — Chuông thông báo và biểu tượng tài khoản chỉ render khi có phiên đăng nhập hợp lệ.** Không có state ẩn/hiện dựa trên props — hai phần tử này hoàn toàn không có trong cây component khi `session == null`. *(§ 4.4)*
...
`GET` `/awards-information` \| `/kudos` \| `/standards` \| `/profile` → `` `ComingSoonPage#render` `` (planned)
`FR-002` `US001`

**Who** · Bất kỳ ai điều hướng ra từ trang chủ (nav, CTA, thẻ giải thưởng, widget, footer)
**FE** · Một component `ComingSoon` dùng chung cho cả 4 route, tái dùng header/footer thật (không phải trang tĩnh cô lập) để giữ trải nghiệm điều hướng nhất quán.
**Request** · không có tham số.
**BE** · không có — render tĩnh, không có nội dung thật (ngoài phạm vi tính năng này — xem functional-spec.md § 1 Non-Scope).
**Rule** · Không có decision logic — 4 route này là shell placeholder theo tuyên bố, không mô phỏng hành vi màn hình đích thật.
...
| A3 | Người dùng bấm liên kết ra khỏi trang chủ (nav/CTA/thẻ/widget/footer) | Luôn tới một route thật, dùng chung `ComingSoon` — không có liên kết 404 (test ID-59) |
...
|---|---|---|---|
| `HomeHeader` (planned) | Logo, 3 mục nav, chuông + tài khoản (điều kiện), bộ chọn ngôn ngữ (tái dùng) | A1, A2 | `app/_components/home-header.tsx` (planned) |
| `LanguageSelector` (reused từ F001_Login, promoted) | Dropdown chọn ngôn ngữ VN/EN | A1 | `app/_components/language-selector.tsx` (planned — di chuyển từ `app/login/_components/`) |
| `CountdownTimer` (planned) | Tick mỗi giây, tính từ `NEXT_PUBLIC_EVENT_START_AT` qua `ALG-001` | A1 | `app/_components/countdown-timer.tsx` (planned) |
| `AwardsGrid` (planned) | Lưới 6 thẻ giải thưởng, responsive 3/2 cột | A1 | `app/_components/awards-grid.tsx` (planned) |
| `NotificationBell` (planned) | Toggle panel "Không có thông báo mới" | A2 | `app/_components/notification-bell.tsx` (planned) |
| `AccountMenu` (planned) | Toggle menu Profile / Sign out / Admin Dashboard (điều kiện) | A2 | `app/_components/account-menu.tsx` (planned) |
| `ComingSoon` (planned) | Shell placeholder dùng chung cho 4 route đích | A3 | `app/_components/coming-soon.tsx` (planned) |
...
**Source:** `TBD (draft)` · route guard đã cite ở F001_Login § 4.4
...
#### US001_BrowseHomepage *(A1, A3)*

**Independent Test:** Tải `/` không cần đăng nhập, xác nhận đủ 7 khối theo đúng thứ tự và mọi liên kết dẫn tới một route thật (kể cả 4 placeholder).
...
#### US002_AccountNotificationMenu *(A2)*
...
- *(A1)* Nội dung giải thưởng/Kudos/hero là dữ liệu tĩnh biên soạn từ thiết kế MoMorph (mock data source theo clarifications.md) — không có API/bảng DB thật đứng sau; nếu sau này cần dữ liệu động, đây là một tính năng khác.
- *(A2)* Role admin đọc từ `app_metadata.role` vì Supabase local chưa có bảng role riêng (assumption A2 trong clarifications.md) — nếu dự án sau này thêm bảng phân quyền, action này cần đọc lại nguồn khác.
- *(A3)* `ComingSoon` là shell thật (không phải stub) — mọi assertion trên trang chủ chạy trên code thật của A1/A2, không phụ thuộc vào nội dung của A3 (assumption A3 trong clarifications.md).

### 5.3 Unresolved Questions

1. **Tên file/thư mục chính xác cho các component đã promote lên `app/_components/`** *(A1, A2)*: tên dự kiến ở trên (`home-header.tsx`, `language-selector.tsx`, v.v.) có thể đổi khi implement, do chưa có source code thật.
2. **Nguồn dữ liệu tĩnh cho 6 thẻ giải thưởng và khối Kudos** *(A1)*: chưa xác định sẽ là hằng số TypeScript trong component hay một file JSON riêng — cả hai đều thoả "mock data source" của clarifications.md; quyết định thuộc lúc implement.
