...
> F001_Login và F002_HomepageSaa. Mọi mã `PERM###`/`ROUTE###` để `TBD (draft)` — dự án chưa có
> generated permissions-matrix.md; máy cấp phát mã khi bước reconcile/promote chạy, không đoán
> ở đây.
...
- Ranh giới duy nhất được `proxy.ts` thực thi (route-guard, chạy trên mọi request trừ asset
  tĩnh) là **có session hay không** — chia đúng hai route: `/todo` (cần session) và `/login`
  (bounce khi đã có session). Đây là ranh giới thật, có kiểm tra ở edge/server
  (`proxy.ts:41-46`).
...
- Ranh giới "admin" **không phải** một ranh giới truy cập theo nghĩa hệ thống access-control —
  nó là một điều kiện hiển thị UI (`app_metadata.role === "admin"` quyết định một `<Link>` có
  render hay không trong `account-menu.tsx:99-107`). Không có route, Server Action, hay lần đọc
  dữ liệu nào kiểm tra lại điều kiện này. Bất kỳ ai gõ thẳng URL `/admin` đều vào được, có
  session hay không, có role admin hay không.
- Ẩn/hiện chuông thông báo và biểu tượng tài khoản theo trạng thái đăng nhập (
