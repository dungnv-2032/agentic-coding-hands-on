...
| **A1** | `StandardsPage#Page` (planned) | `GET` `/standards` | FR-002, FR-003, FR-201, FR-202, FR-203, FR-204, FR-205, FR-403, FR-405, BR-002, BR-003, US001 | — *(read-only)* | § 3.1 |
| **A2** | `RulesPanelDismiss` (planned, client) | *(no request — history/DOM only)* | FR-401, FR-404, BR-004, US002 | — | § 3.2 |
| **A3** | `Viết KUDOS` link (planned, static `<a>`) | `GET` `/kudos/new` *(navigation)* | FR-402, BR-005, US003 | — | § 3.3 |
...
`GET` `/standards` → `` `StandardsPage#Page` `` (planned)
...
**BE** · `getRulesContent()` (planned, `lib/rules/rules-data.ts`) tạo đúng một client mỗi request
...
*(không có request — chỉ lịch sử trình duyệt)* → `` `RulesPanelDismiss` `` (planned, Client Component)
`FR-401` `FR-404` · `US002`
...
`router.push("/")`. Phân biệt hai trường hợp bằng `window.history.length > 1` đọc tại thời điểm bấm —
`document.referrer` không dùng được vì điều hướng client-side của Next không cập nhật nó.
**Rule**
...
`GET` `/kudos/new` *(navigation)* → `` `<a href="/kudos/new">` `` (planned, static)
`FR-402` · `US003`
...
- **FR-001 / FR-601 — RLS public-read là tiền lệ ràng buộc.** `docs/system/permissions.md` ghi rõ mọi bảng
  ...
  minh, không policy ghi (im lặng là từ chối). Cần một mã `PERM###` mới trong
  `docs/generated/permissions-matrix.md` — `doc-writer` cấp lúc delivery.
...
> **LƯU Ý (phase 08).** Đây là bản DRAFT đầu vào của plan, giữ nguyên làm dấu vết lịch sử.
> A1 và A2 bên dưới **đều đã bị bác bỏ khi triển khai**. Bản đúng, đã sửa, nằm ở
> `docs/features/F007_TheLe/technical-spec.md § 5.2` — đó mới là tài liệu của record.
> Tóm tắt: A1 sai vì `about:blank` của tab bị lái vẫn nằm trong history (nay dùng
> `navigation.canGoBack`); A2 sai vì file bút có sẵn là `fill="white"`, vô hình trên nền vàng.

- **A1** — `window.history.length > 1` là tín hiệu đủ tốt để phân biệt "có chỗ quay lại" với "deep link".
  Nó over-report trong một tab đã duyệt nhiều trang khác trước khi tới thẳng `/standards`; hệ quả xấu nhất
  là quay về trang trước đó thay vì `/`, vẫn không phải ngõ cụt (`BR-004` giữ nguyên).
- **A2** — `pen-icon.svg` kéo về trong clarification gate trùng node id với `public/images/home/widget-pen-icon.svg`
  (`186:1763`). Giả định: hai file giống hệt nhau và file có sẵn được tái dùng. Phải diff trước khi ship bản
  copy — nếu khác thì giữ bản mới và ghi lý do.

### 5.3 Unresolved Questions

- Chưa cấp mã `PERM###` cho hai bảng mới. `docs/generated/permissions-matrix.md` là registry;
  `doc-writer` cấp lúc delivery.
