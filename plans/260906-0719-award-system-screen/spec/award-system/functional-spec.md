...
> **PROMOTED 2026-09-06** → `docs/features/F003_AwardSystem/functional-spec.md`. Bản này là bản nháp lịch sử của phase 01; bản trong `docs/` là bản có hiệu lực và đã đối chiếu với code đã ship. Đừng sửa file này.
...
**Problem:** Trang chủ SAA 2025 (F002_HomepageSaa) chỉ giới thiệu sáu hạng mục giải thưởng bằng một dòng mô tả cắt ngắn, rồi trỏ người đọc sang `/awards-information` — nơi hiện chỉ có shell "Coming soon". Người đọc muốn biết mỗi hạng mục vinh danh ai, có bao nhiêu giải và giá trị bao nhiêu thì không có chỗ nào trả lời.
**Solution:** Màn hình công khai "Hệ thống giải" tại `/awards-information` — thay chỗ shell placeholder hiện tại — gồm hero tiêu đề mùa giải, một menu danh mục dính bên trái, sáu thẻ chi tiết giải thưởng xen kẽ trái/phải, rồi khối quảng bá Sun* Kudos và footer dùng lại nguyên trạng của trang chủ.
**Scope:** Trình bày đầy đủ nội dung sáu hạng mục (mô tả, số lượng, giá trị giải); menu danh mục tự sáng theo vị trí cuộn và bấm được để nhảy tới hạng mục tương ứng; nhận deep link `#<slug>` từ trang chủ và sáng đúng mục menu; đủ hai ngôn ngữ VN/EN.
**Non-Scope:** Không có nội dung thật của Sun* Kudos — CTA "Chi tiết" vẫn tới shell placeholder `/kudos` (xem § 11 RISK-02). Không xuất ảnh mới: sáu ảnh giải thưởng và các ảnh nền đã có sẵn từ F002. Không sửa header/footer — trạng thái "Award Information" đang chọn đã tự đúng trên route này. Không có API, không có bảng dữ liệu, không có thao tác ghi. Không render nút widget nổi trên màn hình này.
...
> **Cấp mã (traceability)**: hai user story soạn cục bộ cho tính năng này nhận mã global kế tiếp còn trống — `US007`, `US008` (`docs/generated/user-stories.md` đang dừng ở `US006` của F002). Màn hình nhận `SCR003_AwardSystem` (`docs/generated/screen-list.md` đang dừng ở `SCR002_Homepage`). Bản nháp này ghi sẵn mã dự kiến; việc đăng ký chính thức vào state file diễn ra lúc promote, không phải lúc soạn spec.

## 3. Open Decisions

| D### | Decision | Default proposal | Rationale | Blocks work |
|------|----------|-------------------|-----------|--------------|
| D001 | Test case ID-1 yêu cầu khách chưa đăng nhập vào màn hình này thì bị đẩy về trang đăng nhập. Hành vi đang chạy thì ngược lại: route công khai. Chốt thế nào? | Giữ công khai — không thêm guard (FR-001, FR-601) | Lối vào duy nhất tới màn hình này là trang chủ công khai; chặn đích trong khi để ngỏ nguồn là vô nghĩa, và sẽ làm gãy sáu test trang chủ đang xanh vốn điều hướng tới đây ở trạng thái chưa đăng nhập *(clarifications.md — quyết định thứ hai; test ID-0, ID-1)* | no |

## 4. Requirements

### Foundation (0xx)

- **FR-001** `/awards-information` phải là route công khai — không thêm guard mới; shell "Coming soon" hiện tại ở route này bị thay hẳn bằng màn hình thật. *(clarifications.md — quyết định route và quyết định auth; test ID-0)*
- **FR-002** Toàn bộ nội dung màn hình phải có đủ hai ngôn ngữ VN và EN, và một khoá thiếu ở một ngôn ngữ phải là lỗi biên dịch chứ không phải khoảng trắng lúc chạy. VN chép nguyên văn từ thiết kế, EN là bản dịch sát nghĩa. *(clarifications.md — "i18n coverage")*

### Navigation (1xx)

- **FR-101** Mục "Award Information" trên header phải ở trạng thái đang chọn khi người dùng ở màn hình này — có sẵn nhờ cách header tự suy ra mục đang chọn từ đường dẫn hiện tại, nên không sửa gì ở header. *(clarifications.md — "Shared chrome" và ORCH-03)*
- **FR-102** Vào thẳng `/awards-information#<slug>` (đường dẫn mà mọi thẻ giải thưởng trên trang chủ đang trỏ tới) phải vừa dừng ở đúng thẻ giải thưởng đó, vừa làm sáng đúng mục menu tương ứng — không được kẹt ở mục đầu danh sách. *(clarifications.md — "Deep-link arrival"; test ID-11)*
...
- **FR-202** Hero hiển thị ảnh keyvisual, chữ hình "ROOT FURTHER", và cụm tiêu đề canh giữa: dòng nhỏ `Sun* Annual Awards 2025` màu trắng, một đường kẻ mảnh, rồi tiêu đề vàng `Hệ thống giải thưởng SAA 2025`. *(spec item A)*
- **FR-203** Menu danh mục bên trái dính theo trang cuộn, gồm đúng sáu mục theo thứ tự thiết kế — Top Talent, Top Project, Top Project Leader, Best Manager, Signature 2025 Creator, MVP — mỗi mục có một biểu tượng 24×24 phía trước; mục đang chọn hiện chữ vàng kèm gạch chân vàng. *(spec item C)*
...
- **FR-207** Khối Sun* Kudos ở cuối trang dùng lại y nguyên khối đã có trên trang chủ — cùng dòng nhỏ, tiêu đề, phụ đề, đoạn mô tả và nút `Chi tiết` dẫn sang `/kudos`. *(spec item D1/D2/D2.1; clarifications.md — "Shared chrome")*
...
- **FR-601** Màn hình chỉ chứa nội dung công khai của mùa giải — không có dữ liệu người dùng, không có thao tác ghi — nên không cần và không được thêm bất kỳ lớp chặn đăng nhập nào. Yêu cầu ngược lại ở test ID-1 được ghi làm quyết định còn mở (§ 3 D001), không phải làm yêu cầu triển khai. *(clarifications.md — quyết định auth; test ID-1)*

## 5. Business Rules

- Màn hình luôn công khai, không có guard mới ngoài route guard sẵn có của F001_Login — FR-001, FR-601 (BR-001)
- Menu danh mục luôn có đúng một mục đang chọn; nguồn quyết định theo thứ tự là cú bấm gần nhất, rồi hash lúc mở trang, rồi vị trí cuộn hiện tại — FR-102, FR-401, FR-402 (BR-002)
...
- Mở trang có `#<slug>` hợp lệ thì mục menu tương ứng sáng ngay; không có hash hoặc hash không khớp hạng mục nào thì mặc định là mục đầu tiên — FR-102 (DEC-002)
...
2. Trang mở ra ở đúng hạng mục đó, menu bên trái sáng đúng mục tương ứng; nếu vào thẳng không kèm hash thì trang mở từ hero và mục đầu tiên sáng.
...
**Business value:** Trả lời trọn vẹn câu hỏi mà trang chủ mới chỉ gợi ra, và gỡ bỏ shell "Coming soon" mà mọi thẻ giải thưởng trên trang chủ đang dẫn tới.

**Acceptance Criteria:**
- [ ] Trang hiển thị đủ năm khối theo đúng thứ tự thiết kế: header, hero, phần hệ thống giải, khối Sun* Kudos, footer.
- [ ] Sáu thẻ giải thưởng hiện đúng thứ tự thiết kế, mỗi thẻ đủ ảnh, tiêu đề, mô tả, số lượng và giá trị giải.
- [ ] Thẻ Signature hiện hai mức giải nối bằng `Hoặc`; Best Manager và MVP không có dòng ghi chú dưới số tiền.
- [ ] Đổi sang EN thì toàn bộ nội dung màn hình đổi theo, không còn chuỗi tiếng Việt sót lại.
...
**Acceptance Criteria:**
- [ ] Bấm một mục menu cuộn tới đúng thẻ tương ứng và đổi địa chỉ trang thành `#<slug>` của hạng mục đó.
- [ ] Trong lúc cuộn tay, mục menu sáng luôn khớp hạng mục đang hiện trên màn hình, và chỉ có đúng một mục sáng.
- [ ] Vào thẳng `/awards-information#mvp` thì mục MVP sáng ngay khi trang mở, không phải mục đầu danh sách.
- [ ] Người dùng bật chế độ giảm chuyển động thì cú nhảy là tức thì, không có hiệu ứng cuộn.
...
|----------|--------------|----------------------|
| Mở trang với hash không khớp hạng mục nào | Bỏ qua hash, mở từ đầu trang, mục menu đầu tiên sáng | "None — silent handling" |
...
| Cuộn tới cuối trang, thẻ cuối không cao bằng một màn hình | Mục menu cuối vẫn sáng — mục sáng là hạng mục gần đỉnh vùng nhìn nhất, không phải hạng mục chiếm nhiều diện tích nhất | "None — silent handling" |
| Trang đã hiện nhưng phần tương tác chưa sẵn sàng | Sáu thẻ và menu vẫn đọc được vì là nội dung tĩnh; mục menu chưa tự sáng theo cuộn cho tới khi tương tác sẵn sàng | "None — silent handling" |
...
- **FR-102** → Vào thẳng bằng `#<slug>` của từng hạng mục phải sáng đúng mục menu tương ứng ngay khi trang mở.
...
|----|------|--------------|--------|--------|
| RISK-01 | risk | Test case ID-1 yêu cầu chặn khách chưa đăng nhập, trái với hành vi công khai đang chạy. Bản spec này cố ý không triển khai và ghi lại thành quyết định còn mở (§ 3 D001) thay vì im lặng bỏ qua. | Nếu sau này chủ sản phẩm chốt phải chặn, sẽ phải chặn cả trang chủ và viết lại sáu test trang chủ đang xanh — phạm vi lớn hơn hẳn màn hình này. Quyết định đang chờ chủ sản phẩm — xem § 3 D001. | [EXPECTED] |
| RISK-02 | risk | Nút `Chi tiết` của khối Sun* Kudos dẫn sang `/kudos`, vốn vẫn là shell placeholder (test ID-14 nhắm tới màn hình Kudos thật). | Liên kết không gãy nhưng người đọc chưa tới được nội dung Kudos thật; đây là phạm vi của một tính năng khác. | [EXPECTED] |
| RISK-03 | risk | Sáu ảnh giải thưởng dùng lại nguyên các tệp đã có từ F002 thay vì xuất mới từ thiết kế (assumption A1). | Nếu khâu kiểm thử hình ảnh phát hiện lệch, phải xuất lại ảnh — việc bó hẹp, không chặn phần còn lại của màn hình. | [EXPECTED] |
...
| `@playwright/test` | infrastructure | Cần cho `testPolicy: e2e-red-first` — màn hình có chuyển trạng thái thật (bấm menu → cuộn + đổi mục sáng, scroll-spy, deep link) | clarifications.md — "Test policy" |
